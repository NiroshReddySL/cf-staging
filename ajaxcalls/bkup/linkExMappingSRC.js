...
function getLinkExPreMigrationUsers(pageNo, from) {
  ...
  $.ajax({
    type: "GET",
    url: `${apicallurl}/hyperlinks/getCloudDetails?adminCloudId=${localStorage.lnKxSourceCldId}&page_nbr=${pageNo}&page_size=50&destAdminCloud=${localStorage.lnKxDstnCldId}`,
    async: true,
    headers: {
    ...
    success: function (data) {
      ...
      if (data.length > 0) {
        data?.map((res, index) => {
          let checkInp;
          if (res?.processStatus === "NOT_PROCESSED") {
            checkInp = `<input type="checkbox" style="margin: 0" id="selectPreScanUser" cloudid="${res?.cloudId}" id="selectPreScanUser"  scanstatus="${res?.processStatus}"/>`;
          } else if (res?.processStatus === "PROCESSED") {
            checkInp = `<input type="checkbox" id="selectLinkExMappings" userid="${res?.userId}" srcadmincldid="${res?.adminCloudId}" dstnadminid="${res?.destAdminCloudId}" destCloudId="${res?.destCloudId}" srcCloudId="${res?.cloudId}" scanstatus="${res?.processStatus}"/>`;
          } else {
        ...
        <div class="placeUserEmail preScanDone"><span title="${res?.emailId}">${
            res?.emailId
          }</span></div>
        <div class="placeUserEmail preScanDone"><span title="${
          res?.destEmailId
        }">${res?.destEmailId}</span></div>
        ...
        <div class="placeScanStatus">
          <span scancid="cloud_${res?.cloudId}" class="span-${
            res?.processStatus === "NOT_PROCESSED"
              ? "Not_Scaned"
              : res?.processStatus
          }">${
            res?.processStatus === "NOT_PROCESSED"
...
function csvPreScanMappingsUpload(csvData) {
  $.ajax({
    type: "POST",
    url: `${apicallurl}/mapping/user/path/csv?sourceCloudId=${localStorage.lnKxSourceCldId}&destCloudId=${localStorage.lnKxDstnCldId}&pageNo=1&pageSize=500`,
    async: true,
    data: csvData,
    headers: {
      "Content-Type": "application/json",
    ...
    success: function (data, textStatus, xhr) {
      ...
      if (data.cfMappingCachesList == undefined) {
        var data = data;
      } else {
...
function linkExCsvAutoDownload() {
  $.ajax({
    type: "GET",
    url: `${apicallurl}/mapping/download/csvcreator/${
...
function getCSVLinkExValidatedPairs(pageNo) {
  if (pageNo) {
    pageNo = pageNo;
  } else {
    pageNo = 1;
  ...
  $.ajax({
    type: "GET",
    url: `${apicallurl}/mapping/user/latest/mappingcache/${
      localStorage.lnKxCSVId
    }?sourceAdminCloudId=${localStorage.lnKxSourceCldId}&destAdminCloudId=${
      localStorage.lnKxDstnCldId
    ...
    success: function (res) {
      ...
      `);
        ...
        <tr matched="false">
      <td>
      <input type="checkbox" id="selMailMappings" disabled>
      </td>
      <td title="sophia@pepperwood.club">
      <div style="display: flex; flex-direction: column">
      <span>${
      ...
      if (res?.length === 50) {
        getCSVLinkExValidatedPairs(Number(pageNo) + 1);
      }
    },
    error: function (err) {
      $("#CFShowLoading").hide();
...
function startFixLinks() {
  ...
  $.ajax({
    type: "POST",
    url: `${apicallurl}/hyperlinks/fixLinks`,
...
function startScanLinks() {
  ...
  $.each($("input[id=selectPreScanUser]:checked"), function () {
    let preScanObj = {
      userId: userId,
      adminCloudId: srcAdminCldId,
      cloudId: $(this).attr("cloudid"),
    };
...
function checkScanStatus() {
  ...
  if ($('[id="selectPreScanUser"]').length > 0) {
    $("#scanLinksCTA").css({ background: "#0062ff", color: "#fff" });
    $("#scanLinksCTA").removeClass("disabled");
  }
}
