    ...
    success: function (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (
          ...
          _data.processStatus === "NOT_STARTED" ||
          _data.processStatus === "NOT_PROCESSED" ||
          _data.processStatus === "SUSPENDED" ||
          _data.processStatus === "RETRYING"
        ) {
          var pauseResumeBTN =
            '<img src="../img/Pause.png" class="disablePause" worspaceId=' +
            _data.id +
            " style='cursor:not-allowed;opacity:0.2;'>";
          var cancelBTN =
            '<i class="lnil lnil-cross-circle" style="cursor: not-allowed;opacity: 0.2;color: #ff4c4c;font-size: 15px;font-weight: 600;" id="remoteActionsMiro"></i>';
        } else if (_data.processStatus === "PAUSE") {
          var pauseResumeBTN =
            '<img src="../img/Resume.png" status="resume" worspaceId=' +
            _data.id +
            ' id="remoteActionsMiro">';
          var cancelBTN =
            '<i class="lnil lnil-cross-circle" style="color: #ff4c4c;font-size: 15px;font-weight: 600;" id="remoteActionsMiro" status="cancel" worspaceId=' +
            _data.id +
            "></i>";
        } else if (
          _data.processStatus === "NOT_PROCESSED" ||
          _data.processStatus === "IN_PROGRESS"
        ) {
          var pauseResumeBTN =
            '<img src="../img/Pause.png" status="pause"  worspaceId=' +
            _data.id +
            ' id="remoteActionsMiro">';
          var cancelBTN =
            '<i class="lnil lnil-cross-circle" style="color: #ff4c4c;font-size: 15px;font-weight: 600;" id="remoteActionsMiro" status="cancel" worspaceId=' +
        ...
        $("#muralReportsTable").append(
          '<tr><td title="' +
            sourceBoardName +
            '">' +
            CFManageCloudAccountsAjaxCall.getMaxChars(sourceBoardName, 25) +
            '</td><td title="' +
            ...
            _data.totalWidgets +
            "</td><td>" +
            _data.processedCount +
            "</td><td>" +
            _data.unsupportedWidgets +
            "</td><td>" +
            _data.conflictCount +
            "</td><td class=" +
            _data.processStatus +
            ...
            "'>" +
            jobStatus +
            "</td><td style='text-align:center' id='pauseResumestatus" +
            _data.id +
            "'>" +
            pauseResumeBTN +
            "</td><td style='text-align:center' id='cancelstatus" +
            _data.id +
            "'>" +
            cancelBTN +
            "</td><td>" +
...
$(document).on("change", "#externalUsersMigration", function () {
     if ($("#collaborationsMove").is(":checked")) {
      if ($(this).is(":checked")) {
	...
	$("#optionsModal").modal();
	$("#optionsModal").find(".modal-title").text("Note")
	$("#optionsModal").find(".modal-body p").text("To enable this option, First you need to enable \"Migrate Collaborations\" option");
...
$(document).on("click", "#externalUsersMigrationText", function () {
	...
	$("#optionsModal").find(".modal-body p").text("Guest users on Mural will be added on Miro");
});

$(document).on("click", "#collaborationsMoveText", function () {
	$("#optionsModal").modal();
	$("#optionsModal").find(".modal-title").text("Migrate Collaborations")
	$("#optionsModal").find(".modal-body p").text("All the collaborators on Mural will be added on Miro");
});
