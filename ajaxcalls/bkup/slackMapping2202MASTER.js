...
function slackUsers(){
	 ...
	 $.ajax({
        type: "POST",
        url: apicallurl + "/mapping/user/clouds/save/permissions?sourcceCloudId="+localStorage.multiUsrSrcCldId+"&destCloudId="+localStorage.multiUsrDstnCldId+"&pageNo=1&pageSize=30",
		async: false,
         headers: {
                "Content-Type": "application/json",
                "Authorization": CFManageCloudAccountsAjaxCall.getAuthDetails(),
...
function pushFromCloudToCloud(fromCloud, ToCloud) {
    arr[fromCloud] = ToCloud;
   Object.assign(cloudDataKey, arr);
...
$('.privateChannels').parent().css('margin-bottom','2%')
...
$('.privateChannels').parent().css('margin-bottom','5%')
$(this).addClass("active");
$(this).siblings('li').removeClass("active");
  ...
  $.ajax({
	...
	$("#CFShowLoading").hide();
      ...
      if (data.length == 0) {
	    ...
	    if(data[i].deltaMigration === false && data[i].fromCloudId !== undefined  && data[i].fromCloudId !== null){
			  if(prStat[processStat] === "Processed" || prStat[processStat] === "Processed With Some Conflicts"){
                                  //var deltaBtn = ''
				  var deltaBtn = '<button id="deltaslack" srcDeltaId='+data[i].sourceDeltaId+' cid='+data[i].fromCloudId.adminCloudId+' tocid='+data[i].toCloudId.adminCloudId+' fromrootid='+data[i].fromRootId+' torootid='+data[i].toRootId+' cname='+data[i].channelName+' ctype='+data[i].channelType+' wname='+data[i].workSpaceName+' destchannelname='+data[i].destChannelName+' dstteamname='+data[i].destTeamName+' style="background: #1220F6 0% 0% no-repeat padding-box;color: #fff;border: 0px;height: 25px;border-radius: 4px;font-weight: 500;">Initiate Delta</button>'
			  }
			  else{
                                  //var deltaBtn = ''
				  var deltaBtn = '<button srcDeltaId='+data[i].sourceDeltaId+' cid='+data[i].fromCloudId.adminCloudId+' tocid='+data[i].toCloudId.adminCloudId+' fromrootid='+data[i].fromRootId+' torootid='+data[i].toRootId+' cname='+data[i].channelName+' ctype='+data[i].channelType+' wname='+data[i].workSpaceName+' destchannelname='+data[i].destChannelName+' dstteamname='+data[i].destTeamName+' style="background: #1220F6 0% 0% no-repeat padding-box;color: #fff;border: 0px;height: 25px;border-radius: 4px;font-weight: 500;opacity:0.4;pointer-events:none;">Initiate Delta</button>' 
			  }
			  }else{
				var deltaBtn = '';	
			  }			  
...
function slackMsgs(cloudId, selected,pgNo,nextId) {
  if (cloudId == undefined) cloudId = localStorage.multiUsrSrcCldId;
  if (selected == undefined) selected = "/usersalldmgroup";
  if(nextId === undefined){
    var uri = apicallurl +"/filefolder/userId/" +localStorage.UserId +"/cloudId/" +cloudId +"?folderId=" +selected+"&page_nbr="+pgNo+"&page_size=50"
  }else{
    uri = apicallurl +"/filefolder/userId/" +localStorage.UserId +"/cloudId/" +cloudId +"?folderId=" +selected +"&nextPreviousId="+nextId+"&page_nbr="+pgNo+"&page_size=50"
  }
  ...
  $.ajax({
    ...
    success: function (data) {
     ...
     if(Number(pgNo) === 1){
       localStorage.removeItem('dmsNXToken');
     }
      ...
      for (var i = 0; i < data.length; i++) {
        if(data[i].nextPageToken === null || data[i].nextPageToken === undefined){
          localStorage.removeItem('dmsNXToken');
        }else{
          localStorage.setItem('dmsNXToken',data[i].nextPageToken);
        }
        var _html =
          '<tr fromRootId="' +
          data[i].id +
  ...
  for (i = 0; i < tr.length; i++) {
   ...
   //console.log(dmCount)
   if(localStorage.dmsNXToken !== undefined && localStorage.dmsNXToken !== "undefined" && localStorage.dmsNXToken !== null && localStorage.dmsNXToken !== "null"){
     var pgno = Number(pgNo);
     pgno++;
     slackMsgs(cloudId, selected,pgno,localStorage.dmsNXToken)
   }
...
function downloadDMSCSVFile(csv_data) {
  ...
  temp_link.download = "DM'S_Report.csv";
...
$("#CFShowLoading").ajaxStart(function(){
   $(this).show();
 });
...
//localStorage.setItem("slackWorkSpaceName",'AllCloudsInfo[i].metadataUrl')
    if(nextId == undefined){
        url = apicallurl + "/filefolder/userId/"+localStorage.UserId+"/cloudId/"+cloudId+"?&pageSize=50&pageNo="+pgno;
    }
    else{
        url = apicallurl + "/filefolder/userId/"+localStorage.UserId+"/cloudId/"+cloudId+"?nextPreviousId="+nextId+"&pageSize=50&pageNo="+pgno;
    }
 ...
 if((data[i].channelType !== 'public') && (key[data[i].cloudId] === data[i].creatorEmailId)){
         ...
         '<td style="width: 16%;"><span title="'+createdBy+'" id="createdByUsers">'+CFManageCloudAccountsAjaxCall.getMaxChars(createdBy, 18)+'</span></td>'+
         '<td style="width: 16%;">'+data[i].channelType+'</td>'+
         '<td style="width: 16%;"><select id="msgType_dropDown" style=""><option>Team</option><option>Sub-Channel</option></select></td>'+
         '<td style="width: 19%;"><span id="destTeamNameSpan" title="'+data[i].objectName+'">'+CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName, 18)+'</span><i class="lnil lnil-pencil" id="destTeamEdit" style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>'+
         '<td style="width: 18%;"><span id="destChannelNameSpan" title="'+data[i].objectName+'">'+CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName, 18)+'</span><i class="lnil lnil-pencil" id="destChannelEdit" style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>'+
         '</tr>';
            $("#channelsTable table tbody").append(_html);
            }  
            if(data[i].channelType == "public"){
            ...
            if(localStorage.getItem('slackNextToken') == "null")
                {
                    ...
                    var pgNo = Number(pgno);
                    pgNo = pgNo+1;
                    getPrivateChannels(cloudId,localStorage.getItem('slackNextToken'),pgNo);
      }
            }
         });
},10);
