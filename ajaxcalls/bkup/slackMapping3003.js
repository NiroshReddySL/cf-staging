...
var uniqueDmKey = {};
var dmkeyval = {}
function makeUniqueDmKey(dmidkey, uniqueId) {
    dmkeyval[dmidkey] = uniqueId;
   Object.assign(uniqueDmKey, dmkeyval);
   localStorage.setItem('uniqueDmKey', JSON.stringify(uniqueDmKey));
}
...
function getdmlog(cloudId, selected, pgNo, nextId) {
    if(pgNo === undefined) pgNo = 1;
  if (cloudId == undefined) cloudId = localStorage.multiUsrSrcCldId;
  if (selected == undefined) selected = "/usersalldmgroup";
  if (nextId === undefined) {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.length > 0) {
        ...
        if (Number(pgNo) === 1) {
	  localStorage.removeItem('uniqueDmKey')
	  ...
	  if(localStorage.uniqueDmKey === undefined){
	  makeUniqueDmKey(localStorage.multiUsrSrcCldId,localStorage.multiUsrDstnCldId)	
	  }
	  var findUniqueDM = JSON.parse(localStorage.uniqueDmKey)
	  if(findUniqueDM[data[i].id] === undefined){
          makeUniqueDmKey(data[i].id, data[i].id)
	  var _html =
            '<tr fromRootId="' +
            data[i].id +
        ...
        //console.log(dmCount)
        if (
          localStorage.dmsNXToken !== undefined &&
          localStorage.dmsNXToken !== "undefined" &&
          localStorage.dmsNXToken !== null &&
          localStorage.dmsNXToken !== "null"
        ) {
          var pgno = Number(pgNo);
          pgno++;
          getdmlog(cloudId, selected, pgno, localStorage.dmsNXToken);
        } else {
...
checkDmSelection = [];
	var obj =  {
		...
		"channelDate": $(this).attr('srcdelcldid'),
        	"fromRootId": $(this).attr('fromrootid'),
        	"toRootId": $(this).attr('torootid'),
		"channelName":$(this).attr('cname'),
		...
		"channelType":$(this).attr('ctype'),
		"specialCharacter" : "-",
		"workSpaceId":$(this).attr('mainWSId'),
		"workSpaceName":$(this).attr('wname'),
		};  
