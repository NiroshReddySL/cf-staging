...
function getSlackReports(pgNo) {
  ...
  $.ajax({
	...
	fromCloudAdminId = data[i].fromCloudId.adminCloudId;
	}
	    ...
	    if((data[i].teamClosed === false || data[i].teamClosed === undefined || data[i].teamClosed === null) && data[i].fromCloudId !== undefined  && data[i].fromCloudId !== null){
            ...
            if(data[i].fromCloudId !== undefined  && data[i].fromCloudId !== null){
			 if(prStat[processStat] === "Processed" || prStat[processStat] === "Processed With Some Conflicts"){
                                  //var deltaBtn = ''
				  var deltaBtn = '<button id="dmDeltaslack" channelName="'+data[i].channelName+'" chnalDate="'+data[i].sourceDeltaId+'" srcDelegateCloudId="'+data[i].sourceDelegateCloudId+'" teamsId="'+data[i].teamsId+'" dstnDelegateCloudId="'+data[i].destDelegateCloudId+'" dstnFolderId="'+data[i].destFolderIds+'" dstnWebUrl="'+data[i].destWebUrls+'" srcDelCldId="'+data[i].sourceDelegateCloudId+'" dstnDelCldId="'+data[i].destDelegateCloudId+'" mainWSId="'+data[i].mainWSId+'" emailPairs="'+data[i].emailPairs+'" srcDeltaId="'+data[i].sourceDeltaId+'"fromrootid="'+data[i].fromRootId+'" torootid="'+data[i].toRootId+'" fromcid="'+data[i].fromCloudId.adminCloudId+'" tocid="'+data[i].toCloudId.adminCloudId+'" srcDeltaId="'+data[i].sourceDeltaId+'" ctype="'+data[i].channelType+'" wname="'+data[i].workSpaceName+'" style="background: #1220F6 0% 0% no-repeat padding-box;color: #fff;border: 0px;height: 25px;border-radius: 4px;font-weight: 500;">Initiate Delta</button>'
			  }
			  else{
                                  //var deltaBtn = ''
            ...
            var html2 =
              ...
              "</td><td>" +
              prStat[processStat] +
              '</td><td>'+deltaBtn +'</td><td style="text-align:center;"><i class="lnil lnil-download" id="slackDMReport" style="color:#0062ff;font-size: 16px;font-weight: bold;"></i></td></tr>';
...
for(let i=0;i<=100;i++){
 if(i%2 === 0){
 $("#fetchPublicChannelCountPreMig").html(publicCount++)
 $(".tableSlackChannelsPreMigration table tbody").append("<tr><td><i class='fa fa-hashtag' style='color:#000;font-size: 10px !important;font-weight:100 !important;'></i>&nbsp;Testing</td><td>Tester</td><td>5</td><td>8000</td></tr>")
 }else{
 $("#fetchPrivateChannelCountPreMig").html(privateCount++)
 $(".tableSlackChannelsPreMigration table tbody").append("<tr><td><i class='fa fa-lock' style='color:#000;font-size: 10px !important;font-weight:100 !important;'></i>&nbsp;Testing</td><td>Tester</td><td>5</td><td>8000</td></tr>")
 }
 $("#fetchChannelCountPreMig").html(i)
}*/
})
	...
	if(data.length > 0){
		for(let i = 0; i < data.length ; i++){
		  ...
		  let fileCount = _data.filesCount ? _data.filesCount :"0";	
			...
			$(".tableSlackChannelsPreMigration table tbody").append("<tr id='publicPreMigration'><td title="+_data.channelName+"><i class='fa fa-hashtag' style='color:#000;font-size: 10px !important;font-weight:100 !important;'></i>&nbsp;"+CFManageCloudAccountsAjaxCall.getMaxChars(_data.channelName,15)+"</td><td>"+_data.totalMessageInChannel+"</td><td>"+_data.totalUsersInPublicChannel+"</td><td>"+fileCount+"</td><td>"+CFManageCloudAccountsAjaxCall.getObjectSize(_data.totalDataSize)+"</td><td class="+_data.processStatus+">"+processStatus[_data.processStatus]+"</td></tr>"); 
			}else{
			$(".tableSlackChannelsPreMigration table tbody").append("<tr id='privatePreMigration'><td title="+_data.channelName+"><i class='fa fa-lock' style='color:#000;font-size: 10px !important;font-weight:100 !important;'></i>&nbsp;"+CFManageCloudAccountsAjaxCall.getMaxChars(_data.channelName,15)+"</td><td>"+_data.totalMessageInChannel+"</td><td>"+_data.totalUsersInPrivateChannel+"</td><td>"+fileCount+"</td><td>"+CFManageCloudAccountsAjaxCall.getObjectSize(_data.totalDataSize)+"</td><td class="+_data.processStatus+">"+processStatus[_data.processStatus]+"</td></tr>");
			}
		}
...
function updateTableStyle() {
        $(".tableSlackChannelsPreMigration table tbody tr:visible:odd").addClass("odd");
        $(".tableSlackChannelsPreMigration table tbody tr:visible:even").addClass("even");
}
