...
function slackMsgsCheck(cloudId, selected) {
  ...
  $.ajax({
	...
	list.forEach((e) => { 
 	 slackMsgs(e,'/usersalldmgroup',1);
	 console.log(e,'/usersalldmgroup',1);
 	})
...
//          $("#directMsgsTable table tbody").html("");
  ...
  for (i = 0; i < tr.length; i++) {
   ...
   //console.log(dmCount)
   if(data.length === 50 && (localStorage.dmsNXToken !== undefined || localStorage.dmsNXToken !== null || localStorage.dmsNXToken !== "null")){
     var pgno = Number(pgNo);
     pgno++;
...
function getPrivateChannels(cloudId,nextId){
$("#CFShowLoading").show();  
setTimeout(function () {  
if(cloudId == undefined){
        cloudId = localStorage.multiUsrSrcCldId;
            ...
            if(localStorage.getItem('slackNextToken') == "null")
                {
                    ...
                    getPrivateChannels(cloudId,localStorage.getItem('slackNextToken'));
