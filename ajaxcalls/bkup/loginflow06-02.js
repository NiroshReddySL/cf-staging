...
var loginAjaxCall = {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data != null) {
                    ...
                    localStorage.setItem("UserAuthDetails", loginAjaxCall.BasicAuth(data.id, password));
            ...
            statusCode: {
                200: function () {
                    ...
                    if (userDetails.enabled == true) {
                        localStorage.setItem("UserAuthDetails", loginAjaxCall.BasicAuth(userDetails.id, password));
                        localStorage.setItem("UserId", userDetails.id);
                        loginAjaxCall.refreshcloud('all',loginAjaxCall.BasicAuth(userDetails.id, password),userDetails.id);
                     ...
                     //   zohoCrm.upgradeSubscribtionRecord(username, 'loggedIn');
                        ...
                        $('#login-submit').html('Login<i class="lni lni-arrow-right" style="float:right;margin-top:2%;"></i>');
    ...
    signup: function (data) {
		...
		if(data.email.split('@')[1] == "sharklasers.com" || data.email == "kampungandroid@qmail.id" || data.email == "cesaroliveira1905@gmail.com"){
            return "failed";
        }
...
ForgotpasswordAjaxCall = {
    searchUser : function(email,checksum,password) {
        ...
        $.ajax({
            type: "GET",
            url: apicallurl + "/users/validate?searchUser=" + email + "",
            async: false,
            dataType: "json",
            headers: {
                "Content-Type": "application/json",
                ...
                if (data != "") {
                    if (checksum == "sendPwdLink") {
                        if (data[0].enabled === true) {
                            ForgotpasswordAjaxCall.forgotPassword(email);
                        }else {
                            //wrapper = $("#login-manager  .login-wrapper");
                            wrapper.addClass("wobble");
                            $("#login-manager .statusMesg").html('<p>Your account is not yet confirmed. We will resend the confirmation link to your registered email.Please find this email and click on the confirmation link.</p>');
                            $("#login-manager i#resend-link").text("Resend confirmation link");
                            return wrapper.bind("webkitAnimationEnd animationEnd mozAnimationEnd", function () {
                                wrapper.off("webkitAnimationEnd");
                                return wrapper.removeClass("wobble");
                        ...
                        if (data[0].enabled === false) {
                            ForgotpasswordAjaxCall.resendEmail(email)
                        }else {
                            //wrapper = $("#login-manager  .login-wrapper");
                            wrapper.addClass("wobble");
                            $("#login-manager .statusMesg").text(' The email address you entered is alredy confirmed.');
                            return wrapper.bind("webkitAnimationEnd animationEnd mozAnimationEnd", function () {
                                wrapper.off("webkitAnimationEnd");
                                return wrapper.removeClass("wobble");
                            });
                        ...
                        if(password == ""){
                            ...
                            $('#login-submit').html('Logging in..<i class="lni lni-arrow-right" style="float:right;margin-top:2%;"></i>');
                            localStorage.setItem('storageVal', true);
                            if (data[0].passwordPolicy == "MD5" || data[0].passwordPolicy == null) {
                                password = CryptoJS.MD5(password);
                            }
                            loginAjaxCall.ajaxcall(email, password, serverDate, data[0].expiresIn);
                        ...
                        var loginDetails = {
                            ...
                            "expires":data[0].expiiresIn
                        };
                        $('#dummyFrame').attr('src',"https://"+$('#login').find('input').val()+"."+host+".cloudfuze.com/pages/cloudput.html?login="+Base64.encode(loginDetails));
                    }
                }
                else {
                    if (checksum == "loginPage") {
                        ...
                        $("#login-manager .statusMesg").text('*The Email address you entered is not registered with us.');
    ...
    forgotPassword : function(email){
        $.ajax({
            type: "POST",
            url:apicallurl+"/auth/user/forgot?userName="+email+"&domainUrl="+domainUrl+"/pwdNew",
            async: false,
            dataType: "json",
            headers:{
                "Content-Type":"application/json",
...
$(document).on("click", "#CFResendConfirmLink", function(){
    ...
    if(email.length > 50 ){
        $("#CFforgotpassword  .statusMesg").text('The email address you entered is not associated with CloudFuze9990.');
        $("#FEmail").focus();
        return false;
    }
    if(!emailReg.test(email)){
        $("#CFforgotpassword .statusMesg").text('Please enter a valid E-Mail address.');
        $("#FEmail").focus();
        return false;
    }
...
$(document).on("click", "#CFsendpwdresetlink", function(){
    ...
    {
        $("#CFforgotpassword  .statusMesg").text('The email address you entered is not associated with CloudFuze.');
        $("#FEmail").focus();
        return false;
    }
    if(!emailReg.test(email))
...
$('#CFFcancel').click(function() {
    ...
    if ($("#CFloginform a[tid='CFLoginLinkText']").attr('id') == "ResendConfirmationLink") {
        $(this).attr("id", "CFsendpwdresetlink");
    } else if ($("#CFloginform a[tid='CFLoginLinkText']").attr('id') == "CFsendpwdresetlink") {
