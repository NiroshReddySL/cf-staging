...
var CFManageCloudAccountsAjaxCall = {
	...
	'<div id="mngCloudRemove" style="width: 6%;padding: 2.7% 2%;float:left;text-align: right;"><i cldId="' + AllCloudsInfo[i].id + '" id="delCld" cloudName="'+ AllCloudsInfo[i].cloudName+'"  data-action="delete" data-id="' + AllCloudsInfo[i].id + '"  data-toggle="modal" data-target="#CFCLdeleteModal" class="lnil lnil-trash-can-alt" style="font-size: 17px;cursor:pointer;"></i></div>'+
	...
	'<div class="appendcloudDetails" style="width: 96%;margin: 2%;border-radius: 5px;display:none"></div></div>';
			...
			var title = AllCloudsInfo[i].emailId;
                var email = CFManageCloudAccountsAjaxCall.getMaxChars(AllCloudsInfo[i].emailId, 30);
            }
            else {
            ...
            if(AllCloudsInfo[i].cloudName === "DROPBOX_BUSINESS") {
			  ...
			  if(AllCloudsInfo[i].cloudName === "ONEDRIVE_BUSINESS_ADMIN" || AllCloudsInfo[i].cloudName === "BOX_BUSINESS" ||AllCloudsInfo[i].cloudName === "SHAREPOINT_ONLINE_BUSINESS" || AllCloudsInfo[i].cloudName === "G_SUITE" || AllCloudsInfo[i].cloudName === "DROPBOX_BUSINESS") {
                       $('#manageclouddiv #' + AllCloudsInfo[i].id ).parent().find('#AuthorizeCld').css('display','none').removeClass('retry');
            ...
            if(CldId === AllCloudsInfo[i].id){
				...
				appendHtml = multi_html.replace('dispName',AllCloudsInfo[i].userDisplayName).replace('displayName',CFManageCloudAccountsAjaxCall.getMaxChars(AllCloudsInfo[i].userDisplayName, 30)).replace('totalUsers',total).replace('nameofDomain',AllCloudsInfo[i].domainName).replace('domainName',CFManageCloudAccountsAjaxCall.getMaxChars(AllCloudsInfo[i].domainName, 30)).replace('addedUsers',added).replace('notAddedUsers',notAdded);
                    ...
                    if (AllCloudsInfo[i].loadLock === false){
                        var syncIcon = '<i class="lnil lnil-reload refresh-Cloud" id="refreshIcon" style="font-weight:600;"></i>',
...
//				$('.LVHcheckBox input').prop("disabled",false);
    ...
    deleteCloud: function (a,cloudName) {
        ...
        if(cloudName === "OUTLOOK" || cloudName === "GMAIL"){
            var apiUrl = apicallurl + "/users/" + CFManageCloudAccountsAjaxCall.getUserId() + "/cloud/delete/" + a + "?token="+localStorage.emailToken;
        }else{
...
$('#CFCLDelete').on('click',function() {
    ...
    var _r = CFManageCloudAccountsAjaxCall.deleteCloud(_id,$(this).attr("cloudname"));
