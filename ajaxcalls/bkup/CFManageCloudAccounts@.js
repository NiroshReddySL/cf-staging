...
var CFManageCloudAccountsAjaxCall = {
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        if (jqXHR.status === 401) {
          ...
          if (PageName == "InnerFolders" && previousPage == "Share with Me") {
            ...
            var notification = {};
            if (
              localStorage.getItem("UserAuthDetails") == undefined ||
    ...
    $.ajax({
      ...
      success: function (AllClouds) {
        ...
        $.each(AllClouds, function (i, cloudObj) {
          ...
          if (
            cloudObj.loadLock &&
            /fileManager.html/.test(window.location.href)
          ) {
      ...
      complete: function (xhr, statusText) {
        if (xhr.status > 300) {
          ...
          if (localStorage.getItem("storageVal") == null) {
            alertSuccess("You have been logged out");
          } else alertError("Operation Failed");
        }
      },
    });
  },
        ...
        $.each(AllClouds, function (i, cloudObj) {
          ...
          for (j = 0; j < AllCloudsInfo.length; j++) {
            if (AllCloudsInfo[j].id === cldId) {
              if (
                AllCloudsInfo[i].cloudName === "GOOGLE_SHARED_DRIVES" ||
                AllCloudsInfo[i].cloudName === "EGNYTE_ADMIN" ||
                AllCloudsInfo[i].cloudName === "G_SUITE" ||
                AllCloudsInfo[i].cloudName === "DROPBOX_BUSINESS" ||
                AllCloudsInfo[i].cloudName === "ONEDRIVE_BUSINESS_ADMIN" ||
                AllCloudsInfo[i].cloudName ===
                  "ONEDRIVE_BUSINESS_ADMIN_HYBRID" ||
                AllCloudsInfo[i].cloudName === "BOX_BUSINESS" ||
                AllCloudsInfo[i].cloudName === "SHAREPOINT_ONLINE_BUSINESS"
              ) {
                if (AllCloudsInfo[j].totolClouds != null) {
                  CFManageCloudAccountsAjaxCall.progressOfUsers(
                    cldId,
                    AllCloudsInfo[j].cloudName,
                  );
                } else {
                  setTimeout(function () {
                    CFManageCloudAccountsAjaxCall.getClouds(
                      AllCloudsInfo[i].id,
                    );
                  }, 60000);
                }
  ...
  getAllCloudTitles: function () {
    ...
    $.each(uniqueCloudNames, function (i, CName) {
      ...
      if (MultiUserClouds.indexOf(CName) == -1) {
        $("#CloudDriveList").append(
          '<li style="padding-left:0;border-bottom: 1px solid #024A5B;"><i class="Driveicon" id="' +
            CName +
            '"></i><a href="#" class="cltrigger">' +
            CFCName +
            '</a><ul class="clsubmenu" style="display: none;" id="' +
            CName +
            '"></ul></li>',
        );
      }

      CFManageCloudAccountsAjaxCall.getAllCloudChilds(
        CloudName,
        CName,
        CFCName,
        CloudId,
  ...
  getAllCloudChilds: function (CloudName, CName, CFCName, CloudId, CLParent) {
    for (var i = 0; i < CloudName.length; i++) {
      if (CName == CloudName[i] && MultiUserClouds.indexOf(CName) == -1) {
        var _a =
          AllCloudsInfo[i].cloudStatus == "INACTIVE"
            ? "cf-warning ErrorRefresh"
            : "lnil lnil-reload";
        $("#CloudDriveList")
          .find(".clsubmenu[id='" + CName + "']")
          .append(
            '<li style="padding-left:10px;height:23px;"' +
              ' id="' +
              CloudId[i] +
              '" class="getCloudFiles" pid="' +
              CLParent[i] +
              '">' +
              '<p class="cloudDispName">' +
              CLDisplayName[i] +
              "</p>" +
              '<i class="' +
              _a +
  ...
  getAllCloudTitlesSettings: function (CloudName, CloudId) {
    // CFManageCloudAccountsAjaxCall.getAllCloudDetails(CloudId);
    ...
    for (var i = 0; i < CloudName.length; i++) {
      if (AllCloudsInfo[i].userType === "ADMIN") {
        if (
          ...
          AllCloudsInfo[i].cloudName === "DROPBOX_BUSINESS" ||
          AllCloudsInfo[i].cloudName === "ONEDRIVE_BUSINESS_ADMIN" ||
          AllCloudsInfo[i].cloudName === "ONEDRIVE_BUSINESS_ADMIN_HYBRID" ||
          AllCloudsInfo[i].cloudName === "BOX_BUSINESS" ||
          AllCloudsInfo[i].cloudName === "SHAREPOINT_ONLINE_BUSINESS"
        ) {
          if (
            AllCloudsInfo[i].totolClouds === null ||
            AllCloudsInfo[i].totolClouds === 0
          ) {
            var html =
              '<div class="cloudsinfo" id="cloudInfo" style="background: #FCFCFC 0% 0% no-repeat padding-box;border: 1px solid #E5E5E5;border-radius: 5px;height:80px;width:100%;margin-bottom: 2%;">' +
              '<div id="mngCloudIcon" style="width: 7%;padding: 1.5% 1%;float:left;text-align:center;padding-top: 1%;"><div class="bg-cfIcon"></div></div>' +
              '<div id="mngCloudName" style="width: 22%;padding:2.7% 1%;float:left;color:#1F2129;font-weight:500;"><span>' +
              CLName[CloudName[i]] +
              "</span></div>" +
              '<div id="mngCloudData" style="width: 20%;padding: 2% 1%;float:left;margin-left:-85px;">' +
              '<div style="width: 100%;" id="incrementProgress"><p id="usersDisplay" style="color: #707070;font-size: 12px;width: 135px;float: left;margin-bottom: 2%;">0 out of 0 Users </p>' +
              '<div class="progress" style="height: 6px;width: 170px;border-radius: 15px;background: #d9534f;"><div class="progress-bar progress-bar-success progressGreen" role="progressbar" style="width:64%;background: #4CAF50;height: 100%;border-radius: 25px;"></div></div></div><div id="failUserAppend' +
              AllCloudsInfo[i].id +
              '"></div></div>' +
              ...
              '<div id="mngCloudType" style="width: 6%;padding: 2.7% 1%;float:left;margin-left:-75px;"><span>userType</span></div>' +
              '<div id="quickScanDiv' +
              AllCloudsInfo[i].cloudName +
              '" style="width: 17.5%;padding: 2.7% 1%;float:left;margin-left: 15px;"><button type="button" id="showSummery" cuid="' +
              AllCloudsInfo[i].userId +
              '" uid="' +
              AllCloudsInfo[i].id +
              '" style="display: inline-block;background-color: #fcfcfc;color: #1220f6;border: none;border-radius: 3px;z-index: 999999;width:91px;outline:none;margin-left:50px" onclick="showPremigDivAlna(this)"><i class="lnil lnil-cloud-search" title="Quick Scan"" style="font-size:25px;color:#1220F6;font-weight:bolder;"></i></a></button></div>' +
              '<div style="width: 8%;padding: 2.7% 1%;float:left;margin-left: 0px;display:none;"><a href="https://staging.cloudfuze.com/pages/teammigration.html?ver=1028"><button type="button" id="showSummery" style="display: inline-block;background-color: #1220f6;color: #fff;border: none;border-radius: 3px;z-index: 999999;width: 64px;">Migrate</button></a></div>' +
              '<div id="mngCloudRemove" style="margin-left:-4px;width: 2%;padding: 2.7% 2%;float:left;text-align: right;"><i cldId="' +
              AllCloudsInfo[i].id +
              '" id="delCld"  data-action="delete" data-id="' +
              AllCloudsInfo[i].id +
              '"  data-toggle="modal" data-target="#CFCLdeleteModal" class="lnil lnil-trash-can-alt" style="font-size: 17px;cursor:pointer;"></i></div>' +
              '<div id="' +
              AllCloudsInfo[i].id +
              '" name="' +
              AllCloudsInfo[i].cloudName +
              '"class="getCloudsInfo" style="width: 4%;padding: 2.7% 1%;float:left;text-align:center;cursor:pointer;display:none;"><i class="lnil lnil-chevron-down-circle" style="color: #0062FF;font-size: 20px;font-weight:600;"></i></div><hr id="manageInfoHr' +
              AllCloudsInfo[i].id +
              '" style="margin-top: 6%;width: 0%;display:none">' +
              '<div class="appendcloudDetails" style="height:auto;width: 96%;margin: 2%;border-radius: 5px;display:none"></div></div>';
            ...
            if (
              AllCloudsInfo[i].notProvisioned > 0 ||
              AllCloudsInfo[i].provisionedClouds > 0 ||
              AllCloudsInfo[i].cloudName === "DROPBOX_BUSINESS"
            ) {
              if (
                AllCloudsInfo[i].notProvisioned !== null ||
                AllCloudsInfo[i].notProvisioned !== 0
              ) {
                var prvsnLen = AllCloudsInfo[i].provisionedClouds;
                var prvsnWidth = Math.round((prvsnLen / totalCount) * 100);
                if (
                  AllCloudsInfo[i].notProvisioned !==
                  AllCloudsInfo[i].totolClouds
                ) {
                  if (prvsnWidth === 0) {
                    prvsnWidth = 2;
                  }
                ...
                var html =
                  ...
                  "</span></div>" +
                  '<div id="mngCloudData" style="width: 20%;padding: 2% 1%;float:left;margin-left:-85px;">' +
                  '<div style="width: 100%;" id="incrementProgress"><p id="usersDisplay" style="color: #707070;font-size: 12px;width: 135px;float: left;margin-bottom: 2%;">' +
                  prvsnLen +
                  " out of " +
                  totalCount +
                  " Users </p>" +
                  '<div class="progress" style="height: 6px;width: 170px;border-radius: 15px;background: #d9534f;"><div class="progress-bar progress-bar-success progressGreen" role="progressbar" style="width:' +
                  prvsnWidth +
                  '%;background: #4CAF50;height: 100%;border-radius: 25px;"></div></div></div><div id="failUserAppend' +
                  AllCloudsInfo[i].id +
                  '"></div></div>' +
                  '<div id="mngCloudSync" style="width: 3%;padding: 2.7% 3.8% 0% 0%;float:left;"><i class="lnir lnir-spinner-arrow" id="reAuthorizeCld" title="Reauthenticate users" style="font-size: 15px;font-weight:600;"></i><img src="../img/retry.gif" id="AuthorizeCld" title="Authenticating users"  style="margin-left: 61%;height: 20px;width: 20px;cursor: pointer;margin-top: 12%;display: none"/></div>' +
                  '<div id="mngCloudEmail" style="width: 28%;padding: 2.7% 1%;float:left;"><span title="titleEmail">emailId</span></div>' +
                  ...
                  '" uid="' +
                  AllCloudsInfo[i].id +
                  '" style="display: inline-block;background-color: #fcfcfc;color: #1220f6;border: none;border-radius: 3px;z-index: 999999;width:91px;outline:none;margin-left:50px;" onclick="showPremigDivAlna(this)"><a href="#"><i class="lnil lnil-cloud-search" title="Quick Scan"" style="font-size:25px;color:#1220F6;font-weight:bolder;"></i></a></button></div>' +
                  '<div style="width: 8%;padding: 2.7% 1%;float:left;margin-left: 0px;display:none;"><a href="https://staging.cloudfuze.com/pages/teammigration.html?ver=1028"><button type="button" id="showSummery" style="display: inline-block;background-color: #1220f6;color: #fff;border: none;border-radius: 3px;z-index: 999999;width: 64px;">Migrate</button></a></div>' +
                  '<div id="mngCloudRemove" style="margin-left:-4px;width: 2%;padding: 2.7% 2%;float:left;text-align: right;"><i cldId="' +
                  AllCloudsInfo[i].id +
                  '" id="delCld"  data-action="delete" data-id="' +
                  ...
                  '" name="' +
                  AllCloudsInfo[i].cloudName +
                  '"class="getCloudsInfo" style="width: 4%;padding: 2.7% 1%;float:left;text-align:center;cursor:pointer;display:none;"><i class="lnil lnil-chevron-down-circle" style="color: #0062FF;font-size: 20px;font-weight:600;cursor:pointer;"></i></div><hr id="manageInfoHr' +
                  AllCloudsInfo[i].id +
                  '" style="margin-top: 6%;width: 0%;display:none">' +
                  '<div class="appendcloudDetails" style="height:auto;width: 96%;margin: 2%;border-radius: 5px;display:none"></div></div>';
              } else {
                var html =
                  ...
                  "</span></div>" +
                  '<div id="mngCloudData" style="width: 20%;padding: 2% 1%;float:left;margin-left:-85px;">' +
                  '<div style="width: 100%;" id="incrementProgress"><p id="usersDisplay" style="color: #707070;font-size: 12px;width: 135px;float: left;margin-bottom: 2% 1%;">' +
                  totalCount +
                  " out of " +
                  totalCount +
                  " Users </p>" +
                  '<div class="progress" style="height: 6px;width: 170px;border-radius: 15px;background: #d9534f;"><div class="progress-bar progress-bar-success progressGreen" role="progressbar" style="width:100%;background: #4CAF50;height: 100%;border-radius: 25px;"></div></div></div><div id="failUserAppend' +
                  AllCloudsInfo[i].id +
                  '"></div></div>' +
                  '<div id="mngCloudSync" style="width: 3%;padding: 2.7% 3.8% 0% 0%;float:left;"><i class="lnir lnir-spinner-arrow" id="reAuthorizeCld" title="Reauthenticate users" style="font-size: 15px;font-weight:600;"></i><img src="../img/retry.gif" id="AuthorizeCld" title="Authenticating users"  style="margin-left: 61%;height: 20px;width: 20px;cursor: pointer;margin-top: 12%;display: none"/></div>' +
                  '<div id="mngCloudEmail" style="width: 28%;padding: 2.7% 1%;float:left;"><span title="titleEmail">emailId</span></div>' +
              ...
              var _progress = CFManageCloudAccountsAjaxCall.progress(
                cldId,
                cldname,
              );
              ...
              var html =
                '<div class="cloudsinfo" id="cloudInfo" style="background: #FCFCFC 0% 0% no-repeat padding-box;border: 1px solid #E5E5E5;border-radius: 5px;height:80px;width:100%;margin-bottom: 2%;">' +
                '<div id="mngCloudIcon" style="width: 7%;padding: 1.5% 1%;float:left;text-align:center;padding-top: 1%;"><div class="bg-cfIcon"></div></div>' +
                '<div id="mngCloudName" style="width: 20%;padding:2.7% 1%;float:left;color:#1F2129;font-weight:500;"><span>' +
                CLName[CloudName[i]] +
                "</span></div>" +
                '<div id="mngCloudData" style="width: 22%;padding: 2%;float:left;margin-left:-85px;">' +
                '<div style="width: 100%;" id="incrementProgress"><p id="usersDisplay" style="color: #707070;font-size: 12px;width: 135px;float: left;margin-bottom: 2% 1%;">' +
                _progress +
                " out of " +
                totalCount +
          ...
          if (AllCloudsInfo[i].cloudName === "NETWORK_FILESHARES") {
            var html =
              '<div class="cloudsinfo" id="cloudInfo" style="background: #FCFCFC 0% 0% no-repeat padding-box;border: 1px solid #E5E5E5;border-radius: 5px;height:80px;width:100%;margin-bottom: 2%;">' +
              '<div id="mngCloudIcon" style="width: 7%;padding: 1.5% 1%;float:left;text-align:center;padding-top: 1%;"><div class="bg-cfIcon"></div></div>' +
              '<div id="mngCloudName" style="width: 38.5%;padding:2.7% 1%;float:left;color:#1F2129;font-weight:500;"><span>' +
              CLName[CloudName[i]] +
              "</span></div>" +
              '<div id="mngCloudEmail" style="width: 28%;padding: 2.7% 1%;float:left;"><span title="titleEmail">emailId</span></div>' +
              '<div id="mngCloudType" style="width: 6%;padding: 2.7% 1%;float:left;margin-left:-75px;"><span>userType</span></div>' +
              '<div id="quickScanDiv' +
              AllCloudsInfo[i].cloudName +
              '" style="width: 17.5%;padding: 2.7% 1%;float:left;margin-left: 15px;"><button type="button" id="showSummery" uid="' +
              AllCloudsInfo[i].id +
              '"style="display: inline-block;background-color: #fcfcfc;color: #1220f6;border: none;border-radius: 3px;z-index: 999999;width:91px;outline:none;margin-left:50px;" onclick="showPremigDivAlna(this)"><a href="#"><i class="lnil lnil-cloud-search" title="Quick Scan"" style="font-size:25px;color:#1220F6;font-weight:bolder;"></i></a></button> </div>' +
              '<div style="width: 8%;padding: 2.7% 1%;float:left;margin-left: 0px;display:none;"><a href="https://staging.cloudfuze.com/pages/teammigration.html?ver=1028"><button type="button" id="showSummery" style="display: inline-block;background-color: #1220f6;color: #fff;border: none;border-radius: 3px;z-index: 999999;width: 64px;">Migrate</button></a></div>' +
              '<div id="mngCloudRemove" style="margin-left:-4px;width: 2%;padding: 2.7% 2%;float:left;text-align: right;"><i cldId="' +
            ...
            var html =
              '<div class="cloudsinfo" id="cloudInfo" style="background: #FCFCFC 0% 0% no-repeat padding-box;border: 1px solid #E5E5E5;border-radius: 5px;height:80px;width:100%;margin-bottom: 2%;">' +
              '<div id="mngCloudIcon" style="width: 7%;padding: 1.5% 1%;float:left;text-align:center;padding-top: 1%;"><div class="bg-cfIcon"></div></div>' +
              '<div id="mngCloudName" style="width: 46%;padding:2.7% 1%;float:left;color:#1F2129;font-weight:500;"><span>' +
              CLName[CloudName[i]] +
              "</span></div>" +
              '<div id="mngCloudEmail" style="width: 28%;padding: 2.7% 1%;float:left;"><span title="titleEmail">emailId</span></div>' +
              '<div id="mngCloudType" style="width: 6%;padding: 2.7% 1%;float:left;margin-left:-75px;"><span>userType</span></div>' +
        ...
        if (AllCloudsInfo[i].cloudName === "NETWORK_FILESHARES") {
          var html =
            ...
            CLName[CloudName[i]] +
            "</span></div>" +
            '<div id="mngCloudEmail" style="width: 28%;padding: 2.7% 1%;float:left;margin-left:-89px;"><span title="titleEmail">emailId</span></div>' +
            '<div id="mngCloudType" style="width: 6%;padding: 2.7% 1%;float:left;margin-left:-75px;"><span>userType</span></div>' +
            '<div id="mngCloudRemove" style="margin-left:217px;width: 2%;padding: 2.7% 2%;float:left;text-align: right;"><i cldId="' +
            AllCloudsInfo[i].id +
            '" id="delCld"  data-action="delete" data-id="' +
            AllCloudsInfo[i].id +
            '" data-toggle="modal" data-target="#CFCLdeleteModal" class="lnil lnil-trash-can-alt" style="font-size: 17px;cursor:pointer;"></i></div>' +
      ...
      if (
        AllCloudsInfo[i].cloudName === "DROP_BOX" ||
        AllCloudsInfo[i].cloudName === "AMAZON" ||
        AllCloudsInfo[i].cloudName === "ORANGE" ||
        AllCloudsInfo[i].cloudName === "FTP"
      ) {
        var title = AllCloudsInfo[i].userDisplayName;
        var email = CFManageCloudAccountsAjaxCall.getMaxChars(
          AllCloudsInfo[i].userDisplayName,
          30,
        );
      } else if (AllCloudsInfo[i].cloudName === "NETWORK_FILESHARES") {
        var title = AllCloudsInfo[i].cloudUserId.split("|")[2];
        var email = CFManageCloudAccountsAjaxCall.getMaxChars(
          AllCloudsInfo[i].cloudUserId.split("|")[2],
          30,
        );
      } else {
        var title = AllCloudsInfo[i].cloudUserId.split("|")[1];
        var email = CFManageCloudAccountsAjaxCall.getMaxChars(
          AllCloudsInfo[i].cloudUserId.split("|")[1],
          30,
        );
      }
      if (AllCloudsInfo[i].userType === "ADMIN") {
      ...
      var appendhtml = html
        ...
        .replace("titleEmail", title)
        .replace("cfIcon", AllCloudsInfo[i].cloudName);
      ...
      if (
        ...
        // console.log("[id=quickScanDiv" + AllCloudsInfo[i].cloudName + "]");
        $("[id=quickScanDiv" + AllCloudsInfo[i].cloudName + "]").css(
          "visibility",
          "visible",
        );
      } else {
      ...
      if (
        AllCloudsInfo[i].cloudAddingStatus === false ||
        notPrvsn === null ||
        notPrvsn === 0
      ) {
        if (AllCloudsInfo[i].cloudAddingStatus === false) {
          $("#manageclouddiv #" + AllCloudsInfo[i].id)
            .parent()
            .find("#reAuthorizeCld")
            .css("display", "none");
          $("#manageclouddiv #" + AllCloudsInfo[i].id)
      ...
      CFManageCloudAccountsAjaxCall.getAllCloudDetails(CloudId[i]);
    }
  },
  progress: function (CldId, cldName) {
    ...
    $.ajax({
      ...
      url:
        apicallurl +
        "/users/clouds/multiUser/count?adminCloudId=" +
        CldId +
        "&cloudName=" +
        cldName,
      async: false,
        ...
        for (var i = 0; i < AllCloudsInfo.length; i++) {
          if (AllCloudsInfo[i].id === CldId) {
            if (usersCount === AllCloudsInfo[i].totolClouds) {
              var parent = $("#manageclouddiv #" + CldId)
                .children("." + CldId)
                .children("#incrementProgress");
              var wid = Math.round(
                (usersCount / AllCloudsInfo[i].totolClouds) * 100,
              );
              parent
                .children("div")
                .find(".progressGreen")
              ...
              parent
                .find("#usersDisplay")
                .text(usersCount + " out of " + usersCount + " Users");
              $("#manageclouddiv #" + AllCloudsInfo[i].id)
                .find("#AuthorizeCld")
                .css("display", "none")
                .removeClass("retry");
              $("#manageclouddiv #" + AllCloudsInfo[i].id)
                .find("#reAuthorizeCld")
                .css("display", "");
              var parent = $("#manageclouddiv #" + AllCloudsInfo[i].id).find(
                "#reAuthorizeCld",
              );
        ...
        if (AllCloudsInfo[i].userType === "ADMIN") {
          ...
          if (
            ...
            if (AllCloudsInfo[i].cloudName === "BOX_BUSINESS") {
              var multi_html =
                '<div><a class="" href="#" cid="' +
                AllCloudsInfo[i].id +
                '" onclick="showFailedUeserTable(this)" style="color:#FF4C4C;text-decoration:underline;font-weight:bold;font-size:12px;opacity:0.4;pointer-events:none;">Failed Users Details</a></div>';
            } else {
              var multi_html =
                '<div><a class="" href="#" cid="' +
                AllCloudsInfo[i].id +
            ...
            if (AllCloudsInfo[i].cloudName === "BOX_BUSINESS") {
              ...
              var multi_html =
                '<div><a class="" href="#"cid="' +
                AllCloudsInfo[i].id +
                '" onclick="showFailedUeserTable(this)" style="color:#FF4C4C; text-decoration:underline;font-size:12px;font-weight:bold;">Failed Users Details</a></div>';
            }
          }

          $("#failUserAppend" + CldId).html(multi_html);
          appendHtml = multi_html
            .replace("dispName", AllCloudsInfo[i].userDisplayName)
            .replace(
              "displayName",
            ...
            .replace("totalUsers", total)
            .replace("nameofDomain", AllCloudsInfo[i].domainName)
            .replace(
              "domainName",
              CFManageCloudAccountsAjaxCall.getMaxChars(
                AllCloudsInfo[i].domainName,
                30,
              ),
            )
          ...
          if (used === null || used === 0) {
            used = "----";
          } else used = CFManageCloudAccountsAjaxCall.getObjectSize(used);
          if (notUsed === null || notUsed === 0) {
            notUsed = "----";
          } else notUsed = CFManageCloudAccountsAjaxCall.getObjectSize(notUsed);
          if (_total === null || _total === 0) {
            _total = "----";
          } else _total = CFManageCloudAccountsAjaxCall.getObjectSize(_total);
          if (AllCloudsInfo[i].loadLock === false) {
            var syncIcon =
                '<i class="lnil lnil-reload" id="refreshIcon" style="font-weight:600;"></i>',
              syncStatus = "Completed";
          } else {
            var syncIcon =
                '<i class="cloudSpinn" id="refreshIcon" style="font-weight:600;"></i>',
          ...
          appendHtml = single_html
            ...
            .replace("totalSpace", _total)
            .replace("syncIcon", syncIcon)
            .replace("refreshIcon", AllCloudsInfo[i].id)
            .replace("usedSpace", used)
            .replace("syncStatus", syncStatus)
            .replace("availabeSpace", notUsed);
        }
      }
    }
    $("#manageclouddiv #" + CldId)
      .closest("#cloudInfo")
      .css("height", "auto");
    $("#manageclouddiv #" + CldId)
      .next()
  ...
  failedUsers: function (CldId, cldName, pageNo) {
    ...
    $.ajax({
      ...
      url:
        apicallurl +
        "/users/clouds/multiUser/notprovisioned?adminCloudId=" +
        CldId +
        "&cloudName=" +
        cldName +
        "&pageNo=" +
      ...
      success: function (data) {
        $("#manageclouddiv #" + CldId + ".innerPlus")
          .parent("tr")
          .next(".appendUsersList")
          .find("tbody")
          .html("");
        if (data.length !== 0) {
          ...
          $("#manageclouddiv .paginationReport").twbsPagination({
            ...
            first:
              '<i class="fa fa-angle-left"  style="font-size: 19px;"></i><i class="fa fa-angle-left" style="font-size: 19px;"></i>',
          ...
          for (var j = 0; j < data.length; j++) {
            var _html =
              '<tr style="height: 5vh;"><td style="border: 1px solid lightgray;font-size:12px;padding:2%;" title="usersmail">userName</td><td style="border: 1px solid lightgray;font-size:12px;">description</td><td style="border: 1px solid lightgray;font-size:12px;">solution</td></tr>';
            appendHtml = _html
              .replace(
                "userName",
                CFManageCloudAccountsAjaxCall.getMaxChars(
              ...
              .replace("usersmail", data[j].userEmail)
              .replace("description", data[j].description)
              .replace("solution", data[j].solution);
            $("#manageclouddiv #" + CldId)
              .next()
              .next(".appendcloudDetails")
              .find(".innerList")
  ...
  renameCloud: function (a) {
    var _u =
      apicallurl +
      "/users/update/cloud/displayname/" +
      a.id +
      "?displayName=" +
      a.name;
    var _b = false;
  ...
  CloudSyncStatus: function (cloudid) {
    ...
    $.ajax({
      ...
      success: function (Syncstatus) {
        if (Syncstatus == false) {
          $("#cloudDetails")
            .find('[id="' + cloudid + '"].cloudSpinn')
            .addClass("lnil lnil-reload")
            .removeClass("cloudSpinn");
          $("#initialVal").text("Completed");
          if (PageName == "CloudDrive") {
            CFManageCloudAccountsAjaxCall.getAllRootFoldersAndFiles(
              SingleCloudId,
              1,
            );
          }
          if (
            PageName == "InnerFolders" &&
            !$("#CFSharedWithMe").parent().hasClass("active")
          ) {
            CFManageCloudAccountsAjaxCall.gotoInnerFolderandFiles(
              SingleCloudId,
              SinglePId,
              1,
            );
        ...
        if (Syncstatus == true) {
          ...
          $("#cloudDetails")
            .find('[id="' + cloudid + '"].lnil.lnil-reload')
            .addClass("cloudSpinn")
            .removeClass("lnil lnil-reload");
          if (PageName == "move") {
            $("#initialVal").text("In Progress");
            $(".cloudSpinn")
              .addClass("cloudSpinn")
  ...
  getAllEvents: function (number, len) {
    var apiUrl =
      apicallurl + "/events/all?page_size=" + len + "&page_nbr=" + number;
    $.ajax({
      type: "GET",
      url: apiUrl,
      async: false,
      ...
      success: function (events) {
        ...
        if (number == 1) {
          ...
          if (events == null || events.length == 0) {
            $("#notify_count").hide();
            $(
              ".notification_footer button,.notification_head #ignore,.notification_head #clear",
            ).addClass("buttonDisable");
            _notiBody.append(
              "<li style='font-weight: bold;text-align: center'>You don't have any notifications.</li>",
            );
          }
        }
        var event = {
        ...
        var linktype = {
          ...
          WORKSPACE_FILE_ADDED:
            "pages/workspace.html?wspaceshared=true&workspaceId=",
          WORKSPACE_FILE_VIEWED:
            "pages/workspace.html?wspaceshared=true&workspaceId=",
        ...
        if (events != null || events.length != 0) {
          $.each(events, function (i, e) {
            ...
            var _owner = JSON.parse(
              localStorage.getItem("CFUser"),
            ).primaryEmail;
            ...
            var _html =
              ...
              '<span class="content"><a data-target="' +
              link +
              '">' +
              '<p style="margin:0">' +
              e.description +
      ...
      success: function (eventCount) {
        ...
        if (eventCount != null) {
          ...
          if (eventCount.workspace > 0 || eventCount.workspaceFileAdded > 0) {
            var count =
              parseInt(eventCount.workspace) +
              parseInt(eventCount.workspaceFileAdded);
            _monBadge.show();
  ...
  doIgnoreEvents: function (eventId) {
    ...
    if (
      eventId == "FILESHARE" ||
      eventId == "WORKSPACE" ||
      eventId == "WORKSPACE_FILE_ADDED" ||
      eventId == "WORKSPACE_FILE_VIEWED"
    ) {
  ...
  getSortFiles: function (sortCond, PageNumber) {
    ...
    if (PageName == "Folders") {
      apiUrl =
        ...
        "/filefolder/user/" +
        CFManageCloudAccountsAjaxCall.getUserId() +
        "/childfolders/all?page_size=50&isAscen=" +
        urlParameterObject.isAscen +
        "&orderField=" +
        sortCond +
        "&page_nbr=" +
        PageNumber +
        "&fetchCollabInfo=true&folderId=&isAllFiles=false";
      ...
      apiUrl =
        ...
        "/childfolders/" +
        SingleCloudId +
        "?page_size=50&isAscen=" +
        urlParameterObject.isAscen +
        "&orderField=" +
        sortCond +
        "&page_nbr=" +
        PageNumber +
        "&fetchCollabInfo=true&folderId=&isAllFiles=true";
      ...
      apiUrl =
        ...
        "/fileshare/user/" +
        CFManageCloudAccountsAjaxCall.getUserId() +
        "?page_nbr=1&page_size=50&isAscen=" +
        urlParameterObject.isAscen +
        "&orderField=" +
        sortCond +
        "&sharedBy=me";
      ...
      apiUrl =
        ...
        "?page_nbr=1&page_size=50&isAscen=" +
        urlParameterObject.isAscen +
        "&fetchCollabInfo=true&orderField=" +
        sortCond +
        "&sharedBy=others";
      ...
      apiUrl =
        ...
        "/filefolder/user/" +
        CFManageCloudAccountsAjaxCall.getUserId() +
        "/cloud/all?page_size=50&isAscen=" +
        urlParameterObject.isAscen +
        "&orderField=" +
        sortCond +
        "&fetchCollabInfo=true&page_nbr=" +
        PageNumber +
        "";
  ...
  getAllFiles: function (PageNumber) {
    var apiUrl =
      ...
      urlParameterObject.isAscen +
      "&orderField=" +
      urlParameterObject.orderField +
      "&page_nbr=" +
      PageNumber +
      "&fetchCollabInfo=" +
      urlParameterObject.fetchCollabInfo +
      "&folderId=&isAllFiles=" +
      urlParameterObject.isAllFiles;
    var _a = "";
    $.ajax({
      type: "GET",
      url: apiUrl,
  ...
  getAllFileNames: function (
    ALLFilesForAllClouds,
    PageNumber,
    fileshareUrl,
    sharedFolderId,
  ) {
    enblePanel();
    $("#LVHeader #CFHSortFileName,#LVHeader #CFHSortfileSize").removeAttr(
      "style",
    );
    ...
    if (
      ...
      if (moveCheckSum == "source" && PageNumber == 1) {
        ...
        _moveSrc.append(
          '<div class="list-group" style="max-height: 45vh;width: 87%;height:45vh;margin: 0px 6.5%;">',
        );
        $("#moveSource").scrollTop($("#moveSource").scrollTop() - 10);
      }
      if (moveCheckSum == "dest" && PageNumber == 1) {
        _moveDest.append(
          '<div class="list-group" style="height:auto;margin-top: 4%;">',
        );
      ...
      if (
        ...
        _moveSrc
          .siblings("#movePageShowMore")
          .attr("movepagenumber", PageNumber);
      } else if (
        AllCloudsInfo[i].cloudName == "AMAZON" &&
        moveCheckSum == "dest" &&
        _dstnToken != "null" &&
        ...
        AllCloudsInfo[i].cloudName == "SHAREPOINT_CONSUMER_HYBRID" &&
        moveCheckSum == "source" &&
        _srcToken != "null" &&
        _srcToken != null
      ) {
        ...
        (ALLFilesForAllClouds.length > 24 &&
          AllCloudsInfo[i].cloudName == "G_DRIVE") ||
        (ALLFilesForAllClouds.length > 24 &&
          AllCloudsInfo[i].cloudName == "SHARED_DRIVES")
      ) {
        if (moveCheckSum == "source") {
          _moveSrc.siblings("#movePageShowMore").show();
          _moveSrc
    ...
    if (
      ALLFilesForAllClouds.length > 49 &&
      AllCloudsInfo[i].cloudName == "BOX" &&
      moveCheckSum == "source"
    ) {
      _scroll = true;
    } else if (
    ...
    $.each(ALLFilesForAllClouds, function (i, AllCloudFileNames) {
      if (
        AllCloudFileNames != null &&
        AllCloudFileNames.objectName != null &&
        AllCloudFileNames.objectName != " " &&
        AllCloudFileNames.objectName != ""
      ) {
        ...
        if (fileshareUrl != undefined) {
          fileshareUrl = fileshareUrl.replace(
            /([?|&]fileId=)[^\&]+/,
            "$1" + fileId,
          );
        ...
        var fileIcon = CFManageCloudAccountsAjaxCall.getFileIcon(
          fileExten,
          filetype,
        );
        var dateCreated =
          AllCloudFileNames.createdTime == null
            ? "___"
            : CFManageCloudAccountsAjaxCall.getDateConversion(
                AllCloudFileNames.createdTime,
              );
        var dateModified =
          AllCloudFileNames.modifiedTime == null
            ? "___"
            : CFManageCloudAccountsAjaxCall.getDateConversion(
                AllCloudFileNames.modifiedTime,
              );
        ...
        if (PageName == "InnerWorkspaceFolders") {
          var _workFile =
            ...
            '<td class="sorting_1" style="cursor:pointer;" name=' +
            filetype1 +
            " cloudid=" +
            AllCloudFileNames.cloudId +
            " fileper=" +
            AllCloudFileNames.id +
            " data-type=" +
            AllCloudFileNames.fileType +
            " fexten=" +
            AllCloudFileNames.objectExtn +
            " id=" +
            AllCloudFileNames.id +
            ' title="' +
            src2 +
          ...
          if ((filetype == "FILE" || filetype == "NOTE") && (a || b)) {
            ...
            var _destFile = "";
            moveCheckSum == "source"
              ? $("#moveSource").find(".list-group").append(_srcFile)
              : $("#moveDestination").find(".list-group").append(_destFile);
          } else {
            if (grandParent === null || grandParent === undefined) {
              var _srcFile =
                '<span class="list-group-item dropabbleParent fileSpan"  srcParent="' +
                parent +
                '"  id="' +
                src3 +
                '" cid="' +
                cloudId +
                '" style="height:30px;padding:4px;border-radius:0px;padding:1%;">' +
                ...
                '" size="' +
                objectSize +
                '" style="width:92%;font-weight:100"><i style="float:left;margin:4px 10px 0px 10px;cursor:move;font-size:18px;" class="' +
                typeicon +
                ' fileIcon"></i>' +
                '<p class="fileP"  title="' +
                src2 +
                '" style="float:left;cursor:move;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;font-size:13pxmargin:0;margin-top: 0.5%;font-weight:400;">' +
                src1 +
                '</p></label><i id="verifyStatus"></i></span>';
              if (prevPrntId !== undefined) {
                var _srcFile =
                  '<span class="list-group-item dropabbleParent fileSpan" prevParentId ="' +
                  prevPrntId +
                  '" srcParent="' +
                  parent +
                  '"  id="' +
              ...
              var _srcFile =
                '<span class="list-group-item dropabbleParent"  srcParent="' +
                parent +
                '" srcGrandParent="' +
                grandParent +
                '" id="' +
              ...
              if (prevPrntId !== undefined) {
                var _srcFile =
                  '<span class="list-group-item dropabbleParent"  prevParentId ="' +
                  prevPrntId +
                  '" srcParent="' +
                  parent +
                  '" srcGrandParent="' +
            ...
            var _destInput =
              filetype1 == true
                ? '<input type="radio" style="float:left;margin-left:2%;margin-top:1.5%;width:10px;height:10px;" name="srcfile" />'
                : '<input type="radio" style="float:left;opacity:0.2;margin-left:2.5%;margin-top:1.5%;width:10px;height:10px;" name="srcfile" disabled />';
            if (grandParent === null || grandParent === undefined) {
              var _destFile =
                '<span class="list-group-item dropabbleParent fileSpan" dstnParent="' +
                parent +
                '" id="' +
                src3 +
                '" cid="' +
                ...
                '" cid="' +
                cloudId +
                '" style="width:92%;font-weight:100">' +
                '<i class="' +
                typeicon +
                ' fileIcon" style="float:left;margin:4px 10px 0px 10px;cursor:pointer;font-size:18px;"></i>' +
                '<p class="fileP" title="' +
                src2 +
                '" style="float:left;cursor:pointer;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;font-size:13pxmargin:0;margin-top: 0.5%;font-weight:400;">' +
                src1 +
                '</p></label><i id="verifyStatus"></i></span>';
              if (prevPrntId !== undefined) {
                var _destFile =
              ...
              var _destFile =
                '<span class="list-group-item dropabbleParent" dstnGrandParent="' +
                grandParent +
                '" dstnParent="' +
                parent +
                '" id="' +
                ...
                '" cid="' +
                cloudId +
                '" style="height:30px;padding:4px;border-radius:0px;color:#1F2129;padding:1%;">' +
                _destInput +
                '<label data-type="' +
                filetype +
                '" id="' +
                ...
                '<i class="' +
                typeicon +
                '" style="float:left;margin:4px 10px 0px 10px;cursor:pointer;font-size:18px;"></i>' +
                '<p  title="' +
                src2 +
                '" style="float:left;cursor:pointer;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;font-size:13pxmargin:0;margin-top: 0.5%;font-weight:400;">' +
                src1 +
          ...
          var _Lfile =
            ...
            filetype +
            '">' +
            '<div class="LVcheckBox" name="' +
            filetype +
            '"><input type="checkbox"/></div>' +
            //'<div class="LVfileName" id="' + fileId + '" style="height:20px" name="' + filetype + '" ' + _shareHtml +
            '<div class="LVfileName" id="' +
            fileId.replace('"', "&quot;") +
            '" style="height:20px" name="' +
            filetype +
            '" ' +
            _shareHtml +
            '<i class="pull-left LV' +
            filetype +
            '"></i>' +
            /*'<sup class="share ' + _shareStatus + '"></sup>' +*/
            '<p class="pull-left displvfilename" name="' +
            objectName +
            '" fexten="' +
            fileExten +
            '">' +
            CFManageCloudAccountsAjaxCall.getMaxChars(objectName, 50) +
            "</p>" +
            '<i class="LVShare' +
            AllCloudFileNames.fileShared +
            '"></i></div>' +
            '<div class="LVFavorites"><a href="#" id="LVFavorite" class="' +
            favouriteicon +
            '"></a></div>' +
            '<div class="LVfileSize" data-type=' +
            objectSize +
            ' style="cursor:pointer;">' +
            CFManageCloudAccountsAjaxCall.getObjectSize(objectSize, filetype) +
            "</div>" +
            '<div class="LVdrive" id="' +
            cloudId +
            '">' +
            CLName[cloudName] +
            "</div>" +
            '<div class="LVaddedDate">' +
            dateCreated +
            '</div><div class="LVmodifiedDate">' +
            dateModified +
            "</div></div>";
          ...
          var _TFile =
            ...
            '" id="' +
            cloudId +
            '" style="cursor:pointer;" data-type="' +
            filetype +
            '">' +
            '<i title="' +
            objectName +
            '" class="filethumbnail" name="' +
            filetype +
            '"></i>' +
            '<strong class="filename" pid="' +
            parent +
            '" id="' +
            fileId +
            '" fexten="' +
            fileExten +
            '">' +
            CFManageCloudAccountsAjaxCall.getMaxChars(objectName, 14) +
            "</strong><div" +
            ' class="filesize"' +
            " data-type=" +
            objectSize +
            ...
            CFManageCloudAccountsAjaxCall.getObjectSize(objectSize, filetype) +
            "</div>" +
            '<input type="checkbox" class="fileCheck" name="' +
            filetype +
            '"/>' +
            '<i id="ThFav" class="' +
            favouriteicon +
  ...
  getAllFolders: function (toId, PageNumber) {
    var apiUrl =
      ...
      "/childfolders/" +
      toId +
      "?page_size=40&isAscen=false&orderField=createdTime&page_nbr=" +
      PageNumber +
      "&fetchCollabInfo=true&folderId=&isAllFiles=false";
    $.ajax({
      type: "GET",
  ...
  getChildFolders: function (CId, FId) {
    var apiUrl =
      ...
      "/childfolders/" +
      CId +
      "?page_size=40&isAscen=false&orderField=createdTime&page_nbr=1&fetchCollabInfo=true&folderId=" +
      FId +
      "&isAllFiles=false";
    $.ajax({
      type: "GET",
      ...
      success: function (ChildFolders) {
        if (ChildFolders.length > 0) {
          $.each(ChildFolders, function (i, Folders) {
            $("#MoveToFunctionality ul#" + FId + "").append(
              '<li id="' +
                Folders.id +
                '" ><div style="display: inline-block;width: 100%;padding-left:10px; "><a href="javascript:void(0)" id="' +
                Folders.id +
                '" name="Folder" style="float:left;" class="arrw"></a><i style="float:left;padding-left:5px; color:#fff;" class="loc" id="' +
                Folders.id +
                '" name="Folder">' +
                Folders.objectName +
                '</i></div><ul id="' +
  ...
  getObjectSize: function (sizeBytes, type) {
    if (sizeBytes == undefined || sizeBytes == null || sizeBytes == 0) {
      ...
      var displaySize = "___";
      var fsize = 0;
      if(Math.round(sizeBytes / ONE_TB) > 0){
          fsize = sizeBytes / ONE_TB;
          displaySize = parseFloat(fsize).toFixed(2) + " TB";
      }
      else if (Math.round(sizeBytes / ONE_GB) > 0) {
  ...
  moveToAjaxCall: function (moveFileObject, files, sharePointListItemId) {
    ...
    var gprt = $("#moveSource")
      ...
      .attr("srcgrandparent");
    if (
      $("#moveSourceCheckbox").is(":checked") &&
      (gprt == null || gprt == "")
    ) {
      jsonArray.fromRootId = $("#moveSource")
        .find(".list-group")
        .find("span")
        .attr("srcparent");
    }
    //srikanth end
    var _srcCldName = $(".tab-header.srcFilesFldrCnt")
      .find(".moveHeaderIcon")
      .attr("id");
    var _dstCldName = $(".tab-header.dstnFilesFldrCnt")
      .find(".moveHeaderIcon")
      .attr("id");
    if (
      _srcCldName == "WASABI" ||
      _srcCldName == "DROP_BOX" ||
      _srcCldName == "ONEDRIVE" ||
      _srcCldName == "G_DRIVE" ||
      _srcCldName == "BOX" ||
      _srcCldName == "GOOGLE_STORAGE" ||
      _srcCldName == "AZURE_OBJECT_STORAGE" ||
      _srcCldName == "SHAREPOINT_ONLINE_CONSUMER" ||
      _srcCldName == "SHAREPOINT_CONSUMER_HYBRID" ||
      _srcCldName == "AMAZON" ||
      _srcCldName == "SHARED_DRIVES"
    ) {
      ...
      var apiurl =
        apicallurl + "/move/clouds/files?isCopy=" + moveFileObject.isCopy;

    if (
      _srcCldName == "FTP" ||
      _dstCldName == "SHAREPOINT_ONLINE_CONSUMER" ||
      (_srcCldName == "NTLM_STORAGE" && _dstCldName == "G_DRIVE") ||
      (_srcCldName == "NTLM_STORAGE" && _dstCldName == "SHARED_DRIVES")
    ) {
      var apiurl = apicallurl + "/move/consumer/create";
    }
    if (
      _srcCldName == "NTLM_STORAGE" ||
      _dstCldName == "SHAREPOINT_CONSUMER_HYBRID"
    ) {
    ...
    $.ajax({
      ...
      success: function (move) {
        ...
        alertSuccess(
          "Your migration has been initiated. You can monitor it here or log off and see the migration report that will be emailed to you.",
        );
        ...
        success: function (status) {
          ...
          switch (moveStatus) {
            case "PROCESSED":
              ...
              $('#moveSource [id="' + status.fromRootId + '"]')
                .children("input")
                .prop("checked", false);
    ...
    $.ajax({
      ...
      statusCode: {
        ...
        406: function () {
          ...
          $("#VerifyMsg").text(
            "Already exists in destination folder. Please verify and correct.",
          );
        },
        400: function () {
          $("#VerifyMsg").addClass("verifyFailure");
    ...
    $.ajax({
      ...
      success: function (data) {
        if (status != "all" && !data.length) {
          ...
          $("#" + moveId)
            .siblings(".hide-report2")
            .find("#moveReportFilesList")
            .html("");
          return false;
        }
        var moveStatus = {
        ...
        if (data == undefined || data == null || data.length < 1) {
          $("#moveReportsForUser #" + moveId + " #getMoveDetails")
            .children()
            .removeClass("cf-minus5")
            .addClass("cf-plus5disabled");
          $("#spinner2").hide();
        ...
        if (count.totalFiles != null) {
          ...
          $("#" + moveId)
            ...
            .text(
              "No. of Files/Folders : " +
                (count.totalFiles + count.totalFolders),
            );
          $("#" + moveId)
            .next()
            .find("#_movefolders")
            .text("Conflict Files/Folders : " + _errCount);
          $("#" + moveId)
            .next()
            .find("#_movefiles")
            .text("Processed Files/Folders : " + count.processedCount);
        }
        $("#" + moveId)
          .next()
          .next()
          .find("#_prcsdCnt")
          .text("No: of Processed files : " + count.processedCount);
        $("#" + moveId)
          .next()
          .next()
          .find("#_wrngCnt")
          .text("No: of Warned files : " + count.warningCount);
        $("#" + moveId)
          .next()
          .next()
          .find("#_errCnt")
          .text("No: of Conflict files : " + _errCount);
        ...
        if (page) {
          ...
          var _fcn = formCloud != null ? formCloud.cloudName : "Not Available",
            _fcd =
              formCloud != null ? formCloud.userDisplayName : "Not Available",
        ...
        switch (status) {
          case "PROCESSED":
            _pgntnCnt = count.processedCount;
            break;
          case "ERROR":
            _pgntnCnt = _errCount;
            break;
            ...
            onPageClick: function (event, page) {
              if (page == 1) {
                ...
                $("#" + moveId)
                  ...
                  .next()
                  .find("#moveFiles_pagination")
                  .twbsPagination({ totalPages: _d });
              }

              _id = moveId;
              var _status = $("#moveReportsContainer")
                .find("#" + moveId + ".selectpicker")
                .val();
              CFManageCloudAccountsAjaxCall.movereportForGiveId(
                moveId,
                page,
                _status,
              );
  ...
  fileFolderCount: function (a, _this) {
    if (_this == undefined) {
      var apiCall =
        apicallurl + "/move/moveworkspacefilefoldercount?workspaceId=" + a;
    } else if (
      (_this.parents("tr:eq(0)").attr("srccldname") == "BOX_BUSINESS" &&
        _this.parents("tr:eq(0)").attr("dstncldname") == "DROPBOX_BUSINESS") ||
      (_this.parents("tr:eq(0)").attr("srccldname") == "G_SUITE" &&
        _this.parents("tr:eq(0)").attr("dstncldname") == "DROPBOX_BUSINESS")
    ) {
      var apiCall =
        apicallurl +
    ...
    $.each(data, function (i, file) {
      ...
      if (_a) {
        ...
        _a =
          '<i id="_error' +
          file.id +
          '" class="cf-warning3" style="margin-left:5px;color:red"></i>';
      } else {
        _errorDesc = "";
        _a = "<i></i>";
      }
      ...
      $("#" + moveId)
        ...
        .append(
          "<tr>" +
            ...
            '" ><i class="blockCode ' +
            filetype +
            '" style="margin-right:4%;font-size:16px;"></i>' +
            CFManageCloudAccountsAjaxCall.getMaxChars(fi.objectName, 30) +
            "</td>" +
            '<td class="' +
            fstatus +
            '">' +
            ...
            "</td>" +
            "<td>" +
            CFManageCloudAccountsAjaxCall.getDateConversion(file.createdTime) +
            "</td></tr>",
        );
      if ($("#_error" + file.id).length) {
    ...
    $.ajax({
      ...
      success: function (data) {
        var moveStatus = [
          ...
          {
            key: "PROCESSED_WITH_SOME_ERRORS",
            value: "Processed with some errors",
          },
          {
            key: "PROCESSED_WITH_SOME_WARNINGS",
            value: "Processed with some warnings",
          },
          {
            key: "PROCESSED_WITH_SOME_CONFLICTS",
            value: "Processed with some conflicts",
          },
          {
            key: "PROCESSED_WITH_SOME_PAUSE",
            value: "Trial completed and paused",
          },
        ...
        $.each(data, function (i, move) {
          ...
          if (move.multiUserMove == true || move.consumerBackup == true) {
            ...
            _ertitle =
              move.errorDescription != null
                ? move.errorDescription
                : "Error occurred while moving the files, please contact the support team";
            ...
            toCloud.userDisplayName =
              toCloud.userDisplayName == null
                ? toCloud.cloudUserId.split("|")[
                    toCloud.cloudUserId.split("|").length - 1
                  ]
                : toCloud.userDisplayName;
            fromCloud.userDisplayName =
              fromCloud.userDisplayName == null
                ? fromCloud.cloudUserId.split("|")[
                    fromCloud.cloudUserId.split("|").length - 1
                  ]
                : fromCloud.userDisplayName;
            ...
            if (totalFiles == 0) {
              ...
              if (move.fileName == null) {
                var _html =
                  '<tr id="' +
                  move.id +
                  '" style="font-size: 14px;height: 40px;border-bottom:1px solid #ccc" data-errorcount="' +
                  move.errorCount +
                  '">' +
                  "<td>Undefined</td>" +
                  '<td><div class="Driveicon pull-left"><div class="drive" id="' +
                  move.fromCloudId.cloudName +
                  '"></div></div><span style="display: block;padding-top:4%;float: left;" title="' +
                  fromCloud.userDisplayName +
                  '">' +
                  CFManageCloudAccountsAjaxCall.getMaxChars(
                    fromCloud.userDisplayName,
                  ...
                  jQuery.timeago(move.createdTime) +
                  "</td>" +
                  ...
                  '<td mid="' +
                  move.id +
                  '" id="getMoveDetails"><i class="lnil lnil-chevron-down-circle" style="color: #0062FF;font-size: 18px;font-weight: 600;margin-left: 7px;cursor: pointer;"></i></td></tr>' +
                  ...
                  _filterHtml +
                  '<tr class="hide-report2" style="display:none"><td colspan="8"><table class="table" style="margin-left: 5%;width: 90%;"><thead style="background:#f2f3ff;"><tr><th style="width:30%;">File Name</th><th style="width:28%;">Status</th><th style="width:20%;">Size</th><th style="width:22%;">Created Date</th></tr></thead>' +
                ...
                if (
                  colorClass == "CANCEL" ||
                  colorClass == "NOT_PROCESSED" ||
                  colorClass == "PROCESSED" ||
                  colorClass == "PROCESSED_WITH_SOME_CONFLICTS" ||
                  colorClass == "PROCESSED_WITH_SOME_CONFLICTS_AND_PAUSE" ||
                  colorClass == "PROCESSED_WITH_SOME_ERRORS" ||
                  colorClass == "PROCESSED_WITH_SOME_PAUSE" ||
                  colorClass == "PROCESSED_WITH_SOME_WARNINGS" ||
                  colorClass == "ERROR" ||
                  ...
                  _html = _html.replace(
                    "_cancelImg",
                    '<i class="lnil lnil-cross-circle" style="text-align: center;cursor: not-allowed;opacity:0.2;margin-top: -3px;color: #FF4C4C;font-size: 15px;font-weight: 600;"></i>',
                  );
                } else {
                  _html = _html.replace(
                    "_cancelImg",
                    '<i class="lnil lnil-cross-circle cancelMig" style="text-align: center;cursor: pointer;margin-top: -3px;color: #FF4C4C;font-size: 15px;font-weight: 600;"></i>',
                  );
                }
                ...
                var _html =
                  ...
                  move.fileName +
                  '">' +
                  CFManageCloudAccountsAjaxCall.getMaxChars(move.fileName, 25) +
                  "</td>" +
                  '<td><div class="Driveicon pull-left"><div class="drive" id="' +
                  move.fromCloudId.cloudName +
                  '"></div></div><span style="display: block;padding-top:4%;float: left;" title="' +
                  ...
                  '<td mid="' +
                  move.id +
                  '" id="getMoveDetails"><i class="lnil lnil-chevron-down-circle" style="color: #0062FF;font-size: 18px;font-weight: 600;margin-left: 7px;cursor: pointer;"></i></td>' +
                  "</tr>" +
                  '<tr class="hide-report1" style="display: none;font-weight: bold;font-size: 14px;height:40px"><td style="text-align: center;padding-top:2%;" colspan="7"><div  id="_totfiles" style="border-radius: 4px;color:#1F2129;padding: 1%;width: 28%;float: left;margin-left: 5%;"></div><div id="_movefiles" style="border-radius: 4px;color:#00C64F;padding: 1%;width: 28%;float: left;margin-left: 5%;"></div><div id="_movefolders" style="border-radius: 4px;color:#FF4C4C;padding: 1%;width: 28%;float: left;margin-left: 5%;"></div></td><td></td></tr>' +
                  _filterHtml +
                  '<tr class="hide-report1" style="display: none;font-weight: bold;font-size: 14px;height:40px"><td></td><td id="_prcsdCnt" style="text-align: center"></td><td id="_wrngCnt" style="text-align: right"></td><td></td><td id="_errCnt"></td><td class="_filter" align="right">' +
              ...
              if (move.fileName == null) {
                ...
                if (
                  ...
                  _html = _html.replace(
                    "_cancelImg",
                    '<i class="lnil lnil-cross-circle" style="cursor: not-allowed;opacity:0.2;margin-top: -3px;color: #FF4C4C;font-size: 15px;font-weight: 600;"></i>',
                  );
                } else {
                  _html = _html.replace(
                    "_cancelImg",
                    '<i class="lnil lnil-cross-circle cancelMig" style="cursor: pointer;margin-top: -3px;color: #FF4C4C;font-size: 15px;font-weight: 600;"></i>',
                  );
                }
                $("#moveReportsForUser").append(_html);
              }
            ...
            if (totalFiles == 0) {
              ...
              if (move.fileName == null) {
                var _html =
                  ...
                  '">' +
                  "<td>Undefined</td>" +
                  '<td><div class="Driveicon pull-left" style="display:none;"><div class="drive" id="' +
                  FromCloud.cloudName +
                  '"></div></div><span style="display: block;padding-top:4%;float: left;" title="' +
                  FromCloud.userDisplayName +
                  '">' +
                  ...
                  '<td mid="' +
                  move.id +
                  '" id="getMoveDetails"><i class="lnil lnil-chevron-down-circle disabled" style="color: #0062FF;font-size: 18px;font-weight: 600;margin-left: 7px;cursor: pointer;color: rgba(51,51,51,0.65);"></i></td></tr>' +
                  '<tr class="hide-report1" style="display: none;font-weight: bold;font-size: 14px;height:40px"><td style="text-align: center;padding-top:2%;" colspan="7"><div  id="_totfiles" style="border-radius: 4px;color:#1F2129;padding: 1%;width: 28%;float: left;margin-left: 5%;"></div><div id="_movefiles" style="border-radius: 4px;color:#00C64F;padding: 1%;width: 28%;float: left;margin-left: 5%;"></div><div id="_movefolders" style="border-radius: 4px;color:#FF4C4C;padding: 1%;width: 28%;float: left;margin-left: 5%;"></div></td><td></td></tr>' +
                  _filterHtml +
                  '<tr class="hide-report1" style="display: none;font-weight: bold;font-size: 14px;height:40px"><td></td><td id="_prcsdCnt" style="text-align: center"></td><td id="_wrngCnt" style="text-align: right"></td><td></td><td id="_errCnt"></td><td class="_filter" align="right">' +
                  _filterHtml +
  ...
  getAllMoveStatusForSyncUser: function () {
    var apiUrl =
      apicallurl +
      "/move/consumer/get/moveJob?page_size=60&page_nbr=" +
      PageNumber;
    $.ajax({
      type: "GET",
      url: apiUrl,
      ...
      success: function (data) {
        ...
        for (var i = 0; i < _len; i++) {
          if (
            (data[i].fromCloudId == null || data[i].toCloudId == null) &&
            data[i].jobStatus == "CANCEL"
          ) {
            _cancelImg =
              '<i class="lnil lnil-cross-circle cleBtn" style="cursor: not-allowed;opacity:0.2;margin-top: -3px;color: #FF4C4C;font-size: 15px;font-weight: 600;"></i>';
            _ResumeImg =
              '<img src="../img/Pause.png" style="cursor: not-allowed;opacity:0.2" class="dsblePause">';
          } else {
            if (data[i].jobStatus == "CANCEL") {
              ...
              _ResumeImg =
                '<img src="../img/Resume.png" style="cursor: pointer;" class="conSyncPause">';
              _cancelImg =
                '<i class="lnil lnil-cross-circle cnclConBtn" style="cursor: pointer;margin-top: -3px;color: #FF4C4C;font-size: 15px;font-weight: 600;"></i>';
              //    $(".fa-download").css("opacity", "0.2").css("cursor", "not-allowed");
            } else if (data[i].jobStatus == "IN_PROGRESS") {
              _ResumeImg =
                '<img src="../img/Pause.png" style="cursor: pointer;" class="conSyncPause">';
              _cancelImg =
                '<i class="lnil lnil-cross-circle cnclConBtn" style="cursor: pointer;margin-top: -3px;color: #FF4C4C;font-size: 15px;font-weight: 600;"></i>';
              //   $(".fa-download").css("opacity", "0.2").css("cursor", "not-allowed");
            } else if (
              data[i].jobStatus == "COMPLETED" ||
              data[i].jobStatus == "PROCESSED_WITH_SOME_CONFLICTS"
            ) {
              _ResumeImg =
                '<img src="../img/Pause.png" style="cursor: pointer;" class="conSyncPause">';
              _cancelImg =
        ...
        var _appendHtml =
          '<tr id="jobID" style="font-size: 14px;height: 40px;border-bottom:1px solid #ccc;border-top: 1px solid #ccc!important">' +
        ...
        for (var i = 0; i < _len; i++) {
          ...
          if (_data.jobStatus != "STARTED") {
            _html = _appendHtml
              .replace("jobID", data[i].id)
              .replace("jobId", data[i].id)
              .replace("wsLngth", data[i].listOfMoveWorkspaceId.length)
              .replace("jobname", data[i].jobName)
              .replace(
                "JobName",
                CFManageCloudAccountsAjaxCall.getMaxChars(data[i].jobName, 25),
              )
              .replace("stats", data[i].jobStatus)
              .replace("JobStatus", moveStatus[data[i].jobStatus])
              .replace("Status", data[i].jobStatus)
              .replace("createdTime", jQuery.timeago(data[i].createdTime))
              .replace("_ResumeImg", iconsArray[i].display)
              .replace("_cancelImg", iconsArray[i].cancel);
  ...
  SyncReports: function (jobid, pageNumber, workSpaceLen) {
    $("#moveSyncReportsForUser")
      .find("#" + jobid)
      .next(".report")
      .show();
    var apiUrl =
      apicallurl +
      "/move/consumer/get/list/" +
      jobid +
      "?page_nbr=" +
      pageNumber +
      "&page_size=50";
      ...
      success: function (data) {
        var moveStatus = {
          ...
          PROCESSED_WITH_SOME_CONFLICTS_AND_PAUSE:
            "Trial completed and pause with conflicts",
        };

        var _appendHtml =
          '<tr id="jobID" style="font-size: 14px;height: 40px;border-bottom:1px solid #ccc;border-top: 1px solid #ccc!important" TotalCount = "totalFiles" PrcsdCount ="processedFiles" conflictCount ="ConflictFiles">' +
          '<td><div class="Driveicon pull-left"><div class="drive" id="fromCldName"></div></div><span style="display: block;margin-top: 5px;float: left;" id="cldIdFrom" title="fromCloudName">' +
          CFManageCloudAccountsAjaxCall.getMaxChars("fromCloudName", 25) +
          "</span></td>" +
          '<td><div class="Driveicon pull-left"><div class="drive" id="toCldName"></div></div><span style="display:block;margin-top: 5px;float: left;" id="cldIdTo" title="toCloudName">' +
          CFManageCloudAccountsAjaxCall.getMaxChars("toCloudName", 25) +
          "</span></td>" +
          '<td><span class="stats">JobStatus</span></td>' +
        ...
        for (var i = 0; i < _len; i++) {
          ...
          if (_data.jobStatus != "STARTED") {
            _html = _appendHtml
              .replace("jobID", data[i].jobId)
              .replace("fromCldName", data[i].fromCloudName)
              .replace(
                "srcImage",
                "../img/PNG/" + data[i].fromCloudName + ".png",
              )
              .replace("toCldName", data[i].toCloudName)
              .replace(
                "destImage",
                "../img/PNG/" + data[i].toCloudName + ".png",
              )
              .replace("fromCloudName", data[i].fromMailId)
              .replace("fromCloudName", data[i].fromMailId)
              .replace("toCloudName", data[i].toMailId)
              .replace("toCloudName", data[i].toMailId)
              .replace("stats", data[i].processStatus)
              .replace("JobStatus", moveStatus[data[i].processStatus])
              .replace("createdTime", jQuery.timeago(data[i].createdTime))
              .replace("jobId", data[i].id)
              .replace("Status", data[i].processStatus)
              .replace("totalFiles", data[i].totalFilesAndFolders)
              .replace("processedFiles", data[i].processedCount)
        ...
        switch (status) {
          case "PROCESSED":
            pageCount = count.processedCount;
            break;
          case "ERROR":
            pageCount = _errCount;
            break;
          case "IN_PROGRESS":
            pageCount = totalmoved;
            break;
          default:
            pageCount = totalmoved;
            break;
        ...
        $("#" + jobid)
          ...
          .twbsPagination({
            ...
            onPageClick: function (event, pageNumber) {
              ...
              _id = jobid;
              var _syncStatus = $("#moveSyncReportsContainer")
                .find("#" + jobid + ".selectpicker")
                .val();
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("#" + moveId)
          ...
          .find("#moveReportFilesList")
          .html("");
        CFManageCloudAccountsAjaxCall.appendSyncMoveFiles(
          data,
          moveStatus,
          page,
        );
          ...
          .twbsPagination({
            ...
            onPageClick: function (event, page) {
              ...
              var _syncStatus = $("#moveSyncReportsContainer")
                .find("#" + moveId + ".selectpicker")
                .val();
              CFManageCloudAccountsAjaxCall.moveSyncInnerReports(
                moveId,
                page,
                _syncStatus,
              );
  ...
  appendSyncMoveFiles: function (data, b) {
    ...
    $.each(data, function (i, file) {
      ...
      $("#" + moveId)
        ...
        .find("#moveReportFilesList")
        .append(
          '<tr style="height: 4.5vh;border-bottom: 1px solid #e5e5e5;">' +
            '<td class="" title="' +
            fi.objectName +
            '" style="padding-left:1%" ><i class="blockCode ' +
            filetype +
            '" style="margin-right:4%;font-size:16px;"></i><span>' +
            CFManageCloudAccountsAjaxCall.getMaxChars(fi.objectName, 25) +
            "</span></td>" +
            '<td class="' +
            fstatus +
            '">' +
  ...
  getAllRootFoldersAndFiles: function (
    idofCloud,
    PageNumber,
    parentId,
    prevparentid,
    ...
    if (PageName == "Folders") {
      ...
      $("#breadCrumbdync").empty();
      $("#breadCrumbdync").append(
        '<li id="Home" class="BCRFList" cloudId="Home" fileId="All Files"><a href="#" style="color:blue;text-decoration: underline;cursor:default;">Files</a><span class="divider" style=" font-size: 8pt;padding-left: 5px;">&#9658;</span><a href="#"> All Files</a></li>',
      );
      apiUrl =
        apicallurl +
        "/filefolder/user/" +
      ...
      var _srcCldName = $(".tab-header.srcFilesFldrCnt")
        .find(".moveHeaderIcon")
        .attr("id");
      var _dstnCldName = $(".tab-header.dstnFilesFldrCnt")
        .find(".moveHeaderIcon")
        .attr("id");
      var _check = sessionStorage.getItem("source");
      //sessionStorage.removeItem("source");
      if (
        (_srcCldName === "WASABI" && _check === "source") ||
        (_srcCldName === "DROP_BOX" && _check === "source") ||
        (_srcCldName === "ONEDRIVE" && _check === "source") ||
        (_srcCldName === "G_DRIVE" && _check === "source") ||
        (_srcCldName === "SHARED_DRIVES" && _check === "source") ||
        (_srcCldName === "BOX" && _check === "source") ||
        (_srcCldName === "GOOGLE_STORAGE" && _check === "source") ||
        (_srcCldName === "AZURE_OBJECT_STORAGE" && _check === "source") ||
        (_srcCldName === "SHAREPOINT_ONLINE_CONSUMER" && _check === "source") ||
        (_srcCldName === "AMAZON" && _check === "source") ||
        (_srcCldName === "SHAREPOINT_CONSUMER_HYBRID" && _check === "source") ||
        (_dstnCldName === "WASABI" && _check === "destination") ||
        (_dstnCldName === "DROP_BOX" && _check === "destination") ||
        (_dstnCldName === "ONEDRIVE" && _check === "destination") ||
        (_dstnCldName === "G_DRIVE" && _check === "destination") ||
        (_dstnCldName === "SHARED_DRIVES" && _check === "destination") ||
        (_dstnCldName === "BOX" && _check === "destination") ||
        (_dstnCldName === "GOOGLE_STORAGE" && _check === "destination") ||
        (_dstnCldName === "AZURE_OBJECT_STORAGE" && _check === "destination") ||
        (_dstnCldName === "SHAREPOINT_ONLINE_CONSUMER" &&
          _check === "destination") ||
        (_dstnCldName === "AMAZON" && _check === "destination") ||
        (_dstnCldName === "SHAREPOINT_CONSUMER_HYBRID" &&
          _check === "destination")
      ) {
        ...
        if (
          ...
          $("#CFShowLoading").css("display", "none");
          return false;
        } else _pageSizeVal = "&page_size=50";
        ...
        if (moveCheckSum == "dest" && _dstnTokenVal) {
          apiUrl =
            ...
            _pageSizeVal +
            "&folderId=" +
            AllCloudsInfo[i].rootFolderId +
            "&nextPreviousId=" +
            encodeURIComponent(_dstnTokenVal);
        }
        ...
        if (moveCheckSum == "source" && _srcTokenVal) {
          apiUrl =
            ...
            AllCloudsInfo[i].rootFolderId +
            "&nextPreviousId=" +
            encodeURIComponent(_srcTokenVal);
        }
        if (moveCheckSum == "source" && _scrollSrcTokenVal && _scroll == true) {
          if (
            ...
            apiUrl =
              ...
              AllCloudsInfo[i].rootFolderId +
              "&nextPreviousId=" +
              encodeURIComponent(_scrollSrcTokenVal);
          } else
            apiUrl =
              apicallurl +
              "/filefolder/userId/" +
          ...
          if (
            ...
            apiUrl =
              ...
              localStorage.getItem("UserId") +
              "/cloudId/" +
              encodeURIComponent(SingleCloudId) +
              "?page_nbr=" +
              PageNumber +
              _pageSizeVal +
              "&folderId=" +
              encodeURIComponent(SinglePId);
          } else {
            apiUrl =
              apicallurl +
              "/filefolder/userId/" +
              ...
              encodeURIComponent(
                $(
                  "#moveSource .list-group-item.dropabbleParent.ui-draggable.ui-draggable-handle.ui-selectable",
                ).attr("cid"),
              ) +
              "?page_nbr=" +
              PageNumber +
          ...
          if (
            ...
            if (_dstnCldName === "SHAREPOINT_CONSUMER_HYBRID") {
              apiUrl =
                ...
                encodeURIComponent(
                  $(
                    "#moveDestination .list-group-item.dropabbleParent.ui-droppable",
                  ).attr("cid"),
                ) +
                "?page_nbr=" +
                PageNumber +
        ...
        !(
          ...
          (_dstnCldName === "BOX" && _check === "destination") ||
          (_dstnCldName === "GOOGLE_STORAGE" && _check === "destination") ||
          (_dstnCldName === "AZURE_OBJECT_STORAGE" &&
            _check === "destination") ||
          (_dstnCldName === "SHAREPOINT_ONLINE_CONSUMER" &&
            _check === "destination") ||
          (_dstnCldName === "SHAREPOINT_CONSUMER_HYBRID" &&
      ...
      if (
        ...
        var _dstnInnerTokenVal = $("#moveDestination").attr(
          "innerDstnNextPageToken",
        );
        $("#moveDestination").removeAttr("innerDstnNextPageToken");

        if (
          moveCheckSum == "dest" &&
          (_scrollDstnTokenVal || PageName == "scrollDown") &&
          _scrolll == true
        ) {
          if (
            _dstnCldName === "SHAREPOINT_CONSUMER_HYBRID" &&
            ...
            apiUrl =
              ...
              "&folderId=" +
              encodeURIComponent(
                $(".list-group-item.dropabbleParent.ui-droppable").attr(
                  "dstnparent",
                ),
              ) +
              "&nextPreviousId=" +
          ...
          if (
            $(
              "#moveDestination .list-group-item.dropabbleParent.ui-draggable.ui-draggable-handle.ui-selectable",
            ).attr("cid") == undefined ||
            $(
              "#moveDestination .list-group-item.dropabbleParent.ui-draggable.ui-draggable-handle.ui-selectable",
            ).length === 0
        ...
        if (
          moveCheckSum == "source" &&
          (_scrollSrcTokenVal || PageName == "scrollDown") &&
          _scroll == true
        ) {
          if (
            _srcCldName === "SHAREPOINT_CONSUMER_HYBRID" &&
            ...
            apiUrl =
              ...
              encodeURIComponent(
                $(
                  ".list-group-item.dropabbleParent.ui-draggable.ui-draggable-handle.ui-selectable",
                ).attr("srcparent"),
              ) +
              "&nextPreviousId=" +
              encodeURIComponent(_scrollSrcTokenVal) +
        ...
        success: function (RootFoldersandFiles) {
          ...
          if (RootFoldersandFiles.length) {
            if (
              ...
              $("#moveSource").attr(
                "srcNextPageToken",
                RootFoldersandFiles[RootFoldersandFiles.length - 1]
                  .nextPageToken,
              );
            ...
            if (
              moveCheckSum == "source" &&
              (PageName == "innermove" || PageName == "scrollDown")
            ) {
              $("#moveSource").attr(
                "innerSrcNextPageToken",
                RootFoldersandFiles[RootFoldersandFiles.length - 1]
                  .nextPageToken,
              );
              localStorage.setItem(
          ...
          if (
            ...
            if (
              RootFoldersandFiles.length != 0 &&
              RootFoldersandFiles[0].parentFileRef == null &&
              RootFoldersandFiles[0].cloudName !== "SHAREPOINT_CONSUMER_HYBRID"
            ) {
              var _a = [
                "AMAZON",
                "SALES_FORCE",
              ...
              if ($.inArray(RootFoldersandFiles[0].cloudName, _a) > -1) {
                if (moveCheckSum == "dest") {
                  $("#move-header")
                    .find("#dynamicDestCloudName")
                    .attr("fid", RootFoldersandFiles[0].parent);
                }
              }
              type = "getClouds";
              ...
              if (
                ...
                (_dstnCldName === "ONEDRIVE" && _check === "destination") ||
                (_dstnCldName === "G_DRIVE" && _check === "destination") ||
                (_dstnCldName === "SHARED_DRIVES" &&
                  _check === "destination") ||
                (_dstnCldName === "BOX" && _check === "destination") ||
                (_dstnCldName === "GOOGLE_STORAGE" &&
                  _check === "destination") ||
                (_dstnCldName === "AZURE_OBJECT_STORAGE" &&
                  _check === "destination") ||
                (_dstnCldName === "SHAREPOINT_ONLINE_CONSUMER" &&
                ...
                var fileDetails = CFHPlistview.getFileDetails(
                  SinglePId,
                  SingleCloudId,
                );
              } else
              ...
              if (
                ...
                if (moveCheckSum == "dest") {
                  var _destShowMore =
                    $("#moveDestination").siblings("#movePageShowMore");
                  _destShowMore.hide();
                  _destShowMore.attr("movepagenumber", "-1");
                  if (SinglePId == 0) {
                    $("#moveDestination").html("");
                    $("#moveDestination").append(
                      '<div data-type="destback" style="width: 100%;cursor:pointer;height:30px;padding: 2%;margin-bottom: 2%;" name="' +
                        name +
                        '" id="getClouds" cid="' +
                        SingleCloudId +
              ...
              if ($.inArray(fileDetails.objectName, fileName) > -1) {
                ...
                if (
                  ...
                  if (
                    ...
                    destbackId = $('[data-type="destback"]').attr(
                      "previousParentId",
                    );
                    destbackPrevId = $(
                      "#moveDestination .list-group-item.dropabbleParent.ui-droppable",
              ...
              if (
                RootFoldersandFiles[0].parent.split(":")[
                  RootFoldersandFiles[0].parent.split(":").length - 1
                ] == "SITE"
              )
              ...
              if ($('[data-type="destback"]').attr("id") == undefined)
                ...
                $('[data-type="destback"]').attr("id") == "/" &&
                PageName == "back"
              )
                type = "getClouds";
              else {
                var prevLen =
                  RootFoldersandFiles[0].previousParentIds.split("*").length -
                  2;
                var prev =
                  RootFoldersandFiles[0].previousParentIds.split("*")[prevLen];
                type = prev;
              }
            }
            if (moveCheckSum == "source" && PageNumber == 1) {
            ...
            if (moveCheckSum == "dest" && PageNumber == 1) {
              ...
              if (RootFoldersandFiles.length) {
                $("#move-header")
                  ...
                  .attr("fid", RootFoldersandFiles[0].parent);
                if (
                  RootFoldersandFiles[0].cloudName ===
                  "SHAREPOINT_CONSUMER_HYBRID"
                ) {
                  $('[data-type="destback"]').attr(
                    "previousParentId",
                    RootFoldersandFiles[0].previousParentIds,
                  );
                }
              } else {
                $('[data-type="destback"]').attr(
                  "previousParentId",
                  destbackId + "*" + destbackPrevId,
                );
              }
            }
          }
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                $("#moveSource").attr(
                  "srcNextPageToken",
                  RootFoldersandFiles[0].nextPageToken,
                );
                localStorage.setItem(
                  "_scrollSrcTokenVal",
                  RootFoldersandFiles[0].nextPageToken,
              ...
              if (
                ...
                if (moveCheckSum == "dest") {
                  ...
                  if (SinglePId == 0) {
                    $("#moveDestination").html("");
                    $("#moveDestination").append(
                      '<div data-type="destback" style="width: 50%;cursor:pointer;height:30px;padding: 2%;margin-bottom: 2%;" name="' +
                        name +
                        '" id="getClouds" cid="' +
                        SingleCloudId +
                        '"><i style="width:20%;float: left;display: block;font-size: 18px;margin-left: 5%;margin-top: 0.5%;margin-right: -15%;" class="lnil lnil-arrow-left-circle cf-back"></i><span>Back</span></div><div class="custom-search-input-dstn" style="width: 54%;height: 5vh;padding: 5px;position:relative;margin-left: 3%;float: right;margin-top: -7%;"><input type="text" placeholder="Search Files/Folders" style="width: 89%;height: 5vh;border-radius: 3px;border: 2px solid #0062FF;" id="autocomplete" autocomplete="off" class=""><button type="button" style="background: white;position: absolute;height: 4vh;left: 78%;border-radius: 3px;border: none;top: 25%;outline:none;"><i class="fa fa-search" title="Search" style="color: #0062FF;font-size:14px;"></i></button></div>',
              ...
              if (
                ...
                if ($('[data-type="destback"]').attr("id") == undefined)
                  ...
                  var prevLen =
                    ...
                    2;
                  var prev =
                    RootFoldersandFiles[0].previousParentIds.split("*")[
                      prevLen
                    ];
                  type = prev;
                }
                ...
                if (
                  $.inArray(type1.objectName, fileName) > -1 &&
                  type1.parentFileRef == null
                ) {
                  type = "getClouds";
                  if (moveCheckSum == "dest") {
            ...
            if (RootFoldersandFiles.length == 0) {
              ...
              setTimeout(function () {
                ...
                if (moveCheckSum == "source") {
                  ...
                  _brcrdyncFchild = $(
                    "#breadCrumbdyncmovedest > :nth-child(2)",
                  );
                }
                ...
                do {
                  ...
                  if (fileObject != undefined && fileObject != null) {
                    ...
                    if (
                      fileObject.parentFileRef != undefined ||
                      fileObject.parentFileRef != null
                    ) {
                      if ($.inArray(fileObject.objectName, fileName) < 0) {
                        _brcrdyncFchild.after(
                          '<li style="cursor:pointer;" id="' +
                            fileObject.id +
                            '"' +
                            'class="BCRFList" cloudid="' +
                            fileObject.cloudId +
                            '">' +
                            '<span class="divider" style=" font-size: 8pt;padding-left: 5px;">&#9658;</span><a style="cursor:pointer;">' +
              ...
              do {
                test = [];
                if (
                  fileObject[0].parentFileRef != undefined ||
                  fileObject[0].parentFileRef != null
                ) {
                  if (fileCount > 0) {
                    ...
                    if ($.inArray(fileObject.objectName, fileName) < 0) {
                      _brcrdyncFchild.after(
                        '<li style="cursor:pointer;" id="' +
                          fileObject[0].id +
                          '" class="BCRFList" cloudid="' +
                          fileObject[0].cloudId +
                          '"><span class="divider" style=" font-size: 8pt;padding-left: 5px;">&#9658;</span><a style="cursor:pointer;">' +
                          fileObject[0].objectName +
                          "</a></li>",
                      );
                    }
                    if ($.inArray(fileObject.objectName, fileName) > 0) {
  ...
  gotoInnerFolderandFiles: function (
    cid,
    fid,
    pgn,
    fileshareUrl,
      ...
      success: function (gotoInnerFolderandFiles) {
        ...
        if (test == "SHAREPOINT_ONLINE") {
          if (PageName == "SharepointSite")
            $("#CFUploadFiles").parents(".fm").addClass("buttonDisable");
          else $("#CFUploadFiles").parents(".fm").removeClass("buttonDisable");
        }
      },
      complete: function (xhr, statusText) {
  ...
  getRecentFilesandFolders: function (PageNumber) {
    var apiUrl =
      ...
      "/filefolder/user/" +
      CFManageCloudAccountsAjaxCall.getUserId() +
      "/cloud/all?page_size=50&fetchCollabInfo=" +
      urlParameterObject.fetchCollabInfo +
      "&isAscen=" +
      urlParameterObject.isAscen +
      "&orderField=" +
  ...
  createFolder: function (SingleCloudId, SinglePId, FolderName) {
    ...
    if (previousPage == "Share with Me") {
      sid = encodeURIComponent(
        $("#breadCrumbdync").children("li:eq(1)").attr("id"),
      );
    } else if (previousPage == "Share by Me") {
      sid = encodeURIComponent(
        $("#breadCrumbdync").children("li:eq(1)").attr("id"),
    ...
    $.ajax({
      ...
      success: function (CreateFolder) {
        ...
        var dateCreated = CFManageCloudAccountsAjaxCall.getDateConversion(
          CreateFolder.createdTime,
        );
        var dateModified = CFManageCloudAccountsAjaxCall.getDateConversion(
          CreateFolder.modifiedTime,
        );
        if (PageName == "InnerFolders" || PageName == "CloudDrive") {
          $("#ThumbnailContent").prepend(
            '<div class="file ' +
              fileIcon +
              '" id="' +
              cloudId +
              ...
              ' ><i title="' +
              objectName +
              '" class="filethumbnail" name="FOLDER"></i><strong class="filename" id="' +
              fileId +
              '" fexten="' +
              fileExten +
              '">' +
              CFManageCloudAccountsAjaxCall.getMaxChars(objectName, 14) +
              '</strong><div class="filesize">' +
              CFManageCloudAccountsAjaxCall.getObjectSize(
                objectSize,
                filetype,
              ) +
              '</div><input type="checkbox" class="fileCheck" /><a href="#" id="' +
              fileId +
              '"><!--i class="MetaDataIcon"></i--></a><i id="ThFav" class="' +
              favouriteicon +
              '" style="cursor:pointer;"></i></div>',
          );
          ...
          $("#LVContent").prepend(
            '<div class="panel-data" id="' +
              ...
              '><div  class="LVcheckBox" name="' +
              filetype +
              '"><input type="checkbox"/></div><div class="LVfileName" id="' +
              fileId +
              '" style="height:20px" name="' +
              filetype +
              '"><i class="LV' +
              filetype +
              ' pull-left"></i><p class="pull-left" name="' +
              objectName +
              '" fexten="' +
              fileExten +
              '">' +
              CFManageCloudAccountsAjaxCall.getMaxChars(objectName, 50) +
              '</p><a href="#" title=""><!--i class="MetaDataIcon"></i--></a></div><div class="LVFavorites"><a href="#" id="LVFavorite" class="' +
              favouriteicon +
              '"></a></div><div class="LVfileSize" style="cursor:pointer;">' +
              CFManageCloudAccountsAjaxCall.getObjectSize(
                objectSize,
                filetype,
              ) +
              '</div><div class="LVdrive" id="' +
              cloudId +
              '">' +
              CLName[cloudName] +
              '</div><div class="LVaddedDate">' +
              dateCreated +
              '</div><div class="LVmodifiedDate">' +
              dateModified +
              "</div></div>",
          ...
          $("#ThumbnailContent").prepend(
            '<div class="file ' +
              ...
              '><i title="' +
              objectName +
              '" class="filethumbnail" ></i><strong class="filename" id="' +
              fileId +
              '" fexten="' +
              fileExten +
              '">' +
      ...
      complete: function (xhr, statusText) {
        ...
        if (xhr.status == 406) {
          renameObject = null;
          if (
            text == "Can't be null or empty. Length must be 3 to 63 symbols.."
          ) {
            // $.smallBox({
          ...
          if (/Folder Unique Name: The Folder API Name/i.test(text)) {
            var _a =
              "Folder name must begin with a letter, not include spaces, not end with an underscore," +
  ...
  searchFiles: function (searchTerm, PageNumber) {
    var apiUrl;
    if (
      previousPage == "Folders" ||
      previousPage == "WorkSpace" ||
      previousPage == "InnerWorkSpace"
    ) {
      return false;
    } else {
      apiUrl =
        apicallurl +
        "/filefolder/search/user/" +
        CFManageCloudAccountsAjaxCall.getUserId() +
        "?page_size=50&isAscen=false&fetchCollabInfo=true&orderField=modifiedTime&page_nbr=" +
        PageNumber +
        "&query=" +
        searchTerm;
    }
        ...
        $.each(workspace, function (i, e) {
          if (e.user != null) {
            ...
            if (e.files != null) {
              ...
              noofFilesLabel =
                foldersCount > 1 || foldersCount == 0
                  ? foldersCount + " Folders, "
                  : foldersCount + " Folder, ";

              noofFilesLabel +=
                filesCount > 1 || filesCount == 0
                  ? filesCount + " Files "
                  : filesCount + " File ";
            }
            ...
            if (
              e.password == "" ||
              e.password == "NOT_REQUIRED" ||
              e.password == "IGNORE_UPDATE"
            ) {
            ...
            var _left =
              '<li class="getFilesForWorkspace" title="' +
              workspaceName +
              '">' +
              '<a href="#" id="' +
              workspaceId +
            ...
            var _thumb =
              '<div class="file ' +
              type +
              '" name="Workspace" style="cursor:pointer;position:relative">' +
              ...
              '<i title="' +
              workspaceName +
              '" class="filethumbnail cf-workspace wsIcon" name="Workspace"></i>' +
              '<strong class="filename" id="' +
              workspaceId +
              '" name="' +
              workspaceName +
              '" fexten="null">' +
              CFManageCloudAccountsAjaxCall.getMaxChars(workspaceName, 14) +
              "</strong>" +
              '<div class="filesize"> ' +
              noofFilesLabel +
              '</div><input type="checkbox" class="fileCheck" />' +
              '<p href="#" id="' +
              workspaceId +
              '"></p><i style="cursor:pointer;" id="wsLock" class="' +
              workspaceLock +
              '"></i>' +
              "</div>";
          ...
          bl1: {
            for (var i = 0; i < userData.length; i++) {
              if (userData[i].primaryEmail == email) {
                $("#shareENames").append(
                  "<i><span class='label label-default' style='float:left;'>" +
                    userData[i].userName +
                    "<i class='removeTag1'></i></span></i>",
                );
        ...
        if (userData != "") {
          bl1: {
            for (var i = 0; i < userData.length; i++) {
              ...
              var pcount = $(".addShareContent")
                .find("#addUsersToShare")
                .children("p").length;
              for (var j = 0; j < pcount; j++) {
                var currentRow = $(".addShareContent")
                  .find("#addUsersToShare")
                  .children("p:eq(" + j + ")");
      ...
      success: function (WSInfo) {
        ...
        $.each(Files, function (i, file) {
          ...
          $("#workspaceFiles").append(
            '<tr class="gradeA"><td class="wsfcheckbox" style="width:32px"><input type="checkbox" /></td><td class="' +
              fileClass +
  ...
  deleteFile: function (fileId) {
    ...
    $.ajax({
      ...
      complete: function (xhr, statusText) {
        if (xhr.status == 204) {
          unCheckFile();
          $('#LVContent .LVfileName[id="' + fileId + '"]')
            .parent(".panel-data")
            .remove();
          $('#LVContent .panel-data[id="' + fileId + '"]').each(function () {
            $(this).remove();
          });
          $('#ThumbnailContent strong[id="' + fileId + '"]')
            .parent(".file")
            .remove();
          ...
          var text =
            "The requested file might have deleted or moved from the Cloud . Please verify.";
      ...
      success: function (userData) {
        if (userData != "") {
          bl1: {
            for (var i = 0; i < userData.length; i++) {
              var email = userData[i].primaryEmail;
              var pcount = $(".addWorkspaceContent")
                .find("#addUsers")
                .children("p").length;
              for (var j = 0; j < pcount; j++) {
                var currentRow = $(".addWorkspaceContent")
                  .find("#addUsers")
                  .children("p:eq(" + j + ")");
                if (email == currentRow.children("span").text()) {
                  currentRow.remove();
    ...
    $.ajax({
      ...
      complete: function (xhr, statusText) {
        if (xhr.status == 403) {
          ...
          // });
          alertSuccess(
            "You have only view permission.You can't add to files this workspace.",
          );
        }
      },
    });
  ...
  sendEntLeadRequest: function (requestData) {
    var apiUrl =
      apicallurl +
      "/users/entLeadRequest?domianUrl=" +
      encodeURIComponent(domainUrl);
    $.ajax({
      type: "PUT",
      url: apiUrl,
      async: false,
      ...
      success: function (data) {
        ...
        if (testlength >= 1) {
          ...
          $("#categoryList").append(
            "<li id=" +
              data.id +
              ' class="getFiles"><a href="#">' +
              data.categoryName +
              '</a><i class="removeCategory" data-toggle="modal" data-target="#myModal2"></i><i class="editCategory"></i></li>',
          );
        }
      },
      complete: function (xhr, statusText) {
  ...
  updateUserName: function (Updatedusername) {
    ...
    var apiUrl =
      apicallurl + "/users/update/" + CFManageCloudAccountsAjaxCall.getUserId();
    $.ajax({
      type: "POST",
      url: apiUrl,
      headers: {
      ...
      success: function (json) {
        ...
        $("#homenavbarcollapse")
          .parent()
          .children(".navbar-right")
          .children()
          .children(".dropdown-toggle")
          .html("<span>" + json.lastName + '</span><b class="caret"></b>');
        $(".ProfileContent").html("");
        $(".ProfileContent").text(json.lastName);
        var isCusUser = checkIsaCustomUserForLogin(
          localStorage.getItem("UserId"),
        );
        ...
        } else $("#dash-uname").text(Updatedusername);
    ...
    $.ajax({
      ...
      success: function (json) {
        ...
        $("#homenavbarcollapse")
          ...
          .children()
          .children(".dropdown-toggle")
          .html("<span>" + json.mobileNumber + '</span><b class="caret"></b>');
  ...
  getCloudsFromConfig: function () {
    ...
    $.ajax({
      ...
      success: function (cloudConfig) {
        //cloud_webdav_header cloud_ecm_header cloud_ecm_message
        if (cloudConfig != undefined && cloudConfig != null) {
          if (
            cloudConfig.cloudsList != undefined &&
            cloudConfig.cloudsList.length > 0
          ) {
            var ecmClouds = [
              "CMIS",
              "SHAREFILE",
...
function downloadURL(url) {
  ...
  if (PageName != "Share with Me" && previousPage != "Share with Me") {
    ...
    $.get(url1 + "&isDownloading=false", function () {}).fail(function (data) {
      if (data.status == 404 || data.status == 406) {
        var test = "";
        if (PageName == "InnerWorkSpace") {
          test =
            "The requested file might have deleted or moved from the Cloud . Please verify and add to the workspace.";
        } else {
          test =
            "The requested file might have deleted or moved from the Cloud . Please verify.";
        }
...
$("#breadCrumbdyncmove").on("click", "li.BCRFList", function (e) {
  ...
  if (fileId == "cloud") {
    ...
    $.each(AllCloudsInfo, function (i, cloud) {
      ...
      if (cloud.userDisplayName != "" || cloud.userDisplayName != null) {
        dispanme = cloud.userDisplayName;
      } else {
        dispanme = email;
      }
      ...
      if (MultiUserClouds.indexOf(cloud.cloudName) == -1) {
        $(".span6 #moveSource").append(
          '<div class="moveCloudBlock button ' +
            _warning +
            '" style="width:150px;padding:0" type="button">' +
            '<div class="move ' +
            cloud.cloudName +
            '" cid="' +
            cloud.id +
    ...
    PageNumber = 1;
    if (fileId == "allcloud") {
      //if we click on the cloud name to get all files from a cloud
      PageName = "move";
    }
    CFManageCloudAccountsAjaxCall.getAllRootFoldersAndFiles(
      cloudId,
...
$("#breadCrumbdyncmovedest").on("click", "li.BCRFList", function (e) {
  ...
  if (navid == "cloud") {
    ...
    $.each(AllCloudsInfo, function (i, cloud) {
      ...
      $("#moveDestination .moveCloudBlock").droppable({
        accept: "#moveSource .list-group-item",
        drop: dropEventHandler,
        over: function () {
          $(this)
            .addClass("moveCloudBlockDragHover")
            .removeClass("moveCloudBlock");
        },
        out: function () {
          $(this)
            .removeClass("moveCloudBlockDragHover")
            .addClass("moveCloudBlock");
        },
      });
    });
    PageName = "moveLanding";
    $("#moveDestination")
      .siblings(".tab-header")
      .children("#dynamicDestCloudName")
      .html("");
    $("#moveDestination")
      .siblings(".tab-header")
      .children("#totalfilesdestination")
      .html("");
  ...
  if (
    ...
    if (
      ...
      (!(BrowserDetect.browser == "Safari") &&
        window.location.hash.includes("#backup"))
    ) {
...
$("#manageclouddiv").on("click", '[data-action="delete"]', function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      status = data;
      if (data > 0) {
        $("#CFCLdeleteModal h4").text(
          "Cloud you have selected to delete is associated with file migration. Do you want to delete it anyway?",
        );
      } else {
        if (
          parent.attr("name") === "DROPBOX_BUSINESS" ||
          parent.attr("name") === "ONEDRIVE_BUSINESS_ADMIN"
        ) {
          var s = parent.find("#percentOfUsers").text();
          var t = parent.find("#AuthorizeCld").hasClass("retry");
          if (s !== "100%" || t) {
            $("#CFCLdeleteModal h4").text(
              "The cloud you want delete is currently being processed. Do you wish to delete it anyway?",
            );
          } else
            $("#CFCLdeleteModal h4").text(
              "Are you sure you want to delete the selected Cloud?",
            );
        } else
          $("#CFCLdeleteModal h4").text(
            "Are you sure you want to delete the selected Cloud?",
...
$("#manageclouddiv").on("click", '[data-action="edit"]', function () {
  ...
  var _new =
    '<input type="text" style="width:73%" class="form-control slim-form-control" value="' +
    _name +
    '"><span' +
    ...
    if (_val.length < 1) {
      ...
      if (_r) {
        var _html =
          ...
          _val +
          '</span><i style="padding-right:10px;" class="cf-pencil9 pull-right"' +
          ' data-action="edit"></i>';
        $("body").off("mouseup");
        _parent.html(_html);
      } else {
  ...
  $("body").on("mouseup", function (e) {
    if (_input.is(e.target) || _input.next().find("i").is(e.target)) {
    } else {
...
function helperEvent(e, ui) {
  ...
  if (fid.length == 1) {
    ...
    helperText =
      ...
      "' cid='" +
      cid +
      "' style='z-index:99999;width:100px;padding:5px;background:#0062FF;color:#fff;font-size:10px;word-wrap:break-word'>" +
      fileName +
      "</" +
      "span>";
    ...
    helperText =
      ...
      "' cid='" +
      cid +
      "' style='z-index:99999;width:100px;padding:5px;background:#0062FF;color:#fff;font-size:10px;word-wrap:break-word'>Please select File </" +
      "span>";
  } else {
    helperText =
      "<span fid='" +
      ...
      "' cid='" +
      cid +
      "' style='z-index:99999;width:100px;padding:5px;background:#0062FF;color:#fff;font-size:10px;word-wrap:break-word'>Move " +
      fid.length +
      " item</" +
      "span>";
  }
...
function dropEventHandler(e, ui) {
  ...
  if ($("#moveSource").find("input:checked").length < 1) {
    ...
    $('#moveSource [id="' + fromfid + '"]')
      .children("input")
      .attr("checked", "false");
    $("#moveDestination")
      .find(".draghover")
      .addClass("dropabbleParent")
    ...
    $("#moveDestination")
      .children(".button.ui-droppable")
      .each(function () {
        $(this)
          .removeClass("fileActive")
          .removeClass("moveCloudBlockDragHover")
          .addClass("moveCloudBlock");
      });
    return false;
  ...
  if (PageName == "moveLanding") {
    ...
    $('#moveDestination .dropabbleParent[id="' + tocid + '"]')
      .children("input")
      .attr("checked", "true");
    $('#moveDestination .move[cid="' + tocid + '"]')
      .prev("input")
      .attr("checked", "true");
  ...
  if (PageName == "move" || PageName == "innermove" || PageName == "back") {
    ...
    $('#moveDestination .ui-droppable[id="' + tofid + '"]')
      .addClass("fileActive")
      .addClass("dropabbleParent");
  }
...
$(document).on("click", ".cancelMig", function () {
  ...
  $.ajax({
    ...
    url:
      apicallurl +
      "/move/cancelConsumerMigration/workspaceId?workSpaceId=" +
      _wid +
      "&status=CANCEL&userId=" +
      localStorage.UserId,
    async: true,
    headers: {
      "Content-Type": "application/json",
...
$(document).on("click", ".conSyncPause", function () {
  ...
  if (
    $(this)
      ...
      .siblings()
      .find("span")
      .hasClass("CONSUMERTRAIL_COMPLETED") ||
    $(this).parent().siblings().hasClass("CONSUMERTRAIL_COMPLETED")
  ) {
    $(this).attr("src", "../img/Pause.png");
    _val = "trial_Resume";
  }
    ...
    success: function (data) {
      if (data.jobStatus === "IN_PROGRESS") {
        ...
        parent
          ...
          .children("td:eq(3)")
          .find(".conSyncPause")
          .attr("src", "../img/Resume.png");
        parent
          .parents("tr")
          .children("td:eq(1)")
          .html("Consumer Trail Completed")
          .css("color", "green")
          .css("font-weight", "bold");
        parent
          .parents("tr")
          .children("td:eq(1)")
          .addClass("CONSUMERTRAIL_COMPLETED");
...
$(document).on(
  "click",
  "#manageclouddiv .getCloudsInfo .lnil-chevron-down-circle",
  function () {
    sendGAEvents("Clicked on plus icon to show cloud information");
    if ($("#showSummery").hasClass("clearAppend")) {
      $(".appendcloudDetails").html("").css({ display: "none", height: "" });
      // $("#manageInfoHr").css("display", "none");
      // alert("HasClass");
      $("#showSummery").removeClass("clearAppend");
    }
    var $parentTr = $(this).closest(".getCloudsInfo");
    $("#manageclouddiv .appendcloudDetails").html("").css("display", "none");
    $("#manageclouddiv .getCloudsInfo")
      .find(".fa-chevron-circle-up")
      .removeClass("fa fa-chevron-circle-up")
      .addClass("lnil lnil-chevron-down-circle")
      .css("font-size", "20px");
    $(".cloudsinfo").css("height", "80px");
    $("#manageclouddiv #manageInfoHr").css("display", "none");
    $(this)
      .removeClass("lnil lnil-chevron-down-circle")
...
$(document).on(
  "click",
  "#manageclouddiv .getCloudsInfo .fa-chevron-circle-up",
  function () {
    if ($("#showSummery").hasClass("clearAppend")) {
      $(".appendcloudDetails").html("").css({ display: "none", height: "" });
      // $("#manageInfoHr").css("display", "none");
...
$(document).on(
  "click",
  "#manageclouddiv .appendcloudDetails .innerPlus .failedUsers",
  function () {
    ...
    $("#manageclouddiv .appendcloudDetails .appendUsersList")
      .html("")
      .css("display", "none");
    $("#manageclouddiv .appendcloudDetails .innerPlus")
      .find(".failedUsersminus")
      .removeClass("failedUsersminus")
      .addClass("failedUsers");
...
$(document).on(
  "click",
  "#manageclouddiv .appendcloudDetails .innerPlus .failedUsersminus",
  function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.reauthorization == true) {
        ...
        var _parent = $("#manageclouddiv #" + cldId)
          .children("." + cldId)
          .children("#incrementProgress");
        ...
        _parent
          .find("#usersDisplay")
          .text(usersCount + " out of " + data.totolClouds + " Users");
      } else {
        parent.css("display", "none");
        parent.next().css("display", "").addClass("retry");
        setTimeout(function () {
...
var tableAnalysisData =
  '<div class="container-fluid"> <div id="toggleDataInModel" style=""> <div class="row" style="margin-top: 30px"> <div class="col-8 col-sm-6"> <span style=" float: left; margin-left: 65px; width: 95px; background-color: #6673ff; text-align: center; color: #fff; border-radius: 30px; font-weight: 500; margin-top: -11px; position: absolute; z-index: 1; " > <h5 style="font-size: 14px; margin: 4px 0px">SUMMARY</h5> </span> <table style=" justify-content: center; text-align: center; /* width: 90%; */ border-radius: 5px; width: 100%; font-size: 12px; margin-bottom: 5%; box-shadow: 0px 0px 10px #0000001f; padding: 5%; border-radius: 12px; border-color: #0062ff; /* margin-top: 25%; */ padding: 5px; color: #8f627e; transform: translate(2.5%); /* background-color: #f6f5f8; */ " > <thead style=""> <tr> <th style="text-align: center"> <i class="fa fa-user fa-user" style=" font-size: 24px; padding: 10px; padding-top: 25px; color: #ccd0ff; " ></i> <div style="">Active Users</div> </th> <th style="text-align: center"> <i class="fa fa-folder fa-folder" style=" font-size: 24px; padding: 10px; padding-top: 25px; color: #ccd0ff; " ;="" ></i> <div style="">Total Folders</div> </th> <th style="text-align: center"> <i class="fa fa-file" style=" font-size: 24px; padding: 10px; padding-top: 25px; color: #ccd0ff; " ></i> <div style="">Total Files</div> </th> <th style="text-align: center"> <i class="fa fa-database" style=" font-size: 24px; padding: 10px; padding-top: 25px; color: #ccd0ff; " ></i> <div style="">Used Space</div> </th> </tr> </thead> <tbody> <tr style="font-size: 24px; font-weight: bolder; color: #0062ff"> <td style="padding-bottom: 5px" id="activeUsers">5</td> <td style="padding-bottom: 5px" id="totalFoldersPreMIg">20</td> <td style="padding-bottom: 5px" id="totalFilesPreMIg">500</td> <td style="padding-bottom: 5px" id="totalUsedSpacePreMIg">100 TB</td> </tr> </tbody> </table> </div> <div class="col-4 col-sm-6"> <table style=" justify-content: center; text-align: center; width: 97%; height: 129px; border-radius: 5px; font-size: 12px; margin-bottom: 5%; box-shadow: 0px 0px 10px #0000001f; /* padding: 5%; */ border-radius: 12px; border-color: #0062ff; /* margin-top: 25%; */ padding: 5px; color: #8f627e; transform: translate( 2.5% ); /* background-color: #f6f5f8; */ /* margin-left: 2.2%; */ " > <tbody> <tr style="visibility: hidden"> <td style="font-size: 15px; text-align: left; padding-left: 60px"> Maximum file count <span style="float: right">:</span> </td> <td style=" font-weight: bold; font-size: 15px; color: #0062ff; text-align: left; padding-left: 20px; " > ben@syncorbit.com [2323] </td> </tr> <tr> <td style="font-size: 15px; text-align: left; padding-left: 32px"> Maximum file count <span style="float: right">:</span> </td> <td style=" font-weight: 700; font-size: 14px; color: #0062ff; text-align: left; padding-left: 10px; " id="maxfilesPresentUser"> ben@syncorbit.com [2323] </td> </tr> <tr style="/* margin-top: 20px; */ padding: 10px"> <td style="font-size: 15px; text-align: left; padding-left: 34px"> Maximum Used space<span style="float: right">:</span> </td> <td style=" font-weight: 700; font-size: 14px; color: #0062ff; text-align: left; padding-left: 10px; " id="maxSpaceUsedUser"> ben@syncorbit.com [100GB] </td> </tr> <tr style="visibility: hidden"> <td style="font-size: 15px; text-align: left; padding-left: 60px"> Maximum Used space<span style="float: right">:</span> </td> <td style=" font-weight: bold; font-size: 16px; color: #0062ff; text-align: left; padding-left: 20px; " > ben@syncorbit.com [100GB] </td> </tr> </tbody> </table> </div> <div class="" style="width: 105%"> <div class=""> <div class=""> <div class="tableFixHeadPreMigrate" onscroll="tablePagination(this)"> <table id="preMigrationAnalysisTable"> <thead> <tr> <th scope="col" style="text-align: center">S.No</th> <th scope="col" style="text-align: center"> Email Id <span style="margin-bottom: 3px" ><input type="email" placeholder="Email Id" style=" color: #777; font-weight: 400; width: 184px; height: 26px; padding: 1px 2px;opacity:0.4;pointer-events:none " id="searchEmail" oninput="findByEmail(this)" /><i class="fa fa-search" style="margin-left: -30px; color: black" ></i ></span> </th> <th scope="col" style="text-align: center"> Total Folders </th> <th scope="col" style="text-align: center"> Total Files<!--<i class="fa fa-sort" type="button" style=" margin-top: 0%; font-size: 18px; padding-left: 3px; " ></i>--></th> <!-- <th scope="col" style="text-align: center">Used Space<i class="fa fa-sort" type="button" style="margin-top: 0%;font-size: 18px;padding-left: 3px;"></i></th> --> <!-- <th scope="col" id="AVB" style="text-align: center">User Data Space</th> --><th style="display:none">usedSpace</th><th style="display:none">totalSpace</th> <th scope="col" id="TotalSpace" style=" text-align: center;" > User Data Space</th><th>Status <i class="lnil lnil-download" onclick="getQuickScanCSV()" style="margin-left: 5%; width: 15%; font-size: 16px; font-weight: bold;cursor:pointer;opacity:0.5;pointer-events:none;" id="downloadQSCSV" title="Please Wait We Are Preparing Your Report" ></i></th> </tr> </thead> <tbody id="nan"></tbody> </table> <nav aria-label="..." style=" /* align-items: center; */ /* justify-content: center; */ margin-left: 36%; " > <ul class="pagination"> <li class="page-item disabled"> <a class="page-link" href="#" tabindex="-1">Previous</a> </li> <li class="page-item"><a class="page-link" href="#">1</a></li> <li class="page-item active"> <a class="page-link" href="#" >2 <span class="sr-only">(current)</span></a > </li> <li class="page-item"><a class="page-link" href="#">3</a></li> <li class="page-item"> <a class="page-link" href="#">Next</a> </li> </ul> </nav> </div> </div> </div> </div> </div> </div> </div>';

function showFailedUeserTable(m) {
  console.log($(m).attr("cid"));

  $(m).addClass("clearAppendFailedDiv");
  // CFManageCloudAccountsAjaxCall.failedUserDetails($(m).attr("cid"));
  CldId = $(m).attr("cid");
  for (var i = 0; i < CloudName.length; i++) {
    if (CldId === AllCloudsInfo[i].id) {
      var cldName = AllCloudsInfo[i].cloudName;
      $("#manageclouddiv #" + CldId)
        ...
        .html("");
      var html =
        '<span onclick="CloseAppendTable()" style="color:red;cursor:pointer;left: 93%;position: relative;">Close</span><td colspan="4"><div class="table-responsive box" style="background: white;margin: 2%;width: 96%;box-shadow: none!important;"><table class="table table-condensed" style="font-size: 14px;border: 1px solid lightgray;"><thead style="background: #0062FF;color: white;border-radius:4px;"><tr><th style="font-weight: 500;width: 33%;padding: .5rem;padding-left:2%;font-size:12px;">Username</th><th style="font-weight: 500;width: 27%;padding: .5rem;font-size:12px;">Description</th><th style="font-weight: 500;width: 40%;padding: .5rem;font-size:12px;">Solution</th></tr></thead><tbody class="innerList"></tbody></table></div><div class="pagination p1" style="padding: 0;margin: 0;margin-left: 32%;border-radius: 0;width: 100%;text-align: center;"> <ul class="paginationReport" class="pagination-sm"></ul> </div></td>';
      CFManageCloudAccountsAjaxCall.getAllCloudDetails($(m).attr("cid"));
      $("#manageInfoHr" + $(m).attr("cid")).addClass("hideHr");
      $("#manageInfoHr" + $(m).attr("cid")).css({
        width: "96%",
        "margin-top": "8%",
      });
      $("#manageclouddiv #" + $(m).attr("cid"))
        .next()
        .next(".appendcloudDetails")
        .html(html);
      // $("#manageclouddiv #" + CldId)
...
function CloseAppendTable() {
  console.log("CLICK-");
  $(".hideHr").css({ width: "0%", "margin-top": "6%" });
  $(".appendcloudDetails").html("").css({ display: "none" });
}

function showPremigDivAlna(e) {
  console.log($(e).attr("uid"));
  if ($(e).hasClass("clearAppend")) {
    $(".appendcloudDetails").html("").css({ display: "none" });
    $(e)
      .children("a")
      .css({ color: "#1220f6", "text-decoration": "underline" })
      .html(
        '<i class="lnil lnil-cloud-search" title="Quick Scan"" style="font-size:25px;color:#1220F6;font-weight:bolder;"></i>',
      );
    $("#manageInfoHr" + $(e).attr("uid")).css({
      width: "0%",
      "margin-top": "6%",
    });
    $(e).removeClass("clearAppend");
  } else {
    checkBTN = $(".clearAppend").children("a").text();
    if (checkBTN === "Close") {
      $("#manageInfoHr" + $(".clearAppend").attr("uid")).css({
        width: "0%",
        "margin-top": "6%",
      });
      $(".clearAppend")
        .removeClass("clearAppend")
        .children("a")
        .css({ color: "#1220f6", "text-decoration": "underline" })
        .html(
          '<i class="lnil lnil-cloud-search" title="Quick Scan"" style="font-size:25px;color:#1220F6;font-weight:bolder;"></i>',
    ...
    $(e).addClass("clearAppend");
    $(".appendcloudDetails").css("display", "none");

    CFManageCloudAccountsAjaxCall.getAllCloudDetails($(e).attr("uid"));

    var userId = $(e).attr("cuid");
    var cloudId = $(e).attr("uid");
    var pageNo = 1;
    var pageSize = 5;

    // console.log(userId, cloudId);
    // $("#CFShowLoading").fadeToggle("slow");
    // $("#loaderText").html("Scanning In Progress...");
    $(e).children("a").css("color", "red").html("Scanning In Progress");
    $(e)
      .children("a")
      .css("color", "red")
      .html("<img src='https://staging.cloudfuze.com/img/ajax-loader11.gif'>");
    alertSuccess("Scanning In Progress...");
    $("#nan").html("");
    $('[id="showSummery"]').css({'opacity':'0.5','pointer-events':'none'});
    $.ajax({
      type: "GET",
      url:
        apicallurl +
        "/premigration/activeUsers/userId/" +
        userId +
        "/cloudId/" +
        cloudId +
        "?page_nbr=" +
      ...
      success: function (data) {
        $(e).children("a").css("color", "red").html("Close");
        $("#manageInfoHr" + $(e).attr("uid")).css({
          width: "96%",
          "margin-top": "8%",
        });
        // $("#CFShowLoading").fadeToggle("slow");
        $("#manageclouddiv #" + $(e).attr("uid"))
          .next()
          .next(".appendcloudDetails")
          .html(tableAnalysisData);
        console.log(data);
        console.log(data.length);
        var calc = 1024 * 1024 * 1024 * 1024;
        if (data[0].nextPageToken === null || data[0].nextPageToken === "") {
          $(".tableFixHeadPreMigrate")
            .find(".pagination")
            .css("visibility", "hidden");
        } else {
          $(".tableFixHeadPreMigrate").find(".pagination").css("display", "");
        }
        $("#activeUsers").html(data.length);
        $("#totalFoldersPreMIg").html("");
        $("#totalFilesPreMIg").html("");
        $("#totalUsedSpacePreMIg").html("");
        $("#maxfilesPresentUser").html("");
        $("#maxSpaceUsedUser").html("");
        var usedSpaceForQuickScan = 0;
        var TotalFolderCountForQuickScan = 0;
        var TotalFileCountForQuickScan = 0;
        for (let i = 0; i < data.length; i++) {
          var _data = data[i];
          var emailId = _data.emailId;
          var folderCount = _data.folderCount;
          TotalFolderCountForQuickScan =
            TotalFolderCountForQuickScan + folderCount;
          $("#totalFoldersPreMIg").html(TotalFolderCountForQuickScan);
          var fileCount = _data.fileCount;
          TotalFileCountForQuickScan = TotalFileCountForQuickScan + fileCount;
          $("#totalFilesPreMIg").html(TotalFileCountForQuickScan);
          if (_data.fileFolderPickingCountStatus !== "COMPLETED") {
            folderCount = 0;
            fileCount = 0;
            var status = "In-Progress";
          } else {
            folderCount = _data.folderCount;
            fileCount = _data.fileCount;
            var status = "Completed";
          }
          var totalSpaceInMB = _data.totalSpace;
          var userdSpaceInMB = _data.usedSpace
          var totalSpace = _data.totalSpace / calc;
          var usedSpace = _data.usedSpace / calc;
          var usedSpaceWithOutFix = _data.usedSpace / calc;
          usedSpaceForQuickScan = usedSpaceForQuickScan + usedSpace;
          $("#totalUsedSpacePreMIg").html(
            usedSpaceForQuickScan.toFixed(2) + " TB",
          );
          var totalSpace = totalSpace.toFixed(2);
          var usedSpace = usedSpace.toFixed(2);
          var sno = i + 1;
          $("#nan").append(
            '<tr> <td style="padding: 12px 16px">' +
              sno +
              '</td> <td style="padding: 12px 16px; width: 25%">' +
              emailId +
              '</td> <td style="padding: 12px 16px" class="findFolderStatus" memberid="' +
              _data.memberId +
              '" status="' +
              _data.fileFolderPickingCountStatus +
              '" id="folder' +
              _data.memberId +
              '">' +
              folderCount +
              '</td> <td style="padding: 12px 16px" class="findFileStatus" emailId="' +
              emailId +
              '" id="file' +
              _data.memberId +
              '">' +
              fileCount +
              '</td><td style="display:none;">'+usedSpace+'TB'+
              '</td><td style="display:none;">'+totalSpace+'TB'+
              '</td> <td style="width: 25%"> <progress class="procressValue" emailId="' +
              emailId +
              '" usedspace="' +
              usedSpaceWithOutFix +
        ...
        $('[id="showSummery"]').css({'opacity':'1','pointer-events':'auto'});
        if(data.length === pageSize){
  console.log('Auto');
  paginationQuickScan(userId, cloudId, pageNo);
  }
        getSortQuickScan();
        getFileFolderCount(pageNo, userId, cloudId);
        if (data.length === 0 || data.length !== 5) {
            $('[id="searchEmail"]').css({"opacity":"1","pointer-events":"auto"});
            $('[id="downloadQSCSV"]').css({"opacity":"1","pointer-events":"auto"});
	    $('[id="downloadQSCSV"]').attr('title','Download Reports');
          $('[class="tableFixHeadPreMigrate"]').attr("canScroll", "no");
        }
        $('[class="tableFixHeadPreMigrate"]').attr("userId", userId);
        $('[class="tableFixHeadPreMigrate"]').attr("cloudId", cloudId);
        $('[class="tableFixHeadPreMigrate"]').attr("pageNo", parseInt(pageNo));
        getSortQuickScan();
      },
...
$('[id="showSummery"]').css({'opacity':'1','pointer-events':'auto'});
        ...
        // $("#CFShowLoading").fadeToggle("slow");
        //This Feature Is Not Available For This Cloud At This Moment
        alertError("Data Not Available At This Moment");
...
function tablePagination(e) {
  if ($(e).attr("canScroll") === "no") {
    return null;
  } else {
    if ($(e)[0].scrollHeight - $(e).scrollTop() === $(e)[0].clientHeight) {
      var uid = $(e).attr("userid");
      var cid = $(e).attr("cloudid");
      var pgno = $(e).attr("pageno");
      paginationQuickScan(uid, cid, pgno);
    }
  }
}

function paginationQuickScan(userId, cloudId, pagnoNo) {
  var pgNo = parseInt(pagnoNo) + 1;
  console.log(pgNo)
  var pageSize = 5;
  $('[class="tableFixHeadPreMigrate"]').attr("pageNo", pgNo);
  $.ajax({
    type: "GET",
    url:
      apicallurl +
    ...
    success: function (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        var totalSpace = totalSpace.toFixed(2);
        var usedSpace = usedSpace.toFixed(2);
        var sno = parseInt($("#nan tr").length) + 1;
        $("#nan").append(
          '<tr> <td style="padding: 12px 16px">' +
            sno +
            '</td> <td style="padding: 12px 16px; width: 25%">' +
        ...
        $("#activeUsers").html(parseInt($("#nan tr").length));
      }
 if(data.length === pageSize){
       autoPaginate(userId, cloudId, pagnoNo);
      }
      getFileFolderCount(pgNo, userId, cloudId);
      if (data.length === 0) {
        $('[class="tableFixHeadPreMigrate"]').attr("canScroll", "no");
      }
      $('[class="tableFixHeadPreMigrate"]').attr("userId", userId);
...
function autoPaginate(userId, cloudId, pageNo){
    var pageNo = parseInt(pageNo)+1; 
    paginationQuickScan(userId, cloudId, pageNo);
    console.log(userId, cloudId, pageNo);
}
var count = 0;
function getFileFolderCount(pgNo, userId, cloudId) {
  setInterval(function () {
    // console.log(pgNo, userId, cloudId);
    var pageSize = 5;
    var startCount = 0;
    for (let i = 0; i <= $("#tbPreMigAnal tr").length; i++) {
      var fill = "findFolderStatus:eq(" + i + ")";
      if ($("." + fill).attr("status") === "STARTED") {
        startCount = startCount + 1;
        var memberId = $("." + fill).attr("memberid");
      } else {
        console.log("File/Folder Picking Completed");
      }
    }
    ...
    if (startCount !== 0) {
      $.ajax({
        ...
        url:
          apicallurl +
          "/premigration/getActiveUsersFileFolderCount/userId/" +
          userId +
          "/cloudId/" +
          cloudId +
          "?page_nbr=" +
        ...
        success: function (data) {
          for (i = 0; i < data.length; i++) {
            $("#file" + data[i].userId).html(data[i].totalFiles);
            $("#folder" + data[i].userId).html(data[i].totalFolder);
            $("#folder" + data[i].userId).attr(
              "status",
              data[i].filefolderpickingcountStatus,
            );
            if (data[i].filefolderpickingcountStatus === "STARTED") {
              var status = "In-Progress";
            } else {
              var status = "Completed";
            }
            $("#status" + data[i].userId).html(status);
          }
          getSortQuickScan();
        },
      });
...
function searchEmailInQuickScan() {}

function getSortQuickScan() {
  var totalFolders = 0;
  var totalFiles = 0;
  ...
  var MaxUsedSpace;
  var calc = 1024 * 1024 * 1024 * 1024;
  for (let i = 0; i < $("#nan tr").length; i++) {
    var folder = "findFolderStatus:eq(" + i + ")";
    var file = "findFileStatus:eq(" + i + ")";
    var usedSpace = "procressValue:eq(" + i + ")";
    totalFolders = totalFolders + parseInt($("." + folder).text());
    totalFiles = totalFiles + parseInt($("." + file).text());
    totalUsedSpace = totalUsedSpace + $("." + usedSpace).val();
    if (!MaxFileCounts) {
      MaxFileCounts = 0;
    }
    if (parseInt($("." + file).text()) >= MaxFileCounts) {
      MaxFileCounts = parseInt($("." + file).text());
      $("#maxfilesPresentUser").html(
        $("." + file).attr("emailid") + "[" + MaxFileCounts + "]",
      );
    }
    ...
    if ($("." + usedSpace).attr("usedspace") >= MaxUsedSpace) {
      MaxUsedSpace = $("." + usedSpace).attr("usedspace");
      var fixedSpace = Number(MaxUsedSpace).toFixed(2);
      console.log(fixedSpace);
      $("#maxSpaceUsedUser").html(
        $("." + usedSpace).attr("emailid") + "[" + fixedSpace + " TB]",
      );
    }
  }
  $("#totalFoldersPreMIg").html(totalFolders);
  $("#totalFilesPreMIg").html(totalFiles);
  $("#totalUsedSpacePreMIg").html(CFManageCloudAccountsAjaxCall.getObjectSize(totalUsedSpace));
}
function findByEmail(e) {
  var input, filter, table, tr, td, i, txtValue;
  email = $(e).val();
  filter = e.value.toUpperCase();
  table = document.getElementById("nan");
...
function getQuickScanCSV(){
console.log('CSVDOWNLOAD');
var csv_data = [];
var headForCsv  = ['S.No','Email Id','Total Folders','Total Files','Used Space','Total Space','Scan Status'];
var csvrow = [];
for(var m = 0; m< headForCsv.length;m++){
    csvrow.push(headForCsv[m])
}
    csv_data.push(csvrow.join(","));
            var tablebody = document.getElementById("nan");
			...
			downloadQuickScanCSVFile(csv_data);
}
function downloadQuickScanCSVFile(csv_data) {
			CSVFile = new Blob([csv_data], {
				type: "text/csv"
			});
			var temp_link = document.createElement('a');
			temp_link.download = "QuickScan_Report.csv";
