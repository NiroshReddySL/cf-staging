...
function csvPreScanMappingsUpload(csvData) {
  $.ajax({
    type: "POST",
    url: `${apicallurl}/mapping/user/hyperlink/csv?sourceCloudId=${localStorage.lnKxSourceCldId}&destCloudId=${localStorage.lnKxDstnCldId}&pageNo=1&pageSize=500`,
    async: true,
    data: csvData,
    headers: {
      "Content-Type": "application/json",
...
function linkExCsvAutoDownload() {
  $.ajax({
    type: "GET",
    url: `${apicallurl}/mapping/download/hyperlinkcsvcreator/${
...
function getCSVLinkExValidatedPairs(pageNo) {
  ...
  $.ajax({
    type: "GET",
    url: `${apicallurl}/mapping/user/latest/hyperlinkmappingcache/${
      localStorage.lnKxCSVId
    }?sourceAdminCloudId=${localStorage.lnKxSourceCldId}&destAdminCloudId=${
      localStorage.lnKxDstnCldId
