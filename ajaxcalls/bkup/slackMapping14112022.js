...
var pubCount = 0,pvtCount=0;
		...
		if ($("#selectAllChannels:checked").length) {
                ...
                var _html =
                    ...
                    '</span><i class="lnil lnil-pencil" id="destTeamEdit" channelobject="'+JSON.stringify(data[i])+'" style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>' +
                    '<td style="width: 18%;"><span id="destChannelNameSpan" title="' +
                    dstnChannelName +
                    '">' +
                    CFManageCloudAccountsAjaxCall.getMaxChars(dstnChannelName, 14) +
                    '</span><i class="lnil lnil-pencil" id="destChannelEdit" channelobject="'+JSON.stringify(data[i])+'" style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>' +
                    "</tr>";
                } else {
                var _html =
                    '<tr fromRootId="' +
...
break;
	...
	//url = apicallurl + "/filefolder/userId/"+localStorage.UserId+"/cloudId/"+cloudId+"?&page_size=50&page_nbr="+pageNo;
	url = apicallurl +"/messagemove/get/slack/channel?adminCloudId="+cloudId +"&pageNo="+pageNo+"&pageSize=50&channelType="+fetchType;
	 $.ajax({
        type: "GET", 
        url: url,
		async: true,
	...
	if(data[i].destTeamName){
		...
		if($("#selectAllChannels:checked").length){
        var _input = '<input type="checkbox" name="channelSelect" id="'+data[i].fromRootId+'" cid="'+data[i].cloudId+'" workspacename="'+workspaceName+'" channeltype="'+data[i].types+'" channelname="'+data[i].channelName+'" destchannelname="'+destChannelName+'" destteamname="'+destTeamName+'" subchannel="false" channeldate="'+data[i].channelDate+'" checked>';
		}
		else{
		var _input = '<input type="checkbox" name="channelSelect" id="'+data[i].fromRootId+'" cid="'+data[i].cloudId+'" workspacename="'+workspaceName+'" channeltype="'+data[i].types+'" channelname="'+data[i].channelName+'" destchannelname="'+destChannelName+'" destteamname="'+destTeamName+'" subchannel="false" channeldate="'+data[i].channelDate+'">'; 
		}  
    ...
    var _html = '<tr fromRootId="'+data[i].id+'"><td style="width: 1%;">'+_input+'</td><td style="width:18%;" title="'+data[i].channelName+'" id="public"><i class="fa fa-hashtag" style="color:#000;font-size: 10px !important;font-weight:100 !important;"></i>&nbsp;'+CFManageCloudAccountsAjaxCall.getMaxChars(data[i].channelName, 16)+'</td>'+
	 ...
	 //'<td style="width: 16%;"><select id="JobType_dropDown" style=""><option>One Time</option><option>Delta</option></select></td>'+
	 '<td style="width: 16%;"><select id="msgType_dropDown" style=""><option>Team</option><option>Sub-Channel</option></select></td>'+
	 '<td style="width: 19%;"><span id="destTeamNameSpan" title="'+destTeamName+'">'+CFManageCloudAccountsAjaxCall.getMaxChars(destTeamName, 14)+'</span><i class="lnil lnil-pencil" id="destTeamEdit" channelobject='+encodeURIComponent(JSON.stringify(data[i]))+' style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>'+
	 '<td style="width: 18%;"><span id="destChannelNameSpan" title="'+destChannelName+'">'+CFManageCloudAccountsAjaxCall.getMaxChars(destChannelName, 14)+'</span><i class="lnil lnil-pencil" id="destChannelEdit" channelobject='+encodeURIComponent(JSON.stringify(data[i]))+' style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>'+
	 '</tr>';
		}else{
			    var _html = '<tr fromRootId="'+data[i].id+'"><td style="width: 1%;">'+_input+'</td><td style="width:18%;" title="'+data[i].channelName+'" id="private"><i class="fa fa-lock" style="color:#000;font-size: 10px !important;font-weight:100 !important;"></i>&nbsp;'+CFManageCloudAccountsAjaxCall.getMaxChars(data[i].channelName, 16)+'</td>'+
	 //'<td style="width: 16%;display:none;"><span title="'+createdBy+'" id="createdByUsers">'+CFManageCloudAccountsAjaxCall.getMaxChars(createdBy, 14)+'</span></td>'+
	 //'<td style="width: 16%;"><select id="JobType_dropDown" style=""><option>One Time</option><option>Delta</option></select></td>'+
	 '<td style="width: 16%;"><select id="msgType_dropDown" style=""><option>Team</option><option>Sub-Channel</option></select></td>'+
	 '<td style="width: 19%;"><span id="destTeamNameSpan" title="'+destTeamName+'">'+CFManageCloudAccountsAjaxCall.getMaxChars(destTeamName, 14)+'</span><i class="lnil lnil-pencil" id="destTeamEdit" channelobject='+encodeURIComponent(JSON.stringify(data[i]))+' style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>'+
	 ...
	 '</tr>';
		...
		$("#pubDiv").text("Public : "); 
		$("#pvtDiv").text("Private : "); 
		$("#publicCntNewCount").text(Number($("#publicCntNewCount").text())+pubCount); 
		...
		if(data?.length < 50 && fetchType === "public"){
...
//        $("#displayFooterText").css('display','flex');
	...
	$("#usrMapping").css('margin-top','-3%');
        ...
        $("#pubDiv").text("Matched : ");
        $("#pvtDiv").text("Unmatched : ");
        $("#unMatchedNewCount").text(pvtCount);       
        $("#selDiv").css("display", "none");
        $("#selCount").text("00");
        $("#matchedNewCount").text(pubCount)
      }
      setTimeout(function () {
        $("#CFShowLoading").hide();
      }, 1000);
      $("#usrMapping").css('margin-top','-2%');
...
$(document).on('change','#selectAllChannels',function () {
 if($("#selectAllChannels:checked").length){
		...
		$("#newChannelsSelCount").text($("input[name=channelSelect]:visible").length);
		$("#forNextMove").removeClass('disabled');
 }
 else{
...
$(document).on('change','input[name=channelSelect]',function () {
	...
	$("#selDivNewChannelsCount").css("display","");
 	$("#newChannelsSelCount").text($("input[name=channelSelect]:checked").length);
...
$.each($('[id="JobType_dropDown"]').find('option:selected'),function(){
  ...
  if ($("#channelsTab").hasClass("active")) {
	...
	if($(this).attr('channeltype') === "private"){
          var obj =  {
       	 "fromCloudId": {
	      "id": $(this).attr('cid')
	    ...
	    if(data[i]?.deltaMigration){
	     ...
	     jobType="One-Time"	
            ...
            var _html =
              ...
              '" channeldate="' +
              data[i].channelDate +
              '"><td style="width:5px;">'+selectMultiDelta+'</td><td style="display: flex;" class="chnlName"><svg xmlns="http://www.w3.org/2000/svg"  style="margin-right: 2%;" class="icon" width="20" height="20" viewBox="0 0 1024 1024"><path fill="#4A144B" d="M880 112H144c-17.7 0-32 14.3-32 32v736c0 17.7 14.3 32 32 32h736c17.7 0 32-14.3 32-32V144c0-17.7-14.3-32-32-32zM529 311.4c0-27.8 22.5-50.4 50.3-50.4 27.8 0 50.3 22.6 50.3 50.4v134.4c0 27.8-22.5 50.4-50.3 50.4-27.8 0-50.3-22.6-50.3-50.4V311.4zM361.5 580.2c0 27.8-22.5 50.4-50.3 50.4-13.3 0-26.1-5.3-35.6-14.8-9.4-9.5-14.7-22.3-14.7-35.6 0-27.8 22.5-50.4 50.3-50.4h50.3v50.4zm134 134.4c0 27.8-22.5 50.4-50.3 50.4-27.8 0-50.3-22.6-50.3-50.4V580.2c0-27.8 22.5-50.4 50.3-50.4 13.3 0 26.1 5.3 35.6 14.8s14.7 22.3 14.7 35.6v134.4zm-50.2-218.4h-134c-27.8 0-50.3-22.6-50.3-50.4 0-27.8 22.5-50.4 50.3-50.4h134c27.8 0 50.3 22.6 50.3 50.4-.1 27.9-22.6 50.4-50.3 50.4zm0-134.4c-13.3 0-26.1-5.3-35.6-14.8S395 324.8 395 311.4c0-27.8 22.5-50.4 50.3-50.4 27.8 0 50.3 22.6 50.3 50.4v50.4h-50.3zm134 403.2c-27.8 0-50.3-22.6-50.3-50.4v-50.4h50.3c27.8 0 50.3 22.6 50.3 50.4 0 27.8-22.5 50.4-50.3 50.4zm134-134.4h-134c-13.3 0-26.1-5.3-35.6-14.8-9.4-9.5-14.7-22.3-14.7-35.6 0-27.8 22.5-50.4 50.3-50.4h134c27.8 0 50.3 22.6 50.3 50.4 0 27.8-22.5 50.4-50.3 50.4zm0-134.4H663v-50.4c0-27.8 22.5-50.4 50.3-50.4s50.3 22.6 50.3 50.4c0 27.8-22.5 50.4-50.3 50.4z"/></svg>' +
              data[i].channelName +
              "</td><td class='jobType'>"+jobType+"</td><td>" +
              data[i].workSpaceName +
              "</td><td class='chnlName'>" +
              data[i].channelType +
              "</td><td id="+processStat+">" +
              prStat[processStat] +
              '</td><td class="dltaBTN" style="display:none;">'+deltaBtn+'</td><td style="text-align:center;"><i class="lnil lnil-download" id="slackChannelReport" style="color:#0062ff;font-size: 16px;font-weight: bold;"></i></td></tr>';
            $("#channelsTable table tbody").append(_html);
          } else if (
...
for(var i=0;i<AllCloudsInfo.length;i++){
    ...
    break;
    	 ...
    	 url = apicallurl +"/messagemove/get/slackdms?adminCloudId="+cloudId +"&pageNo="+pgno+"&pageSize=50&channelType=private";
 ...
 if((data[i].channelType !== 'public') && (key[data[i].cloudId] === data[i].creatorEmailId)){
	privateChannelsFetchedList.push(data[i]);
	 ...
	 //'<td style="width: 16%;"><select id="JobType_dropDown" style=""><option>One Time</option><option>Delta</option></select></td>'+
	 '<td style="width: 16%;"><select id="msgType_dropDown" style=""><option>Team</option><option>Sub-Channel</option></select></td>'+
	 '<td style="width: 19%;"><span id="destTeamNameSpan" title="'+destTeamName+'">'+CFManageCloudAccountsAjaxCall.getMaxChars(destTeamName, 14)+'</span><i class="lnil lnil-pencil" id="destTeamEdit" style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>'+
	 '<td style="width: 18%;"><span id="destChannelNameSpan" title="'+destChannelName+'">'+CFManageCloudAccountsAjaxCall.getMaxChars(destChannelName, 14)+'</span><i class="lnil lnil-pencil" id="destChannelEdit" style="font-weight: 600;font-size: 15px;float: right;margin-right: 15%;"></i></td>'+
	 '</tr>';
            $("#channelsTable table tbody").append(_html);  
            if(data[i].channelType == "public"){
                pubCount++;
...
$("#syncChannels").click(function(){
 ...
 $.ajax({
	type: "GET",
	url: apicallurl+ "/messagemove/sync/slack/channel?adminCloudId="+localStorage.multiUsrSrcCldId,
	headers: {
      		"Content-Type": "application/json",
      		Authorization: CFManageCloudAccountsAjaxCall.getAuthDetails(),
      		"Access-Control-Allow-Methods": "GET, POST, PUT, DELETE,OPTIONS",
