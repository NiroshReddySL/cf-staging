    ...
    $.ajax({
      ...
      success: function (data) {
        if (data != null) {
          ...
          loginEmailUser(loginAjaxCall.BasicAuth(data.id, password));
        ...
        if (data != "") {
          if (checksum == "sendPwdLink") {
            if (data[0].enabled === true) {
              ...
              $("#login-manager i#resend-link").text(
...
function signUpEmail(userObject) {
  $.ajax({
    type: "POST",
    ...
    headers: {
      "Content-Type": "application/json",
      Authorization: loginAjaxCall.BasicAuth(
        userObject.id,
        userObject.password
      ),
    },
    success: function (res) {
      console.log("Email Migration Signup successfull");
    },
  });
}

function loginEmailUser(auth) {
  $.ajax({
    type: "POST",
