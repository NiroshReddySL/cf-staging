...
$("#mapdUsers").on("change","input[name=inputMapdUrs]",function () {
    ...
    if ($('#mapdUsers input[name= "csvMapngCheckBox"]').length>0) {
		$("#clearSelected").css("pointer-events","auto").css("opacity","1");
    ...
    if (checkedLength == totalLength) {
		$("#clearSelected").css("pointer-events","auto").css("opacity","1");
		$("#clearSelected").text("Clear all");
        $('#chkInput').prop('checked', true);
    } else {
		$("#clearSelected").css("pointer-events","none").css("opacity","0.4");
		$("#clearSelected").text("Clear all");
...
$("#forNextMove").click(function () {
   ...
   // $("#preview").css("display","none");
	...
	$('#notBoxtoSpoh').modal('hide');	
		...
		if(_srcCld === "DROPBOX_BUSINESS" && _dstnCld === "SHAREPOINT_ONLINE_BUSINESS"){
            $("#sortngSrc").css("display","none");
            $("#sortngDstn").css("display","none");
			$("#srcImg").attr("src","../img/PNG/"+_srcCld+".png");
            $("#dstnImg").attr("src","../img/PNG/"+_dstnCld+".png");
			$("#srcname").text(_cldDtls.srcAdminName);
            $("#dstnname").text(_cldDtls.dstAdminName);
			$("#srcImg").css("display","");
            $("#dstnImg").css("display","");
        }
        else{
            $("#srcImg").css("display","none");
            $("#dstnImg").css("display","none");
            $("#sortngSrc").css("display","");
            $("#sortngDstn").css("display","");
        } 		
    ...
    $.ajax({
        ...
        success: function (data) {
			...
			var _parentName = _dataVal.documentLibrary;  
                    mapdSlectedUrs(_srcUsrDetails, _dstnUsrDetails, csvID,_teamFldr, csvName, migrateDstnFolderName, undefined, isFolder,'',_dataVal.duplicateCache,_dataVal.pathException,_parentName,validationStatus);
...
/*$(document).on('click','#srcUsrs .forDomainNameMin .fa-plus',function() {
    ...
    $("#srcUsrs .forDomainNameMin").find('span.down').removeClass('down').addClass('right');
    $("#srcUsrs .forDomainName").css("display","block");
});
$(document).on('click','#dstnUsrs .forDomainNameMin .fa-plus',function() {
    $("#dstnUsrs .forDomainNameMin").css("display","none");
    $("#dstnUsrs .forDomainNameMin").find('.fldr_parent1').css("display","none");
    $("#dstnUsrs .forDomainNameMin").find('span.down').removeClass('down').addClass('right');
...
$(document).on('click','#srcUsrs .forDomainNameMin>.fa-minus',function() {
    ...
    $("#srcUsrs .forDomainNameMin").find('.usr-folder').removeClass('fa-minus').addClass('fa-plus');
    $("#srcUsrs .forDomainNameMin").css("display","none");
    $("#srcUsrs .forDomainNameMin").find('.fldr_parent1').css("display","none");
    $("#srcUsrs .forDomainNameMin").find('span.down').removeClass('down').addClass('right');
    $("#srcUsrs .forDomainName").css("display","block");
...
$(document).on('click','#dstnUsrs .forDomainNameMin>.fa-minus',function() {
    ...
    $("#dstnUsrs .forDomainNameMin").find('.usr-folder').removeClass('fa-minus').addClass('fa-plus');
    $("#dstnUsrs .forDomainNameMin").css("display","none");
    $("#dstnUsrs .forDomainNameMin").find('.fldr_parent1').css("display","none");
    $("#dstnUsrs .forDomainNameMin").find('span.down').removeClass('down').addClass('right');
    $("#dstnUsrs .forDomainName").css("display","block");
...
function mapdSlectedUrs(src,dstn,csvId,_teamFldr,csvName,migrateDstnFolderName,append,isFolder,automap,duplicate,pathException,parentName,validationStatus) {
    ...
    var _input,input;
    if ((src.userCloudName === "ONEDRIVE_BUSINESS_ADMIN" && dstn.dstnUserCloudName === "DROPBOX_BUSINESS") || (src.userCloudName === "DROPBOX_BUSINESS" && dstn.dstnUserCloudName === "ONEDRIVE_BUSINESS_ADMIN")|| (src.userCloudName === "GOOGLE_SHARED_DRIVES" && dstn.dstnUserCloudName === "SHAREPOINT_ONLINE_BUSINESS")|| (src.userCloudName === "DROPBOX_BUSINESS" && dstn.dstnUserCloudName === "GOOGLE_SHARED_DRIVES")|| (src.userCloudName === "DROPBOX_BUSINESS" && dstn.dstnUserCloudName === "SHAREPOINT_ONLINE_BUSINESS")) {
        if(src.folderPath === null || dstn.dstnFolderPath === null || src.folderPath === undefined || dstn.dstnFolderPath === undefined || src.folderPath === "undefined" || dstn.dstnFolderPath === "undefined"){
	...
	if(src.userCloudName === "DROPBOX_BUSINESS" && dstn.dstnUserCloudName === "SHAREPOINT_ONLINE_BUSINESS"){
					$("#mapdUsers tbody tr td:eq(3) span").remove();
				if(validationStatus == false){
				var _append = '<img title="Valid Mapping" alt="pic" src="../img/PNG/greenTick.png" style="height: 11px;width: 11px;vertical-align: -webkit-baseline-middle;">';
				}
				else{
				var _append = '<img title="Invalid Mapping" alt="pic" src="../img/PNG/cross.png" style="height: 15px;width: 15px;vertical-align: -webkit-baseline-middle;">';				
				}
	
				$("#mapdUsers tbody tr td:eq(3)").append(_append);
	}
...
function autoSaving(_srcMapdCldId,_dstnMapdCldId,isFolder,parentName) {
 ...
 $(document).find("#chkInput").prop( "checked", false );
	...
	if(localStorage.getItem('multiUsrSrcCldName')==="GOOGLE_SHARED_DRIVES" && localStorage.getItem('multiUsrDstnCldName') ==="SHAREPOINT_ONLINE_BUSINESS" && (_dstnMapdCldId.dstnUserRtFolId.split(':')[1] === "SITE")){
       ...
       mapdSlectedUrs(_srcMapdCldId, _dstnMapdCldId, '','', '', '', undefined, isFolder, '', _autoSave[0].duplicateCache,_autoSave[0].pathException,parentName,validationStatus);
                }
            }
        }
    });
        ...
        $.ajax({
            ...
            success: function (domainData) {
                ...
                for (var j = 0; j < _usrsLength; j++) {
                    if((_srcCldName === "ONEDRIVE_BUSINESS_ADMIN" && _dstnCldName === "DROPBOX_BUSINESS") || (_srcCldName === "DROPBOX_BUSINESS" && _dstnCldName === "ONEDRIVE_BUSINESS_ADMIN")||(_srcCldName === "GOOGLE_SHARED_DRIVES" && _dstnCldName === "SHAREPOINT_ONLINE_BUSINESS")||(_srcCldName === "DROPBOX_BUSINESS" && _dstnCldName === "SHAREPOINT_ONLINE_BUSINESS")|| (_srcCldName === "DROPBOX_BUSINESS" && _dstnCldName === "GOOGLE_SHARED_DRIVES")){
                        if (target == "source"){
                            if (_srcCldName === "ONEDRIVE_BUSINESS_ADMIN" && domainData.cloudDetail[j].rootFolderId === "/") {
                                _usrAppend = '<div class="fldr_parent" pgno="1" style="border-bottom: 1' +
                                    'px solid #f2f2f2;padding: 5px 10px" title="User authentication failed"> <p class="usr-data" style="padding: 0px 0px 10px 35px;margin-bottom:0;"><i class="fa fa-plus usr-folder11" folder-id="root" cloudname="cloudName" user-id="cldId" useremail="emailId" aria-hidden="true" style="color: #ddd;cursor:pointer;font-size: 24px!important;padding-bottom: 2%;margin-left: -13%;"></i><div style="margin-left: 31px;margin-top: -13%;"><input type="checkbox"  name="srccheckBox"  class="sourceChkBox" folderPath="/" usremail="emailId" cloudId="cldId" rtfolId="root" cldName="cloudName" disabled></div><div style="margin-left: 19%;margin-top: -10%;"><i class="lnil lnil-user-alt" aria-hidden="true" style="font-size: large;margin-right: -3px;cursor:default;pointer-events: none;margin-left:10px;"></i><span id="textSpan1" style="font-size:initial;" title="'+domainData.cloudDetail[j].emailId+'">&nbsp;&nbsp;userName</span>' +
							...
							if (_srcCldName === "DROPBOX_BUSINESS" && _dstnCldName === "SHAREPOINT_ONLINE_BUSINESS"){
							 _usrAppend = '<div class="fldr_parent" pgno="1" style="border-bottom: 1' +
                                    'px solid #f2f2f2;padding: 5px 10px"> <p class="usr-data" style="padding: 0px 0px 10px 35px;margin-bottom:0;"><i class="fa fa-plus usr-folder" folder-id="root" cloudname="cloudName" user-id="cldId" useremail="emailId" aria-hidden="true" style="cursor:pointer;font-size: 24px!important;padding-bottom: 2%;margin-left: 95%;"></i><div style="margin-top: -10%;">'+input+'</div><div style="margin-left: 5%;margin-top: -10%;"><i class="lnil lnil-user-alt" aria-hidden="true" style="font-size: large;margin-right: -3px;cursor:default;pointer-events: none;margin-left:10px;"></i><span id="textSpan1" style="font-size:initial;margin-left: 3%;" title="'+domainData.cloudDetail[j].emailId+'">&nbsp;&nbsp;userName</span>' +
                                    '</div></p><span id="folderLoading" style="position: absolute;left: 10%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" >Loading...</span></div>';
						
						}
						else{
                                _usrAppend = '<div class="fldr_parent" pgno="1" style="border-bottom: 1' +
                                    'px solid #f2f2f2;padding: 5px 10px"> <p class="usr-data" style="padding: 0px 0px 10px 35px;margin-bottom:0;"><i class="fa fa-plus usr-folder" folder-id="root" cloudname="cloudName" user-id="cldId" useremail="emailId" aria-hidden="true" style="cursor:pointer;font-size: 24px!important;padding-bottom: 2%;margin-left: -13%;"></i><div style="margin-left: 31px;margin-top: -13%;">'+input+'</div><div style="margin-left: 19%;margin-top: -10%;"><i class="lnil lnil-user-alt" aria-hidden="true" style="font-size: large;margin-right: -3px;cursor:default;pointer-events: none;margin-left:10px;"></i><span id="textSpan1" style="font-size:initial;" title="'+domainData.cloudDetail[j].emailId+'">&nbsp;&nbsp;userName</span>' +
                                    '</div></p><span id="folderLoading" style="position: absolute;left: 10%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" >Loading...</span></div>';
						}
                            }
                        }
			...
			else if(_srcCldName === "DROPBOX_BUSINESS" && _dstnCldName === "SHAREPOINT_ONLINE_BUSINESS") {
			 _usrAppend = '<div class="fldr_parent" pgno="1" style="border-bottom: 1' +
                                    'px solid #f2f2f2;padding: 5px 0px"> <p class="usr-data" style="padding: 0px 0px 10px 35px;margin-bottom:0;"><i class="fa fa-plus usr-folder" folder-id="root" cloudname="cloudName" user-id="cldId" useremail="emailId" aria-hidden="true" style="cursor:pointer;font-size: 24px!important;padding-bottom: 2%;margin-left: 90%;"></i><div style="margin-top: -13%;"><input type="radio" name="folderradiobtn" folderPath="/" usremail="emailId" cloudId="cldId" rtfolId="root" cldName="cloudName" style="display:none;" disabled="disabled"></div><div style="margin-top: -7%;"><i class="lnil lnil-user-alt" aria-hidden="true" style="font-size: large;margin-right: 1%;cursor:default;pointer-events: none;margin-left:10px;"></i><span id="textSpan1" style="font-size:initial;" title="'+domainData.cloudDetail[j].emailId+'">&nbsp;&nbsp;userName</span>' +
                                    '</div></p><span id="folderLoading" style="position: absolute;left: 10%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" >Loading...</span></div>';
			}
...
if(_srcCldName === "DROPBOX_BUSINESS" && _dstnCldName === "SHAREPOINT_ONLINE_BUSINESS"){
	$(".src-usrs").find(".paginationDiv").css("display","none");
	$(".dest-usrs").find(".paginationDiv").css("display","none");
	$("#mapdUsrs").find(".paginationDiv").css("display","none");
	$("#mapdUsrs").find("#mapngTblDiv").css("display","");
	$(".pull-right").css("display","none");
	$("#clearBtn").css("display","none");
}
else{
	$(".src-usrs").find(".paginationDiv").css("display","");
	$(".dest-usrs").find(".paginationDiv").css("display","");
	$("#mapdUsrs").find(".paginationDiv").css("display","");
	$("#mapdUsrs").find("#mapngTblDiv").css("display","none");
	$(".pull-right").css("display","");
                    ...
                    if (appendTarget == "source") {
                        if (_srcCldName === "ONEDRIVE_BUSINESS_ADMIN" && domainData.cloudDetail[j].rootFolderId === "/") {
                            _usrAppend1 = '<div class="fldr_parent" pgno="1" style="border-bottom: 1' +
							 ...
							 _usrAppend1 = '<div class="fldr_parent" pgno="1" style="border-bottom: 1' +
									'px solid #f2f2f2;padding: 5px 10px"> <p class="usr-data" style="padding: 0px 0px 10px 35px;margin-bottom:0;"><i class="fa fa-plus usr-folder" folder-id="root" cloudname="cloudName" user-id="cldId" useremail="emailId" aria-hidden="true" style="cursor:pointer;font-size: 24px!important;padding-bottom: 2%;margin-left: 95%;"></i><div style="margin-top: -10%;"><input type="checkbox"  name="srccheckBox"  class="sourceChkBox" folderPath="/" usremail="emailId" cloudId="cldId" rtfolId="root" cldName="cloudName"></div><div style="margin-left: 5%;margin-top: -10%;"><i class="lnil lnil-user-alt" aria-hidden="true" style="font-size: large;margin-right: -3px;cursor:default;pointer-events: none;margin-left:10px;"></i><span id="textSpan1" style="font-size:initial;margin-left: 3%;" title="'+domainData.cloudDetail[j].emailId+'">&nbsp;&nbsp;userName</span>\n' +
									'</div></p> <span id="folderLoading" style="position: absolute;left: 10%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" >Loading...</span></div>';
                        ...
                        if (_dstnCldName === "ONEDRIVE_BUSINESS_ADMIN" && domainData.cloudDetail[j].rootFolderId === "/") {
                            ...
                            _usrAppend1 = '<div class="fldr_parent" pgno="1" style="border-bottom: 1' +
                                'px solid #f2f2f2;padding: 5px 10px"> <p class="usr-data" style="padding: 0px 0px 10px 35px;margin-bottom:0;"><i class="fa fa-plus usr-folder" folder-id="root" cloudname="cloudName" user-id="cldId" useremail="emailId" aria-hidden="true" style="cursor:pointer;font-size: 24px!important;padding-bottom: 2%;margin-left: 95%;"></i><div style="margin-left: 31px;margin-top: -10%;"><input type="radio"  class="dstnRadioBtn" name="folderradiobtn" disabled="disabled" folderPath="/" usremail="emailId" cloudId="cldId" rtfolId="root" cldName="cloudName" style="display:none;"></div><div style="margin-left: 5%;margin-top: -10%;"><i class="lnil lnil-user-alt" aria-hidden="true" style="font-size: large;margin-right: -3px;cursor:default;pointer-events: none;margin-left:10px;"></i><span id="textSpan1" style="font-size:initial;" title="'+domainData.cloudDetail[j].emailId+'">&nbsp;&nbsp;userName</span>' +
                                '</div></p><span id="folderLoading" style="position: absolute;left: 10%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" >Loading...</span> </div>';
...
$(document).on('click','#dstnFolderScroll .forDomainNameMin .fa-plus.usr-folder',function(){
    ...
    $('#dstnFolderScroll .fldr_parent').find('.usr-folder').removeClass('fa-minus').addClass('fa-plus');
...
$(document).on('click','#dstnFolderScroll .forDomainNameMin .fa-minus.usr-folder',function() {
    ...
    $(this).parents('.fldr_parent').find('.down').removeClass('down').addClass('right');
    $(this).parents('.fldr_parent').find('.right').css('color','gray');
    $(this).removeClass('fa-minus').addClass('fa-plus');
    $(this).parents('.fldr_parent').siblings().show();
    return false;
});
$(document).on('click','#srcFolderScroll .forDomainNameMin .fa-plus.usr-folder',function(){
    ...
    $('#srcFolderScroll .fldr_parent').find('.usr-folder').removeClass('fa-minus').addClass('fa-plus');
...
$(document).on('click','#srcFolderScroll .forDomainNameMin .fa-minus.usr-folder',function() {
    $(this).parent('.usr-data').parent().siblings().siblings().siblings().find('span#folderLoading').css("display","none");
    $(this).parents('.fldr_parent').css('padding-bottom','0%');
    $(this).parents('.fldr_parent').find('.fldr_parent1').remove();
    $(this).parents('.fldr_parent').removeClass('active');
...
$('#mapdUsers').on('scroll', function() {
            $("#mapdUsers .fa-chevron-right:last").trigger('click');
			$("#CFShowLoading").modal('hide');			
			return false;
...
$('#srcFolderScroll,#dstnFolderScroll').on('scroll', function() {
		 if ($(this).attr('id') === "srcFolderScroll") {
			if($("#srcUsrs .forDomainNameMin").css('display') == "none"){
            $("#srcUsrs .fa-chevron-right:last").trigger('click');
			$("#CFShowLoading").modal('hide');			
			return false;
        	} 
		 }else{
		if($("#dstnUsrs .forDomainNameMin").css('display') == "none"){
            $("#dstnUsrs .fa-chevron-right:last").trigger('click');
			$("#CFShowLoading").modal('hide');
			return false;
        	}
		 }
...
$(document).on('click','.forDomainNameMin .fldr_parent1 .dropatag',function(){
	var email =$(this).attr('useremail');
   if($(this).text() == "Home"){
	   $('.forDomainNameMin .fldr_parent1 #changeIcon').trigger('click');
   }
   else{
	   		$(this).nextAll().remove();
	var len = $(this).siblings().length;
	var _this = $('.forDomainNameMin .fldr_parent1 .usr-folder1.down:eq('+len+')');
	 _this = _this.closest('.fldr_parent1');
	 if(_this.parent('.usr-data').children('.usr-folder1').children('img').attr('src') == "../images/icons/home.png"){
	 
	 	_this.parent('.usr-data').children('.usr-folder1').css('margin-left','0%');
		   if($(this).parents('.forDomainNameMin').parents('.col-md-3').hasClass('dest-usrs') == true){
			_this.parent('.usr-data').children('div').children('input').css('display','none');
			_this.parent('.usr-data').children('.usr-folder1').css('display','');
		   }
		   else{
			_this.parent('.usr-data').children('div').children('input').css('display','');  
_this.parent('.usr-data').children('.usr-folder1').css('display','inline-flex');
_this.parent('.usr-data').children('.usr-folder1').css('margin-left','6%');			
		   }
	 }
	 else{
	 _this.parent('.usr-data').children('.usr-folder1').css('display','inline-flex');
	 _this.parent('.usr-data').children('div').children('input').css('display','');
 	_this.parent('.usr-data').children('.usr-folder1').css('margin-left','5%');
	 }
    _this.children('.usr-data').find('.fldr_parent1').remove();
	_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').css('display','inline-flex'); 
	_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('div').children('input').css('display','inline-flex');
	_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').children('img').attr('src','../images/icons/folder.png').css('display','');
	_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').children('.dropdown').css('display','none');
	_this.parent('.usr-data').children('.usr-folder1').children('.dropdown').children('.dropdown-content').children('a:eq('+(len-1)+')').nextAll().remove();
	var _len = _this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').children('.dropdown').length;
	for(var i=0;i<_len;i++){ 
		_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').children('.dropdown').children('.dropdown-content:eq('+i+')').children('a:eq('+(len-1)+')').nextAll().remove();
	}
	_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').css('display','');
	_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').children('img').css('display','');  
    _this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').children('.dropdown').css('display','none');
	_this.parent('.usr-data').find('.fldr_parent1').children('.usr-data').children('.usr-folder1').removeClass('down').addClass('right');	
	$(this).remove();
    return false;
   }
});
$(document).on('click','.forDomainNameMin .fldr_parent1 .usr-folder1.right',function(){ 
    ...
    var folderPath = $(this).parents('.usr-data').children('div:eq(0)').children('input').attr('folderPath'); 
    ...
    /*   if(!$(this).parent('.usr-data').hasClass('haschild')) {
	   ...
	   var atitle = $(this).parents('.active').siblings('div.usr-folder1').children('#textSpan').attr('title');
	   var userId = $(this).parents('.active').siblings('div.usr-folder1').attr('user-id');
var toCloudId = $(this).parents('.active').siblings('div.usr-folder1').attr('tocloudid');
var folderId = $(this).parents('.active').siblings('div.usr-folder1').attr('folder-id');
var parentId = $(this).parents('.active').siblings('div.usr-folder1').attr('parentid');
var cldNam = $(this).parents('.active').siblings('div.usr-folder1').attr('name');	  

var fPath = $(this).parents('.active').siblings('div:eq(0)').children('input').attr('folderPath')
var pPath = $(this).parents('.active').siblings('div:eq(0)').children('input').attr('parentPath')
	    var atag = '<a href="#" useremail="'+email+'" user-id="'+userId+'" folder-id="'+folderId+'" parentid="'+parentId+'"class="dropatag" name="'+cldNam+'" tocloudid="'+toCloudId+'" folderPath="'+fPath+'" parentPath="'+pPath+'">'+atitle+'</a>'
	   if($(this).parents('.forDomainNameMin').parents('.col-md-3').hasClass('src-usrs') == true){
		$("#srcUsrs .fldr_parent1 .dropdown-content").append(atag); 
		var dropcont = $("#srcUsrs .fldr_parent1 .dropdown-content");		
	   }
	   else{
	   $("#dstnUsrs .fldr_parent1 .dropdown-content").append(atag);
	   var dropcont = $("#dstnUsrs .fldr_parent1 .dropdown-content");
	   }
    subDataList1(this, pgno,email,folderPath,dropcont);
    // $(this).parent().siblings().hide();
    // $(this).closest('.fldr_parent1').siblings('.fldr_parent1').hide();
    $(this).parent().find('> .fldr_parent1').remove();

	$(this).parents('div.usr-data').siblings().css('display','none');
...
$(document).on('click','.forDomainNameMin .fldr_parent1 .usr-folder1.down',function() {
	if(localStorage.multiUsrSrcCldName == "DROPBOX_BUSINESS" && localStorage.multiUsrDstnCldName == "SHAREPOINT_ONLINE_BUSINESS"){
		return false;
	}
...
$(document).on('click','.forDomainNameMin .fldr_parent1 #changeIcon',function() {
	var _this = $(this).parent('div');
	_this.css('display','flex');
	_this.css('margin-left','12%');
_this.siblings('div').children('input').css('display','');	
    _this.closest('.fldr_parent1').find('.clickeddiv').siblings('#folderLoading').css('display','none');
    _this.parent('.usr-data').find('.fldr_parent1').remove();
    _this.parent('.usr-data').siblings('.usr-data:eq(7)').addClass('scrollActive');
    _this.css('margin-bottom', '0');
    _this.closest('.fldr_parent1').find('.usr-data > .fa').css('margin-bottom', '0');
    _this.closest('.fldr_parent1').find('.opened').removeClass('opened');
    _this.closest('.fldr_parent1').find('.active').removeClass('active');
    _this.closest('.fldr_parent1').find('.openeddiv').removeClass('openeddiv');
    _this.closest('.fldr_parent1').find('.clickeddiv').removeClass('clickeddiv');
    _this.closest('.fldr_parent1').removeClass('active').removeClass('opened');
    _this.parents('div').eq(1).find('> .usr-data > .openeddiv').removeClass('openeddiv').addClass('clickeddiv');
    _this.parents('div').eq(1).removeClass('opened').addClass('active');
    _this.removeClass('clickeddiv').removeClass('openeddiv');
    _this.parent('.usr-data').find('.down').removeClass('down').addClass('right');
    _this.removeClass('down').addClass('right');
   // _this.css('color','gray');
    _this.parent('.usr-data').siblings().show();
    _this.closest('.fldr_parent1').siblings('.fldr_parent1').show();
	if(_this.attr('name') == "SHAREPOINT_ONLINE_BUSINESS" && _this.attr('parentid') == "null"){
	_this.children('img').attr('src','../img/PNG/SHAREPOINT_ONLINE_BUSINESS.png');	
	}
	else{
	_this.children('img').attr('src','../images/icons/folder.png');
	}
	_this.children('#hideGt').css('display','none');
    return false;
});
...
$('#sampleCsv').click(function() {
  
  var _data = "Source Cloud,Source Path,Destination Cloud,Destination Path\n"+
	"erik@filefuze.co,/Alex Folder 3.2 Folder,daniel@cloudfuze.co,/CloudFuze LLC Team Site";
...
$('#exportCsv').click(function() {
	...
	var trLen = $("#mapdUsers tbody tr").length;
	for (var i=0; i<trLen; i++){
		array1.push($("#mapdUsers tbody tr:eq("+i+") td:eq(1)").children('span').attr('title'));
	}
	for (var j=0; j<trLen; j++){
		array2.push($("#mapdUsers tbody tr:eq("+j+") td:eq(1)").children('p').children('span').attr('title'));
	}
	for (var k=0; k<trLen; k++){
		array3.push($("#mapdUsers tbody tr:eq("+k+") td:eq(2)").children('span').attr('title'));
	}
	for (var l=0; l<trLen; l++){
		array4.push($("#mapdUsers tbody tr:eq("+l+") td:eq(2)").children('p').children('span').attr('title'));
	}
	for (var m=0; m<trLen; m++){
		var obj={
			"Source Cloud":array1[m],
			"Source Path":array2[m],
			"Destination Cloud":array3[m],
			"Destination Path":array4[m]
		};
		array5.push(obj);
	}
	var keys = Object.keys(array5[0]);
var csv = [keys.join(',')];
array5.forEach(function (data) {
  var row = [];
...
$("#dstCloudsUsers input[cloudid=" + data[i].destCloudDetails.id + "]").attr('disabled', false).parent().removeClass("selectedClass").parent().removeClass("selectedClass");
        ...
        var dest2 = desti.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
        var src2 = sourc.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
		if(data[i].sourceCloudDetails.name === "DROPBOX_BUSINESS" && data[i].destCloudDetails.name === "SHAREPOINT_ONLINE_BUSINESS"){
		var _html = '<tr style="width: 100%;display:table;" id="csvRow"  teamFolder="' + data[i].teamFolder +'" csvid="' + data[i].csvId + '" srcemail="' + data[i].sourceCloudDetails.emailId + '" dstnemail="' + data[i].destCloudDetails.emailId + '" sourcAdminCloudId="' + data[i].sourceAdminCloudId + '" destAdminCloudId="' + data[i].destAdminCloudId + '" csvname="' + csvName + '" srccldid="' + data[i].sourceCloudId + '"  srcrt="' + data[i].sourceCloudDetails.rootFolderId + '" dstncldid="' + data[i].destCloudId + '" dstnrt="' + data[i].destCloudDetails.rootFolderId + '" class="CsvMapd" srcPathRootFolderId="' + data[i].sourceCloudDetails.pathRootFolderId + '" srcFolderPath="' + src2 + '"  srcRootFolderId="' + data[i].sourceCloudDetails.rootFolderId + '"  migrateSrcFolderName="' + data[i].sourceCloudDetails.migrateFolderName + '" srcCldName="' + data[i].sourceCloudDetails.name + '"dstnCldName="' + data[i].destCloudDetails.name + '" dstnPathRootFolderId="' + data[i].destCloudDetails.pathRootFolderId + '" dstnRootFolderId="' + data[i].destCloudDetails.rootFolderId + '" dstnFolderPath="' + dest2 + '" migrateDstnFolderName="' + data[i].migrateFolderName + '">' + '<td><input type="checkbox" name="csvMapngCheckBox" style="margin-top:70%;"></td><td style="width: 45%"><img src="../img/PNG/' + data[i].sourceCloudDetails.name + '.png" alt="pic" class="forImage" style="margin-top: 2%;float: left;"><span class="manualPath" title="EmailSrcTitle" style="">' + emailMaxChar(data[i].sourceCloudDetails.emailId, 25) + '</span><br/><img src="../images/icons/folder.png" style="height: 20px;width: 20px;"><span class="csvPath"  style="margin-left: 2%;" title="'+src2+'" data-placement="bottom">' + src1 + '</span></td><td style="width: 48%"><img src="../img/PNG/' + data[i].destCloudDetails.name + '.png" alt="pic" class="forImage" style="margin-top: 2%;float: left;"><span class="manualPath" title="EmailDstnTitle">' + emailMaxChar(data[i].destCloudDetails.emailId, 25) + '</span><br/><img src="../images/icons/folder.png" style="height: 20px;width: 20px;"><span class="csvPath" style="margin-left: 2%;" title="'+dest2+'">' + dest1 + '</span></td><td></td></tr>';
...
UpdateCsvPath = function(pgNo,pgSize){
  ...
  if((localStorage.getItem('multiUsrSrcCldName') ==="DROPBOX_BUSINESS"&&localStorage.getItem('multiUsrDstnCldName')==="ONEDRIVE_BUSINESS_ADMIN")|| (localStorage.getItem('multiUsrSrcCldName') ==="ONEDRIVE_BUSINESS_ADMIN"&&localStorage.getItem('multiUsrDstnCldName')==="DROPBOX_BUSINESS")|| (localStorage.getItem('multiUsrSrcCldName') ==="GOOGLE_SHARED_DRIVES"&&localStorage.getItem('multiUsrDstnCldName')==="SHAREPOINT_ONLINE_BUSINESS")|| (localStorage.getItem('multiUsrSrcCldName') ==="DROPBOX_BUSINESS"&&localStorage.getItem('multiUsrDstnCldName')==="GOOGLE_SHARED_DRIVES")|| (localStorage.getItem('multiUsrSrcCldName') ==="DROPBOX_BUSINESS"&&localStorage.getItem('multiUsrDstnCldName')==="SHAREPOINT_ONLINE_BUSINESS"))
				...
				if(data[i].sourceCloudDetails.name === "DROPBOX_BUSINESS" && data[i].destCloudDetails.name==='SHAREPOINT_ONLINE_BUSINESS'){
					if (CsvOperation(_obj, 'check')) {
                            input = '<td style="width: 5%;"><input type="checkbox" name="csvMapngCheckBox" checked="true" style="margin-top:70%;"></td>';
                        } else {
                            input = '<td style="width: 5%;"><input type="checkbox" name="csvMapngCheckBox" style="margin-top:70%;"></td>';
                        }
				}
				else{
                        if (CsvOperation(_obj, 'check')) {
                    ...
                    if(data[i].destCloudDetails.folderPath === "" && data[i].destCloudDetails.name==='SHAREPOINT_ONLINE_BUSINESS'){
                        ...
                        var src2 = sourc.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
						if(data[i].sourceCloudDetails.name === "DROPBOX_BUSINESS" && data[i].destCloudDetails.name === "SHAREPOINT_ONLINE_BUSINESS"){
							if(data[i].validationStatus == true)
						var _html = '<tr style="width: 100%" id="csvRow"  teamFolder="' + data[i].teamFolder +'" csvid="' + data[i].csvId + '" srcemail="' + data[i].sourceCloudDetails.emailId + '" dstnemail="' + data[i].destCloudDetails.emailId + '" sourcAdminCloudId="' + data[i].sourceAdminCloudId + '" destAdminCloudId="' + data[i].destAdminCloudId + '" csvname="' + csvName + '" srccldid="' + data[i].sourceCloudId + '"  srcrt="' + data[i].sourceCloudDetails.rootFolderId + '" dstncldid="' + data[i].destCloudId + '" dstnrt="' + data[i].destCloudDetails.rootFolderId + '" class="CsvMapd" srcPathRootFolderId="' + data[i].sourceCloudDetails.pathRootFolderId + '" srcFolderPath="' + src2 + '"  srcRootFolderId="' + data[i].sourceCloudDetails.rootFolderId + '"  migrateSrcFolderName="' + data[i].sourceCloudDetails.migrateFolderName + '" srcCldName="' + data[i].sourceCloudDetails.name + '"dstnCldName="' + data[i].destCloudDetails.name + '" dstnPathRootFolderId="' + data[i].destCloudDetails.pathRootFolderId + '" dstnRootFolderId="' + data[i].destCloudDetails.rootFolderId + '" dstnFolderPath="' + dest2 + '" migrateDstnFolderName="' + data[i].migrateFolderName + '">'+input+'<td style="width: 45%"><img src="../img/PNG/' + data[i].sourceCloudDetails.name + '.png" alt="pic" class="forImage" style="margin-top: 2%;float: left;"><span class="manualPath" title="EmailSrcTitle" data-placement="bottom" style="">' + emailMaxChar(data[i].sourceCloudDetails.emailId, 25) + '</span><br/><p style="margin:0px;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;"><span class="csvPath"  style="margin-left: 2%;" title="'+src2+'" data-placement="bottom" style="margin-left: 2%;">' + src1+ '</span></p></td><td style="width: 48%"><img src="../img/PNG/' + data[i].destCloudDetails.name + '.png" alt="pic" class="forImage" style="margin-top: 2%;float: left;"><span class="manualPath" title="EmailDstnTitle">' + emailMaxChar(data[i].destCloudDetails.emailId, 25) + '</span><br/><p style="margin:0px;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;"><span class="csvPath"  style="margin-left: 2%;" title="'+dest2+'">' + dest1 + '</span></p></td><td><img title="Invalid Mapping" alt="pic" src="../img/PNG/cross.png" style="height: 15px;width: 15px;vertical-align: -webkit-baseline-middle;"></td></tr>';
                        else
						var _html = '<tr style="width: 100%" id="csvRow"  teamFolder="' + data[i].teamFolder +'" csvid="' + data[i].csvId + '" srcemail="' + data[i].sourceCloudDetails.emailId + '" dstnemail="' + data[i].destCloudDetails.emailId + '" sourcAdminCloudId="' + data[i].sourceAdminCloudId + '" destAdminCloudId="' + data[i].destAdminCloudId + '" csvname="' + csvName + '" srccldid="' + data[i].sourceCloudId + '"  srcrt="' + data[i].sourceCloudDetails.rootFolderId + '" dstncldid="' + data[i].destCloudId + '" dstnrt="' + data[i].destCloudDetails.rootFolderId + '" class="CsvMapd" srcPathRootFolderId="' + data[i].sourceCloudDetails.pathRootFolderId + '" srcFolderPath="' + src2 + '"  srcRootFolderId="' + data[i].sourceCloudDetails.rootFolderId + '"  migrateSrcFolderName="' + data[i].sourceCloudDetails.migrateFolderName + '" srcCldName="' + data[i].sourceCloudDetails.name + '"dstnCldName="' + data[i].destCloudDetails.name + '" dstnPathRootFolderId="' + data[i].destCloudDetails.pathRootFolderId + '" dstnRootFolderId="' + data[i].destCloudDetails.rootFolderId + '" dstnFolderPath="' + dest2 + '" migrateDstnFolderName="' + data[i].migrateFolderName + '">'+input+'<td style="width: 45%"><img src="../img/PNG/' + data[i].sourceCloudDetails.name + '.png" alt="pic" class="forImage" style="margin-top: 2%;float: left;"><span class="manualPath" title="EmailSrcTitle" data-placement="bottom" style="">' + emailMaxChar(data[i].sourceCloudDetails.emailId, 25) + '</span><br/><p style="margin:0px;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;"><span class="csvPath"  style="margin-left: 2%;" title="'+src2+'" data-placement="bottom" style="margin-left: 2%;">' + src1+ '</span></p></td><td style="width: 48%"><img src="../img/PNG/' + data[i].destCloudDetails.name + '.png" alt="pic" class="forImage" style="margin-top: 2%;float: left;"><span class="manualPath" title="EmailDstnTitle">' + emailMaxChar(data[i].destCloudDetails.emailId, 25) + '</span><br/><p style="margin:0px;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;"><span class="csvPath"  style="margin-left: 2%;" title="'+dest2+'">' + dest1 + '</span></p></td><td><img title="Valid Mapping" alt="pic" src="../img/PNG/greenTick.png" style="height: 11px;width: 11px;vertical-align: -webkit-baseline-middle;"></td></tr>';
						_html = _html.replace('TitleSrce', src2).replace('TitleDstin', dest2).replace('EmailSrcTitle', data[i].sourceCloudDetails.emailId).replace('EmailDstnTitle',  data[i].destCloudDetails.emailId);
						}
						else{
...
function dataList1(thisevent, pgno,nxtToken,email){
    var triggeredclass = $(thisevent).attr('id');
    var _cloudId = $(thisevent).attr('user-id');
    var _folderId = encodeURIComponent($(thisevent).attr('folder-id'));
    var _userId = localStorage.getItem("UserId");
		...
		var dataLen = data.length -1;
            ...
            for(var i=0;i<data.length;i++){
                ...
                if(data[i].cloudName == 'SHAREPOINT_ONLINE_BUSINESS' && data[i].type == 'SITE')
                {
                    array.push(data[i]);
                     ...
                     if ($(thisevent).parents('.mapp-blck').hasClass('src-usrs'))
                         _html += '<p style="display:block;margin-left:22px;width:100%;" class="usr-data"><span user-id="' + data[i].cloudId + '" fromCloudId="' + data[i].cloudId + '" _folderid="' + data[i].id.split("/")[1] + '" folder-id="' + data[i].id.split("/")[2] + '" parentid="' + data[i].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="checkbox"  name="firstInnChkBox" parentPath="/" folderPath="/' + data[i].objectName + '" cldname="' + data[i].cloudName + '"cloudid="' + data[i].cloudId + '" rtfolid="' + data[i].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" class="folderChkBox"/><i class="lnil lnil-folder" style="height: 15px;width: 15px;margin-left: 7px;margin-right: -3px;margin-top: -2px;"></i><span title="' + foldrHover + '" style="font-size:initial;width:100%;">&nbsp;&nbsp;' + data[i].objectName + '</span></p>';
                     else {
							if (i === 14)
                                 _html += '<div style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive" nextPageToken="' + data[i].nextPageToken + '"><div><input type="radio" parentPath="/" folderPath="/' + data[i].objectName + '" cldname="' + data[i].cloudName + '"cloudid="' + data[i].cloudId + '" rtfolid="' + data[i].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="' + data[i].cloudName + '" user-id="' + data[i].cloudId + '" toCloudId="' + data[i].cloudId + '" folder-id="' + data[i].id + '" parentid="' + data[i].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;color: gray;font-size: 20px;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 5px;margin-right: -6px;margin-top: -2px;"><span id="textSpan" title="' + foldrHover + '" style="font-size:initial;position: absolute;margin-left:16px;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName, 23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
							else
							     _html += '<div style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data" nextPageToken="' + data[i].nextPageToken + '"><div><input type="radio" parentPath="/" folderPath="/' + data[i].objectName + '" cldname="' + data[i].cloudName + '"cloudid="' + data[i].cloudId + '" rtfolid="' + data[i].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="' + data[i].cloudName + '" user-id="' + data[i].cloudId + '" toCloudId="' + data[i].cloudId + '" folder-id="' + data[i].id + '" parentid="' + data[i].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;color: gray;font-size: 20px;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 5px;margin-right: -6px;margin-top: -2px;"><span id="textSpan" title="' + foldrHover + '" style="font-size:initial;position: absolute;margin-left:16px;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName, 23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                     }
                 }
                else if(data[i].type == 'FOLDER' || data[i].type == 'DRIVE'||data[i].type == 'DOCUMENT_LIBRARY') {
                    if ($(thisevent).parents('.mapp-blck').hasClass('src-usrs')){
			...
			dataId = dataId.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
                       if(fldrStorage(_obj,'check1')){
							...
							else if(data[i].cloudName === "DROPBOX_BUSINESS"){
							var input = '<input type="checkbox"  name="firstInnChkBox" parentPath="/" folderPath="/'+ _objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ dataId +'"usremail="'+ email +'" style="margin-left:20%;margin-top:27%;" class="folderChkBox"   checked="checked"/>';	
							}
                            else
                            var input = '<input type="checkbox"  name="firstInnChkBox" parentPath="/" folderPath="/'+ _objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ dataId +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" class="folderChkBox"   checked="checked"/>';
                        }
                            ...
                            var input = '<input type="checkbox"  name="firstInnChkBox" parentPath="/" folderPath="/'+ _objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ dataId +'"usremail="'+ email +'" style="margin-left:20%;margin-top:27%;" class="folderChkBox"/>';
							}
                            else
                            var input = '<input type="checkbox"  name="firstInnChkBox" parentPath="/" folderPath="/'+ _objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ dataId +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" class="folderChkBox"/>';
                        }
                        if(data[i].cloudName === "DROPBOX_BUSINESS" && ((localStorage.getItem('multiUsrSrcCldName') === "DROPBOX_BUSINESS")&& (localStorage.getItem('multiUsrDstnCldName') === "SHAREPOINT_ONLINE_BUSINESS"))){
                          
                            if(i === 14)
                                _html += '<div style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data scrollActive" nextPageToken="'+data[i].nextPageToken+'"><div style="float:left;">'+input+'</div><div  name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + dataId + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;display:flex;color: black;font-size: 20px;"><img id="changeIcon" src="../images/icons/folder.png"style="height: 20px;width: 20px;margin-left: -2%;margin-right: 3%;"><span id="hideGt"display:none;> > </span><span id="textSpan"  title="'+foldrHover1+'" style="font-size:initial;width:100%;position:absolute;margin-left: 10%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span></div><span id="folderLoading" style="position: absolute;top: 80%;left: 25%;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                            else
                                _html += '<div style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data" nextPageToken="'+data[i].nextPageToken+'"><div style="float:left;">'+input+'</div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + dataId + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;color: black;font-size: 20px;display:flex;"><img id="changeIcon" src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: -2%;margin-right: 3%;"><span id="hideGt" style="display:none;"> > </span><span id="textSpan"  title="'+foldrHover1+'" style="font-size:initial;width:100%;position:absolute;margin-left: 10%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span></div><span id="folderLoading" style="position: absolute;top:80%;left: 25%;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                        }
                        else{
                            if(_foldrName >= 23) {
                                if(i === 14)
                                _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive" nextPageToken="'+data[i].nextPageToken+'"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + dataId + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span>'+input+'<i class="lnil lnil-folder" style="height: 15px;width: 15px;margin-left: 7px;margin-right: -3px;margin-top: -2px;"></i><span id="textSpan"  title="'+foldrHover1+'" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                                else
                                _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data" nextPageToken="'+data[i].nextPageToken+'"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + dataId + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span>'+input+'<i class="lnil lnil-folder" style="height: 15px;width: 15px;margin-left: 7px;margin-right: -3px;margin-top: -2px;"></i><span id="textSpan"  title="'+foldrHover1+'" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                            }
                            else
                            if(i === 14)
                                _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive" nextPageToken="'+data[i].nextPageToken+'"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + dataId + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span>'+input+'<i class="lnil lnil-folder" style="height: 15px;width: 15px;margin-left: 7px;margin-right: -3px;margin-top: -2px;"></i><span id="textSpan"  style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
			...
			dataId = dataId.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
                        if (data[i].cloudName === "DROPBOX_BUSINESS" || data[i].cloudName === "GOOGLE_SHARED_DRIVES") {
                            if(_foldrName >= 23) {
                                if(i === 14)
                                    _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive" nextPageToken="' + data[i].nextPageToken + '"><span name="'+data[i].cloudName +'" user-id="' + data[i].cloudId + '" toCloudId="' + data[i].cloudId + '" folder-id="' + dataId + '" parentid="' + data[i].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" parentPath="/" folderPath="/' + _objName + '" cldname="' + data[i].cloudName + '"cloudid="' + data[i].cloudId + '" rtfolid="' + data[i].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" name="folderradiobtn"/><i class="lnil lnil-folder" style="height: 15px;width: 15px;margin-left: 7px;margin-right: -3px;margin-top: -2px;"></i><span id="textSpan" title="' + foldrHover1 + '" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(_objName, 23) + '</span><span id="folderLoading" style="position: absolute;padding-left: 7px;top: 65%;left: 11%;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;"><span style="position: absolute;">Loading...</span></span></span></p>';
                                else
                                    _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data" nextPageToken="' + data[i].nextPageToken + '"><span name="'+data[i].cloudName +'" user-id="' + data[i].cloudId + '" toCloudId="' + data[i].cloudId + '" folder-id="' + dataId + '" parentid="' + data[i].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" parentPath="/" folderPath="/' + _objName + '" cldname="' + data[i].cloudName + '"cloudid="' + data[i].cloudId + '" rtfolid="' + data[i].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" name="folderradiobtn"/><i class="lnil lnil-folder" style="height: 15px;width: 15px;margin-left: 7px;margin-right: -3px;margin-top: -2px;"></i><span id="textSpan" title="' + foldrHover1 + '" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars($.trim(_objName), 23) + '</span><span id="folderLoading" style="position: absolute;padding-left: 7px;top: 65%;left: 11%;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;"><span style="position: absolute;">Loading...</span></span></span></p>';
                            }
                            else
                            if(i === 14)
                                _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive" nextPageToken="' + data[i].nextPageToken + '"><span name="'+data[i].cloudName +'" user-id="' + data[i].cloudId + '" toCloudId="' + data[i].cloudId + '" folder-id="' + dataId + '" parentid="' + data[i].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" parentPath="/" folderPath="/' + _objName  + '" cldname="' + data[i].cloudName + '"cloudid="' + data[i].cloudId + '" rtfolid="' + data[i].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" name="folderradiobtn"/><i class="lnil lnil-folder" style="height: 15px;width: 15px;margin-left: 7px;margin-right: -3px;margin-top: -2px;"></i><span id="textSpan" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(_objName, 23) + '</span><span id="folderLoading" style="position: absolute;padding-left: 7px;top: 65%;left: 11%;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;"><span style="position: absolute;">Loading...</span></span></span></p>';
            ...
            if(array.length !== 0 ){
                ...
                for(var j=0;j<array.length;j++){
       			...
       			var pl = /\</g;
                 	...
                 	folderHover = folderHover.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
                    if ($(thisevent).parents('.mapp-blck').hasClass('src-usrs'))
                        _html += '<p style="display:block;margin-left:40px;width:100%;" class="usr-data"><span user-id="'+array[j].cloudId+'" fromCloudId="'+array[j].cloudId+'" _folderid="' + array[j].id.split("/")[1] + '" folder-id="' + array[j].id.split("/")[2] + '" parentid="'+ array[j].parent +'" type="'+ array[j].type +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="checkbox"   name="firstInnChkBox" parentPath="/" folderPath="/'+ data[i].objectName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" class="folderChkBox" /><i class="pull-left LVSITE" style="height: 15px;width: 15px;position: relative;left: 45px;"></i><span title="'+folderHover +'" style="font-size:initial;margin-left: 12px;">&nbsp;&nbsp;' + data[i].objectName + '</span></p>';
                    else {
                     if (j === 14)
								_html += '<div style="display:block;position:relative;width:100%;" class="usr-data scrollActive" nextPageToken="' + array[j].nextPageToken + '"><div><input type="radio" disabled="disabled" parentPath="/" folderPath="/' + objName + '" cldname="' + array[j].cloudName + '"cloudid="' + array[j].cloudId + '" rtfolid="' + array[j].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;display: none;" name="folderradiobtn"/></div><div name="' + array[j].cloudName + '" user-id="' + array[j].cloudId + '" toCloudId="' + array[j].cloudId + '" folder-id="' + array[j].id + '" parentid="' + array[j].parent + '" class="usr-folder1 right" style="cursor:pointer;color: gray;font-size: 20px;"><img src="../img/PNG/'+array[j].cloudName+'.png" id="changeIcon" style="height: 20px;width: 15px;position: relative;margin-left:11%;"><span id="hideGt" style="display:none;"> > </span><span id="textSpan" title="' + folderHover + '" style="font-size:initial;position: absolute;width:100%;margin-top: 3%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(array[j].objectName, 23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                        else
								_html += '<div style="display:block;position:relative;width:100%;" class="usr-data" nextPageToken="' + array[j].nextPageToken + '"><div><input type="radio"  disabled="disabled"  parentPath="/" folderPath="/' + objName + '" cldname="' + array[j].cloudName + '"cloudid="' + array[j].cloudId + '" rtfolid="' + array[j].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;display: none;" name="folderradiobtn"/></div><div name="' + array[j].cloudName + '" user-id="' + array[j].cloudId + '" toCloudId="' + array[j].cloudId + '" folder-id="' + array[j].id + '" parentid="' + array[j].parent + '" class="usr-folder1 right" style="cursor:pointer;color: gray;font-size: 20px;"><img src="../img/PNG/'+array[j].cloudName+'.png" id="changeIcon" style="height: 20px;width: 20px;position: relative;margin-left:11%;"><span id="hideGt" style="display:none;"> > </span><span id="textSpan" title="' + folderHover + '" style="font-size:initial;position: absolute;width:100%;margin-top: 3%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(array[j].objectName, 23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                    }
                }
            }
            _html += '</div>';
            if($(thisevent).hasClass('fa-minus')|| triggeredclass == "srcFolderScroll" || triggeredclass == "dstnFolderScroll") {
                if ((triggeredclass == "srcFolderScroll" || triggeredclass == "dstnFolderScroll") && (data[0].nextPageToken == "" || data.length < 20)) {
                    $(thisevent).find('.active').append(_html);
                    $(thisevent).find('.active').addClass('ended');
                } else if (triggeredclass == "srcFolderScroll" || triggeredclass == "dstnFolderScroll") {
...
function subDataList1(thisevent, pgno,email,folderPath,dropcont)
{
    var triggeredclass = $(thisevent).attr('id');
    var _cloudId = $(thisevent).parents('.mapp-blck').find('.clickeddiv').attr('user-id');
    var _folderId = encodeURIComponent($(thisevent).parents('.mapp-blck').find('.clickeddiv').attr('folder-id'));
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            /*     if ($(thisevent).parents('.mapp-blck').hasClass('src-usrs')) {
				 ...
				 $(thisevent).children("#changeIcon").attr('src','../images/icons/home.png');
				 $(thisevent).children('#hideGt').css('display', '');
            if (data.length == 0) {
                $(thisevent).siblings('#folderLoading').css('display', 'none');
                $(thisevent).siblings('.fldr_parent1').remove();
                $(thisevent).siblings("#textSpan").css('display', '');
            ...
            if(data.length === 0){
                ...
                $(thisevent).siblings('.fldr_parent1').remove();
		var dataLen = data.length -1;
                _html = '<div class="fldr_parent1" nxtpgtoken="' + data[dataLen].nextPageToken + '" pgno="1" style="margin-top:4%;">';
            }
            var array = [],array1=[];
            for (var i = 0; i < data.length; i++) {
                var createdTime = CFManageCloudAccountsAjaxCall.getDateConversion(data[i].createdTime);
			...
			folderPath = folderPath.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
                 	foldrHover = foldrHover.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
                    if ($(thisevent).parents('.mapp-blck').hasClass('src-usrs'))
		        _html += '<p style="display:block;margin-left:22px;width:100%;" class="usr-data"><span user-id="' + data[i].cloudId + '" fromCloudId="' + data[i].cloudId + '" _folderid="' + data[i].id.split("/")[1] + '" folder-id="' + data[i].id + '" parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + data[i].type + '" parentid="' + data[i].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;font-size: 20px;color: gray;"></span><input type="checkbox" name="folderSubChkBox" parentPath="' + folderPath + '" folderPath="' + folderPath + '/' + objName + '" cldname="' + data[i].cloudName + '"cloudid="' + data[i].cloudId + '" rtfolid="' + data[i].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" name="folderSubChkBox"/><img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 3px;margin-right: -5px;position:relative;bottom:1px;"><span title="' + foldrHover + '" style="font-size:initial;width:100%;">&nbsp;&nbsp;' + objName + '</span></p>';
                    else{
		    	if(data[i].objectName === "Documents"){
                       
                        if(i === 9)
               _html += '<div style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data scrollActive"><div style="float:left;"><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + data[i].type + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;color: gray;font-size: 20px;display:flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -2%;"><div class="dropdown-content" style="top:100%;"><a href="#" usremail="'+ email +'" name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="/" class="dropatag">Home</a></div><span id="hideGt"> > </span></div><span id="textSpan" title="' + foldrHover + '"  style="margin-left:12%;font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                        else
                            _html += '<div style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data"><div style="float:left;"><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + data[i].type + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;color: gray;font-size: 20px;display:flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -2%;"><div class="dropdown-content" style="top:100%;"><a href="#" usremail="'+ email +'" name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="/" class="dropatag">Home</a></div><span id="hideGt"> > </span></div><span id="textSpan" title="' + foldrHover + '"  style="margin-left:12%;font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';

                    }
					else if(data[i].type=="DOCUMENT_LIBRARY"){
					   if(i === 9)
               _html += '<div style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data scrollActive"><div style="float:left;"><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + data[i].type + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;color: gray;font-size: 20px;display:flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -2%;"><div class="dropdown-content" style="top:100%;"><a href="#" usremail="'+ email +'" name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="/" class="dropatag">Home</a></div><span id="hideGt"> > </span></div><span id="textSpan" title="' + foldrHover + '"  style="margin-left:12%;font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                        else
                            _html += '<div style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data"><div style="float:left;"><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + data[i].type + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 12%;color: gray;font-size: 20px;display:flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -2%;"><div class="dropdown-content" style="top:100%;"><a href="#" usremail="'+ email +'" name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="/" class="dropatag">Home</a></div><span id="hideGt"> > </span></div><span id="textSpan" title="' + foldrHover + '"  style="margin-left:12%;font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
		...
		array1.push(data[i]);
			...
			var foldrHover1 = foldrHover.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
			 ...
			 if(fldrStorage(_obj,'check1')){
							...
							var input = '<input type="checkbox" name="folderSubChkBox" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName+'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:4px 0 0 3px;float:left;" name="folderSubChkBox"  checked="true"/>';
							}
                            else
							var input = '<input type="checkbox" name="folderSubChkBox" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName+'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderSubChkBox"  checked="true"/>';
							}
							...
							var input = '<input type="checkbox" name="folderSubChkBox" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:4px 0 0 3px;float:left;" name="folderSubChkBox"/>';								
							}
                            else 
							var input = '<input type="checkbox" name="folderSubChkBox" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderSubChkBox"/>';
                        }
                        if(data[i].cloudName === "DROPBOX_BUSINESS" || data[i].cloudName === "SHAREPOINT_ONLINE_BUSINESS"){
							if(data[i].parentFileRef.parent == "/"){
							 if(i === 9)
                   _html += '<div style="display:block;width:100%;position:relative;margin-bottom:3%;" class="usr-data scrollActive"><div style="float:left;">'+input+'</div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + data[i].type + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 6%;color: black;font-size: 20px;display:inline-flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: -12%;margin-right: 3%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: -22%;margin-right: 3%;margin-top: -2%;"><div class="dropdown-content" style="top:100%;"><a href="#" usremail="'+ email +'" name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="/" class="dropatag">Home</a></div><span id="hideGt"> > </span></div><span id="textSpan" title="'+foldrHover1+'"  style="font-size:initial;position: absolute;width:100%;margin-left:10%;margin-top:0.5%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span></div><span id="folderLoading" style="top: 60%;left: 24%;position: absolute;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;padding-left: 25%;">Loading...</span></span></div>';
                            else 
                                _html += '<div style="display:block;width:100%;position:relative;margin-bottom:3%;" class="usr-data"><div style="float:left;">'+input+'</div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + data[i].type + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 6%;color: black;font-size: 20px;display:inline-flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: -12%;margin-right: 3%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: -22%;margin-right: 3%;margin-top: -2%;"><div class="dropdown-content" style="top:100%;"><a href="#" usremail="'+ email +'" name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="/" class="dropatag">Home</a></div><span id="hideGt"> > </span></div><span id="textSpan" title="'+foldrHover1+'"  style="font-size:initial;position: absolute;width:100%;margin-left:10%;margin-top:0.5%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span></div><span id="folderLoading" style="top: 60%;left: 24%;position: absolute;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;padding-left: 25%;">Loading...</span></span></div>';				
							}
							else{
						   if(i === 9)
                   _html += '<div style="display:block;width:100%;position:relative;margin-bottom:3%;" class="usr-data scrollActive"><div style="float:left;">'+input+'</div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 3%;color: black;font-size: 20px;display:inline-flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 25%;margin-right: 4%;margin-top: -8%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -8%;">dropcontent<span id="hideGt"> > </span></div><span id="textSpan" title="'+foldrHover1+'"  style="font-size:initial;position: absolute;width:100%;margin-left:12%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span></div><span id="folderLoading" style="top: 60%;left: 24%;position: absolute;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;padding-left: 25%;">Loading...</span></span></div>';
                            else 
                                _html += '<div style="display:block;width:100%;position:relative;margin-bottom:3%;" class="usr-data"><div style="float:left;">'+input+'</div><div name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 3%;color: black;font-size: 20px;display:inline-flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 25%;margin-right: 4%;margin-top: -8%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -8%;">dropcontent<span id="hideGt"> > </span></div><span id="textSpan" title="'+foldrHover1+'"  style="font-size:initial;position: absolute;width:100%;margin-left:12%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span></div><span id="folderLoading" style="top: 60%;left: 24%;position: absolute;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;padding-left: 25%;">Loading...</span></span></div>';
                        _html = _html.replace('dropcontent',dropcont[0].outerHTML); 
							}
						}
                        else{
                            if(_foldrName >= 23) {
                                if(i === 9) 
                                    _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span>'+input+'<img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 10px;margin-right: -3px;"><span id="textSpan" title="'+foldrHover1+'" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span><span id="folderLoading" style="top: 65%;left: 11%;position: absolute;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                                else
                                    _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span>'+input+'<img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 10px;margin-right: -3px;"><span id="textSpan" title="'+foldrHover1+'" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span><span id="folderLoading" style="top: 65%;left: 11%;position: absolute;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                            }
                            else
                            if(i === 9)
                                _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span>'+input+'<img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 10px;margin-right: -3px;"><span id="textSpan"  style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(foldrHover1,23) + '</span><span id="folderLoading" style="top: 65%;left: 11%;position: absolute;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                        ...
                        if(data[i].cloudName === "DROPBOX_BUSINESS" || data[i].cloudName === "GOOGLE_SHARED_DRIVES"){
                            if(_foldrName >= 23) {
                                if(i === 9)
                                    _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/><img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 10px;margin-right: -3px;"><span id="textSpan"  title="'+foldrHover+'"  style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                                else
                                    _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/><img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 10px;margin-right: -3px;"><span id="textSpan"  title="'+foldrHover+'"  style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                            }
                            else
                            if(i === 9)
                                _html += '<p style="display:block;margin-left:22px;position:relative;margin-bottom: 6%;width:100%;" class="usr-data scrollActive"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/><img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 10px;margin-right: -3px;"><span id="textSpan"  style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                            ...
                            if(_foldrName >= 23) {
                                if(i === 9)
                                    ...
                                    _html += '<p style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data"><span name="'+data[i].cloudName +'" user-id="'+data[i].cloudId+'" fromCloudId="'+data[i].cloudId+'" folder-id="' + data[i].id + '" parentid="'+ data[i].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ data[i].objectName +'" cldname="'+ data[i].cloudName +'"cloudid="'+ data[i].cloudId +'" rtfolid="'+ data[i].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/><img src="../images/icons/folder.png" style="height: 15px;width: 15px;margin-left: 10px;margin-right: -3px;"><span id="textSpan" title="'+foldrHover+'" style="font-size:initial;position: absolute;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(data[i].objectName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
		...
		if(array1.length !==0){
		 ...
		 var folderHover = array1[k].objectName;
			...
			folderPath = folderPath.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
                        ...
                        if(k === 9)
                   _html += '<div style="display:block;width:100%;position:relative;margin-bottom:3%;" class="usr-data scrollActive"><div style="float:left;"><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ array1[k].cloudName +'"cloudid="'+ array1[k].cloudId +'" rtfolid="'+ array1[k].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="'+array1[k].cloudName +'" user-id="'+array1[k].cloudId+'" fromCloudId="'+array1[k].cloudId+'" folder-id="' + array1[k].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + array1[k].type + '" parentid="'+ array1[k].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 3%;color: gray;font-size: 20px;display:inline-flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 20%;margin-right: 4%;margin-top: -8%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -8%;">dropcontent<span id="hideGt"> > </span></div><span id="textSpan" title="'+folderHover+'" style="font-size:initial;position: absolute;width:100%;margin-left:12%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
                        else
                                _html += '<div style="display:block;width:100%;position:relative;margin-bottom:3%;" class="usr-data"><div style="float:left;"><input type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ array1[k].cloudName +'"cloudid="'+ array1[k].cloudId +'" rtfolid="'+ array1[k].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;" name="folderradiobtn"/></div><div name="'+array1[k].cloudName +'" user-id="'+array1[k].cloudId+'" fromCloudId="'+array1[k].cloudId+'" folder-id="' + array1[k].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + array1[k].type + '" parentid="'+ array1[k].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 3%;color: gray;font-size: 20px;display:inline-flex;"><img src="../images/icons/folder.png" style="height: 20px;width: 20px;margin-left: 20%;margin-right: 4%;margin-top: -8%;"><div class="dropdown" style="display:none;"><img class="dropbtn" src="../images/icons/tree.png" style="height: 20px;width: 20px;margin-left: 3%;margin-right: 3%;margin-top: -8%;">dropcontent<span id="hideGt"> > </span></div><span id="textSpan" title="'+folderHover+'" style="font-size:initial;position: absolute;width:100%;margin-left:12%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span></div><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></div>';
				_html = _html.replace('dropcontent',dropcont[0].outerHTML);
			...
			var pl = /\</g;
                 	...
                 	folderHover = folderHover.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
                        folderPath = folderPath.replace(pl,'&lt;').replace(pl1,'&gt;').replace(pl2,'&quot;').replace(pl3,'&apos;');
			if ($(thisevent).parents('.mapp-blck').hasClass('src-usrs'))
                        _html += '<p style="display:block;margin-left:10px;width:100%;" class="usr-data"><span user-id="' + array[j].cloudId+'" fromCloudId="' + array[j].cloudId + '" _folderid="' + array[j].id.split("/")[1] + '" folder-id="' + array[j].id + '" parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + array[j].type + '" parentid="' + array[j].parent + '" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;font-size: 20px;color: gray;"></span><input type="checkbox" name="folderSubChkBox" parentPath="' + folderPath + '" folderPath="' + folderPath + '/' + objName + '" cldname="' + array[j].cloudName + '"cloudid="' + array[j].cloudId + '" rtfolid="' + array[j].id + '"usremail="' + email + '" style="margin:-2px 0 0 14px;" class="folderChkBox" /><i class="pull-left LVSITE" style="height: 15px;width: 15px;position: relative;left: 45px;"></i>><span title="' + folderHover + '" style="font-size:initial;margin-left: 12px;width:100%;">&nbsp;&nbsp;' + objName + '</span></p>';
                    else{
                        if(_folderName >= 23) {
                            if(j === 9)
                                _html += '<p style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data scrollActive"><span name="'+array[j].cloudName +'" user-id="'+array[j].cloudId+'" fromCloudId="'+array[j].cloudId+'" folder-id="' + array[j].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + array[j].type + '" parentid="'+ array[j].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio"  disabled="disabled"  parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ array[j].cloudName +'"cloudid="'+ array[j].cloudId +'" rtfolid="'+ array[j].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;display: none;" name="folderradiobtn"/><i class="pull-left LVSITE" style="height: 15px;width: 15px;position: relative;left: 45px;"></i><span id="textSpan" title="'+folderHover+'" style="font-size:initial;position: absolute;margin-left:26px;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                            else
                                _html += '<p style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data"><span name="'+array[j].cloudName +'" user-id="'+array[j].cloudId+'" fromCloudId="'+array[j].cloudId+'" folder-id="' + array[j].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + array[j].type + '" parentid="'+ array[j].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" disabled="disabled"  parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ array[j].cloudName +'"cloudid="'+ array[j].cloudId +'" rtfolid="'+ array[j].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;display: none;" name="folderradiobtn"/><i class="pull-left LVSITE" style="height: 15px;width: 15px;position: relative;left: 45px;"></i><span id="textSpan" title="'+folderHover+'" style="font-size:initial;position: absolute;margin-left:26px;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                        ...
                        if(j === 9)
                            _html += '<p style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data scrollActive"><span name="'+array[j].cloudName +'" user-id="'+array[j].cloudId+'" fromCloudId="'+array[j].cloudId+'" folder-id="' + array[j].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + array[j].type + '" parentid="'+ array[j].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input  disabled="disabled"   type="radio" parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ array[j].cloudName +'"cloudid="'+ array[j].cloudId +'" rtfolid="'+ array[j].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;display: none;" name="folderradiobtn"/><i class="pull-left LVSITE" style="height: 15px;width: 15px;position: relative;left: 45px;"></i><span id="textSpan"  style="font-size:initial;position: absolute;margin-left:26px;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
                        else
                            _html += '<p style="display:block;margin-left:22px;width:100%;position:relative;margin-bottom: 6%;" class="usr-data"><span name="'+array[j].cloudName +'" user-id="'+array[j].cloudId+'" fromCloudId="'+array[j].cloudId+'" folder-id="' + array[j].id + '"  parent-name="' + _parentName + '" folder-name="' + objName + '" folder-type="' + array[j].type + '" parentid="'+ array[j].parent +'" class="usr-folder1 right" style="cursor:pointer;margin-left: 5%;color: gray;font-size: 20px;"></span><input type="radio" disabled="disabled"  parentPath="'+folderPath+'" folderPath="'+folderPath+'/'+ objName +'" cldname="'+ array[j].cloudName +'"cloudid="'+ array[j].cloudId +'" rtfolid="'+ array[j].id +'"usremail="'+ email +'" style="margin:-2px 0 0 14px;display: none;" name="folderradiobtn"/><i class="pull-left LVSITE" style="height: 15px;width: 15px;position: relative;left: 45px;"></i><span id="textSpan"  style="font-size:initial;position: absolute;margin-left:26px;width:100%;">&nbsp;&nbsp;' + CFManageCloudAccountsAjaxCall.getMaxChars(objName,23) + '</span><span id="folderLoading" style="position: absolute;top: 65%;left: 11%;padding-left: 7px;display: none;"><img  src="../img/ajax-loader11.gif" alt="img" ><span style="position: absolute;">Loading...</span></span></p>';
            ...
            _html += '</div>';
            if ($(thisevent).hasClass('down')|| triggeredclass == "srcFolderScroll" || triggeredclass == "dstnFolderScroll") {
                if ((triggeredclass == "srcFolderScroll" || triggeredclass == "dstnFolderScroll") && (data[0].nextPageToken == "" || data.length < 10)) {
                    $(thisevent).find('.clickeddiv').parent('.usr-data').append(_html);
                    $(thisevent).find('.active').addClass('ended');
                } else if (triggeredclass == "srcFolderScroll" || triggeredclass == "dstnFolderScroll") {
					...
					$(thisevent).parent('.usr-data').append(_html);
										...
										$(thisevent).parents('.active').parent('.usr-data').children('.usr-folder1').css('display','none');
										if(data.length == 0){
											$(thisevent).parents('.active').parent('.usr-data').children('div').children('input').css('display','none');
										$(thisevent).parent('.usr-data').children('.usr-folder1').children('img').css("display","none");
					$(thisevent).parent('.usr-data').children('.usr-folder1').children('.dropdown').css("display","flex");
										}
										else{
					if((data[0].cloudName == 'SHAREPOINT_ONLINE_BUSINESS' && data[0].parentFileRef == null)  || data[0].parentFileRef.parent == "/"){
					$(thisevent).parent('.usr-data').children('.usr-folder1').children('img').css('display','');	 
					}
					else{
					$(thisevent).parents('.active').parent('.usr-data').children('div').children('input').css('display','none');						
					$(thisevent).parent('.usr-data').children('.usr-folder1').children('img').css("display","none");
					...
					//$(thisevent).parent('.usr-data').children('.usr-folder1').children('.dropdown').css('display','');
...
$('#clearSelected').click(function() {
   ...
   /* var parent = $(this).parents("tr");
    ...
    var _path = $(this).parents("tr").attr('pathException');
	var csvid = $(this).parents("tr").attr('csvid');*/
	var length=$("#mapdUsers tbody tr").length;
	var array = [];
	for (var i=0;i<length;i++){
		if($("#mapdUsers tbody tr:eq("+i+") td:eq(0) input:checked").length){
			var row = $("#mapdUsers tbody tr:eq("+i+")");
			var obj = {
				"fromRootFolderId": row.attr("srcrt"),
				"toRootFolderId": row.attr("dstnrt"),
				"fromFolderPath": row.attr("srcfolderpath"),
				"toFolderPath": row.attr("dstnfolderpath"),
				"sourceCloudId":row.attr("srcCldid"),
				"destCloudId":row.attr("dstnCldid"),
			"pathException": null				
			}
			$("#mapdUsers tbody tr:eq("+i+")").remove();
			i--;
			array.push(obj);
		}
	}
    ...
    $.ajax({
        type: "DELETE",
        url: apicallurl + "/migrationmapping/delete/mappingpairs?sourceAdminCloudId="+localStorage.multiUsrSrcCldId+"&destAdminCloudId="+localStorage.multiUsrDstnCldId,
