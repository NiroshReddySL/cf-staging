let usersMappingRequest, channelsMappingRequest;
...
function slackChannels(cloudId, type, pgNo) {
  ...
  channelsMappingRequest = $.ajax({
    ...
    success: function (data) {
      ...
      if(data === "No Provision User Found" && fetchType === "public"){
      ...
      if (data.length > 0) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (
            ...
            var destTeamName = null;
          } else if (data[i].destTeamName) {
  ...
  usersMappingRequest = $.ajax({
    ...
    success: function (data) {
      ...
      if (data.length == 0) {
        ...
        $("#selDiv").css("display", "none");
        $("#selCount").text("00");
        $("#matchedNewCount").text($('[data-attr="matchedUserPairs"]').length);
      }
