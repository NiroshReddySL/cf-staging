...
function getMuralReports(pgNo) {
  //var pgNo = 1;
  $(".actionPannel").find("div").css("display", "none");
  var userId = localStorage.getItem("UserId");
  var moveStatus = {
    PROCESSED: "Processed",
    RETRYING: "Retrying",
  ...
  $.ajax({
    type: "GET",
    url: "http://127.0.0.1:8000/mural/reports/new",
    // url:
    //   apicallurl +
    ...
    success: function (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (
          ...
          sourceBoardName = escapeSpecialChar(sourceBoardName);
        }
        if (
          ...
          destinationBoardName = escapeSpecialChar(destinationBoardName);
        }

        let processStauts;
        if (_data.processStatus === "PROCESSED_WITH_SOME_CONFLICTS") {
          processStauts = "PROCESSED";
        } else {
          processStauts = _data.processStatus;
        }
        ...
        if (
          ...
          var inputCheckbox = `<input type="checkbox" class="inputReportsMural" disabled/>`;
        } else {
          var inputCheckbox = `<input type="checkbox" wsid="${_data?.id}" id="actinCheckBox" status="${processStauts}" class="inputReportsMural"/>`;
        }

        let srcLink, dstnLink;
        if (_data?.sourceBoardLink) {
          srcLink = `<i class="fa fa-external-link openLinksMural" id="openReportsLink" cloudname="MURAL" openlink="${encodeURIComponent(
            _data?.sourceBoardLink
          )}"  title="Open Board In Mural"></i>`;
        } else {
          srcLink = `<i class="fa fa-external-link openLinksMural" style="visibility: hidden;"></i>`;
        }

        if (_data?.destBoardLink) {
          dstnLink = `<i class="fa fa-external-link openLinksMural" id="openReportsLink" cloudname="MIRO" openlink="${encodeURIComponent(
            _data?.destBoardLink
          )}" title="Open Board In Miro"></i>`;
        } else {
          dstnLink = `<i class="fa fa-external-link openLinksMural" style="visibility: hidden;"></i>`;
        }
        $("#muralReportsTable").append(`<tr>
        <td style="width: 20px;">${inputCheckbox}</td>
        <td title="${sourceBoardName}">${CFManageCloudAccountsAjaxCall.getMaxChars(
          sourceBoardName,
          25
        )} ${srcLink}</td>
        <td title="${destinationBoardName}">${CFManageCloudAccountsAjaxCall.getMaxChars(
...
var preMigFilter = {
  appendUsers: function (userName) {
    $(".selectRoomFilter").html(
      `<option value="CFDEFAULTROOM">Select Created By</option>`
    );
...
$(".createBatchBtn").click(function () {
  ...
  if (batchName) {
    ...
    return alertError("BATCH NAME MISSING");
  }
  console.log(newBatchData);
  createNewBatchApi(ids, batchName, newBatchData);
  $(".dropDown-CreateBatch").css("display", "none");
});
...
function createNewBatchApi(ids, batchName, batches) {
  appendNewBatch(batches);
  if (ids.length === 0) {
    return alertError("No Murals selected to create a batch");
  }
  $("#CFShowLoading").show();
  $("#loaderTextS").html("Batch creation is in-progress...");
  $.ajax({
    type: "POST",
    url: `${apicallurl}/boardservice/create/batch/${localStorage.multiUsrSrcCldId}/?batchname=${batchName}`,
    async: true,
    headers: {
      "Content-Type": "application/json",
      Authorization: CFManageCloudAccountsAjaxCall.getAuthDetails(),
    ...
    success: function (data) {
      if (data === "Batch Name updated Successfully ") {
        appendNewBatch(batches);
        alertSuccess("Batch Created Successfully");
        $('[id="preMigCheckBox"]:checked').parent().parent().remove();
      }
      $("#CFShowLoading").hide();
    ...
    error: function (err) {
      $("#CFShowLoading").hide();
      alertError("Falied to create batch...");
      $("#loaderTextS").html("Please wait while loading...");
    },
  });
}
...
function appendNewBatch(batches) {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    if (_data.batchName) {
      $(".batchName-Filters").css("display", "flex");
      if (!batchArray.includes(_data.batchName)) {
        batchArray.push(_data.batchName);
      }
      $("#muralPreMigrationBoardsTable")
        .prepend(`<tr class="hasBatch" trbatchname="${escapeSpecialChar(
        _data.batchName
      )}">
              <td><input type="checkbox" id="preMigCheckBox" batchNameMural="${escapeSpecialChar(
                _data.batchName
              )}" boardObject="${encodeURIComponent(
        JSON.stringify(_data)
      )}"/></td>
              <td title="${boardName}" style="width: 255px;" created="${getPremigrationTimeStamp(
        _data.boardCreatedTime
      )}" updated="${getPremigrationTimeStamp(
        _data.boardLastModifiedTime
      )}">${CFManageCloudAccountsAjaxCall.getMaxChars(boardName, 25)}</td>
                      <td title="${createdBy}">${CFManageCloudAccountsAjaxCall.getMaxChars(
        createdBy,
        25
      )}</td>
                      <td>${_data?.batchName ? _data?.batchName : "-"}</td>
      ...
      $("#muralPreMigrationBoardsTable").append(`<tr>
              <td><input type="checkbox" id="preMigCheckBox" bordid="${
                _data.id
              }" boardObject="${encodeURIComponent(
        JSON.stringify(_data)
      )}"/></td>
              <td style="width: 255px;" title="${boardName}" created="${getPremigrationTimeStamp(
        _data.boardCreatedTime
      )}" updated="${getPremigrationTimeStamp(
        _data.boardLastModifiedTime
      )}">${CFManageCloudAccountsAjaxCall.getMaxChars(boardName, 25)}</td>
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (data?.length > 0) {
        data.map((_data) => {
          console.log(_data);
          if (!workSpacesArray.includes(_data?.muralWorkspaceName)) {
            workSpacesArray.push(_data?.muralWorkspaceName);
            $("#appendWorkspaces").append(
              '<div class="row justify-content-start muralWorkspaces openWorkSpace">' +
                '<div class="my-auto text-center"><i class="lnil lnil-chevron-up-circle" style="color: #0062ff;cursor:pointer;" id="openRooms" ></i></div>' +
                '<div class="my-auto"><span style="margin-left: -20px;text-overflow: ellipsis;" title="' +
          ...
          if (workSpacesArray.includes(_data?.muralWorkspaceName)) {
            if (!roomsArray.includes(_data?.roomName)) {
              roomsArray.push(_data?.roomName);
              $('[id="viewRoom' + _data?.muralWorkspaceName + '"]').append(
                '<div class="justify-content-start" id="roomsBody"><div class="my-auto text-center" style="cursor:pointer;" id="closeRoomMural">' +
                  '<i class="fa fa-folder-open" aria-hidden="true"></i><span style="font-size: 14px; margin-left: 5px;text-overflow: ellipsis;" title="' +
                  _data?.roomName +
                  '">' +
                  CFManageCloudAccountsAjaxCall.getMaxChars(
                    _data?.roomName,
          ...
          $('[id="showFolders' + _data?.roomName + '"]').append(
            '<div class="" id="viewMural"><div class="my-auto text-center">' +
              '<span class="selectMuralSPAN"><input type="checkbox" id="boardLevel" fromBoardId="' +
              _data.fromBoardId +
              '" sourceBoardName="' +
              escapeSpecialChar(_data.boardName) +
              '" parentBid="' +
              _data.id +
              '"><span title="' +
              escapeSpecialChar(_data.boardName) +
              '">' +
              CFManageCloudAccountsAjaxCall.getMaxChars(_data.boardName, 20) +
              "</span></span></div>" +
              '<div class="my-auto" id="hierachyIndicator" style="margin-left: auto;">Mural</div></div>'
          );
          // $("#appendWorkspaces").append(
...
function appendBatchDropDown() {
  $(".dropDown-Batches").html("");
  $(".dropDown-Batches").append(`
      <div id="changeBatchFilter" data-attr="all"><i id="fil-BatchData" class="fa fa-check-circle activeBatchFilter" data-batch-filter="all"></i>
        <span>All</span>
      </div>
    `);
  batchArray.map((data) => {
    $(".dropDown-Batches").append(`
      <div id="changeBatchFilter" data-attr="${data}"><i class="fa fa-check-circle activeBatchFilter" id="fil-BatchData" data-batch-filter="${data}"></i>
        <span>${data}</span>
      </div>
    `);
  });
...
$(".batchName-Filters").click(function () {
  if ($(this).hasClass("activeDropDown")) {
    $(this).removeClass("activeDropDown");
    $(".dropDown-Batches").css("display", "none");
  } else {
    $(this).addClass("activeDropDown");
    $(".dropDown-Batches").css("display", "flex");
  }
});

let filtersArray = [];
$(document).on("click", "#changeBatchFilter", function () {
  $(".dropDown-Batches").css("display", "none");
  let filterName = $(this).attr("data-attr");
  var table = $("#muralPreMigrationBoardsTable tr");

  $('[data-attr="all"]').removeClass("activeBatchFilter");

  if (
    !$('[data-batch-filter="' + filterName + '"]').hasClass("activeBatchFilter")
  ) {
    filtersArray.push(filterName);
    $('[data-batch-filter="' + filterName + '"]').addClass("activeBatchFilter");
    for (i = 0; i < table.length; i++) {
      ...
      if (td) {
        ...
        if (filterName === "all") {
          filtersArray = [];
          // $('[data-attr="all"]').add("activeBatchFilter");
          $('[id="fil-BatchData"]').addClass("activeBatchFilter");
          table[i].style.display = "";
        } else {
          if (txtValue.indexOf(filterName) > -1) {
            table[i].style.display = "";
          } else if (txtValue === "-") {
            table[i].style.display = "none";
          } else if (!filtersArray.includes(txtValue)) {
            table[i].style.display = "none";
          }
        }
      }
    ...
    $('[data-batch-filter="' + filterName + '"]')
      .removeClass("activeBatchFilter")
      .css("display", "");
    ...
    for (i = 0; i < table.length; i++) {
      ...
      if (td) {
        ...
        if (filterName === "all") {
          filtersArray = [];
          $('[id="fil-BatchData"]').removeClass("activeBatchFilter");
          table[i].style.display = "none";
        } else {
          if (txtValue.indexOf(filterName) > -1) {
            table[i].style.display = "none";
...
$(".selectAllPreMigrationMurals").click(function () {
  if ($(".selectAllPreMigrationMurals").is(":checked")) {
    $('[id="preMigCheckBox"]:visible').prop("checked", true);
  } else {
    $('[id="preMigCheckBox"]').prop("checked", false);
  }
});
...
function getPremigrationTimeStamp(timeStamp) {
  let dateConvertedArray = timeStamp.split(":");
  // let formatedDate = `${timeStamp?.split(":")[2]}-${timeStamp?.split(":")[1]}-${
  //   timeStamp?.split(":")[0]
  // }`;
  let formatedDate = `${dateConvertedArray[1]}-${dateConvertedArray[0]}-${dateConvertedArray[2]} ${dateConvertedArray[3]}:${dateConvertedArray[4]}:${dateConvertedArray[5]}`;
  return new Date(formatedDate).getTime();
}

// $(document).on("click", ".openLinksMural", function () {
//   let muralURI =
//     "https://app.mural.co/t/pepperwood5868/m/pepperwood5868/1670419206277/1144b94cb150504638445f2ea176711734f48a16?sender=u5934da9aa7d5e11242067099";
//   muralURI = encodeURIComponent(muralURI);
//   localStorage.setItem("openMuralUrl", muralURI);

//   let miroURI = "https://miro.com/app/board/uXjVPVMEw0E=/";
//   miroURI = encodeURIComponent(miroURI);
//   localStorage.setItem("openMiroUrl", miroURI);

//   // $(".modal-openLinks").css("display", "flex");
// });

// $("#closeOpenLink").click(function () {
//   $(".modal-openLinks").css("display", "none");
// });

$(document).on("click", "#openReportsLink", function () {
  let cloudName = $(this).attr("cloudname");
  let URI;
  if (cloudName === "MURAL") {
    URI = decodeURIComponent($(this).attr("openlink"));
  } else {
    URI = decodeURIComponent($(this).attr("openlink"));
  }
  $(".modal-openLinks").css("display", "none");
...
// let selectedReportsStatus = [];
$(document).on("click", "#actinCheckBox", function () {
  if ($(this).is(":checked")) {
    $(this).removeClass("inputReportsMural");
  } else {
    $(this).addClass("inputReportsMural");
  }

  // let status = $(this).attr("status");
  if ($('[id="actinCheckBox"]:checked').length === 0) {
    selectedReportsStatus = [];
    $("#pauseMuralBoards").parent("div").css("display", "none");
    $("#resumeMuralBoards").parent("div").css("display", "none");
    $("#cancelMuralBoards").parent("div").css("display", "none");
    $("#downloadMuralBoards").parent("div").css("display", "none");
    $(".actionPannel").find("div").css("display", "none");
  }
  // else {
  //   if (!selectedReportsStatus.includes(status)) {
  //     selectedReportsStatus.push(status);
  //   } else {
  //     if (!selectedReportsStatus.includes(status)) {
  //       $(this).prop("checked", false);
  //       alertError("Please select murals with same status");
  //     }
  //   }
  // }

  // selectedReportsStatus.map((_data) => {
  //   if (_data === "PROCESSED") {
  //     $("#downloadMuralBoards").parent("div").css("display", "flex");
  //   } else if (
  //     _data === "NOT_PROCESSED" ||
  //     _data === "NOT_STARTED" ||
  //     _data === "IN_QUEUE" ||
  ...
  //     _data === "IN_PROGRESS"
  //   ) {
  //     $("#pauseMuralBoards").parent("div").css("display", "flex");
  //     $("#cancelMuralBoards").parent("div").css("display", "flex");
  //   } else if (_data === "PAUSE") {
  //     $("#resumeMuralBoards").parent("div").css("display", "flex");
  //   }
  // });

  if ($('[status="PAUSE"]:checked').length === 0) {
    $("#resumeMuralBoards").parent("div").css("display", "none");
  } else {
    $("#resumeMuralBoards").parent("div").css("display", "flex");
  }
  ...
  if (
    $('[status="NOT_PROCESSED"]:checked').length === 0 &&
    $('[status="NOT_STARTED"]:checked').length === 0 &&
    $('[status="IN_QUEUE"]:checked').length === 0 &&
    $('[status="RETRYING"]:checked').length === 0 &&
    $('[status="IN_PROGRESS"]:checked').length === 0 &&
    $('[status="IN_PROGRESS"]:checked').length === 0
  ) {
    $("#pauseMuralBoards").parent("div").css("display", "none");
    $("#cancelMuralBoards").parent("div").css("display", "none");
  ...
  if ($('[status="PROCESSED"]:checked').length === 0) {
    $("#downloadMuralBoards").parent("div").css("display", "none");
  } else {
    $("#downloadMuralBoards").parent("div").css("display", "flex");
  }
...
// $(".dropmatchStatusMural a").click(function () {
//   filter = $(this).text();
//   if (filter === "All") {
//     $(".fa-check-circle.all").css("display", "");
//     $(".fa-check-circle.Processed").css("display", "none");
//     $(".fa-check-circle.in_progress").css("display", "none");
//   } else if (filter === "Completed") {
//     $(".fa-check-circle.all").css("display", "none");
//     $(".fa-check-circle.Processed").css("display", "");
//     $(".fa-check-circle.in_progress").css("display", "none");
//   } else {
//     $(".fa-check-circle.all").css("display", "none");
//     $(".fa-check-circle.Processed").css("display", "none");
//     $(".fa-check-circle.in_progress").css("display", "");
//   }
//   tr = $("#muralReportsTable tr");
//   for (i = 0; i < tr.length; i++) {
//     td = tr[i].getElementsByTagName("td")[7];
//     if (td) {
//       txtValue = td.getAttribute("class");
//       console.log(txtValue);
//       console.log(filter.toUpperCase());
//       if (filter == "All") {
//         tr[i].style.display = "";
//       } else if (txtValue.indexOf(filter.toUpperCase()) > -1) {
//         tr[i].style.display = "";
//       } else {
//         tr[i].style.display = "none";
//       }
