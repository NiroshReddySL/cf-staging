    ...
    success: function (data) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (
          ...
          var inputCheckbox = `<input type="checkbox" wsid="${_data?.id}" id="actinCheckBox" status="${processStauts}"/>`;
        }
        $("#muralReportsTable").append(
          ` <tr>
            ...
            sourceBoardName,
            25
          )} <i class="fa fa-link openLinksMural"></i></td>
			...
			<div style="display: flex;flex-direction: column;gap: 0;">
                        <span>${
                          processStatus[_data?.premigrationBoardStatus]
                            ...
                            <span style="color: #acacac;font-weight: 400;font-size: 10px;">created:${getPremigrationTimeStamp(
                              _data?.boardCreatedTime,
                              "date"
                            )}</span>
                            <span style="color: #acacac;font-weight: 400;font-size: 10px;">Modified:${getPremigrationTimeStamp(
                              _data.boardLastModifiedTime,
                              "date"
                            )}</span>
                        </div>
...
function getPremigrationTimeStamp(timeStamp, format) {
  ...
  if (format === "date") {
    return `${new Date(formatedDate).toString().split(" ")[1]} ${
      new Date(formatedDate).toString().split(" ")[2]
    },${new Date(formatedDate).toString().split(" ")[3]}`;
  } else {
...
$(document).on("click", "#openReportsLink", function () {
  ...
  let URI;
  if (cloudName === "MURAL") {
    URI = decodeURIComponent(localStorage.openMuralUrl);
  } else {
    URI = decodeURIComponent(localStorage.openMiroUrl);
  }
  $(".modal-openLinks").css("display", "none");
...
$(".dropDown-Batches, .dropDown-CreateBatch, .dropDownFilter").mouseleave(
  function () {
