...
function getBoardsFolder(folderId, pgNo, nxToken) {
  ...
  $.ajax({
    type: "GET",
    //url: uri,
    url: "http://127.0.0.1:8000/new/murals",
    aync: false,
    headers: {
...
//        $("#appendWorkspaces").html("");
      ...
      $("#appendWorkspaces table tbody tr").html("")
      for (let i = 0; i < data.length; i++) {
        var _data = data[i];
        $("#appendWorkspaces table tbody").append(
          `<tr> 
 		<td><input type="checkbox" id="boardLevel" fromBoardId="${_data.boardId}" sourceBoardName="${escapeSpecialChar(_data.boardName)}" parentBid="${_data.id}"/></td>
		<td title="${_data.boardName}">${CFManageCloudAccountsAjaxCall.getMaxChars(escapeSpecialChar(_data.boardName), 20)}</td>
		<td title="${_data.boardName}">${CFManageCloudAccountsAjaxCall.getMaxChars(escapeSpecialChar(_data.roomName), 20)}</td>
		<td title="${_data.boardName}">${CFManageCloudAccountsAjaxCall.getMaxChars(escapeSpecialChar(_data.muralWorkspaceName), 20)}</td>
	  </tr>`
        );
      }
    },
      ...
      if (data?.length > 0) {
        for (let i = 0; i < data?.length; i++) {
          ...
          $("#muralPreMigrationBoardsTable").append(`<tr>
		    ...
		    <td><input type="checkbox" workspacename="${_data?.muralWorkspaceName}" boardObject="${encodeURIComponent(JSON.stringify(_data))}" id="checkForPreMig"/></td>
                    <td title="${
...
function csvMuralMappingFileUpload() {
  ...
  reader.onload = function () {
    ...
    valudateCSVMuralMappingUsersUpld(reader.result);
  };
...
function csvMuralMappingUsersUpld(fileStream){
  ...
  $.ajax({
   method:"POST",
   url: apicallurl + "/boardservice/csv/workspaces/"+localStorage.getItem("multiUsrSrcCldId"),
   data: fileStream,
   headers: {
      "Content-Type": "application/json",
      Authorization: CFManageCloudAccountsAjaxCall.getAuthDetails(),
...
function valudateCSVMuralMappingUsersUpld(fileStream){
  ...
  $.ajax({
   method:"POST",
   url: apicallurl + "/boardservice/csv/validation/"+localStorage.getItem("multiUsrSrcCldId"),
   data: fileStream,
   headers: {
      "Content-Type": "application/json",
      Authorization: CFManageCloudAccountsAjaxCall.getAuthDetails(),
