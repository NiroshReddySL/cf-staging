...
var jobStatus = {
  ...
  METADATA_INPROGRESS: "Metadata In Progress",
...
function getEmailReports(pgNo) {
  $('[id="refreshEmailFileFolders"]').attr("id", "refreshEmailJobs");
  $('[id="refreshEmailWorkspace"]').attr("id", "refreshEmailJobs");
...
$(document).on("click", '[id="goToWorkSpaces"]', function () {
  ...
  $("#CFShowLoading").show();
  getEmailWorkspaces(jobId, jobName);
});

function getEmailWorkspaces(jobId, jobName) {
  $("#backToJobs").attr("at", "");
  $("#appendEmailWorkspaces").attr("jobid", jobId);
  $("#appendEmailWorkspaces").attr("jobname", jobName);
  $('[id="refreshEmailFileFolders"]').attr("id", "refreshEmailWorkspace");
  $('[id="refreshEmailJobs"]').attr("id", "refreshEmailWorkspace");
  $("#CFShowLoading").show();
  $.ajax({
    type: "GET",
    ...
    success: function (data) {
      $("#CFShowLoading").hide();
      $('[id="refreshEmailJobs"]').attr("id", "refreshEmailWorkSpaces");
      $(".emailJobsContainer").css("display", "none");
      ...
      for (let i = 0; i < _res.length; i++) {
        let data = _res[i];
        let fromCloudImg, toCloudImage;
        fromCloudImg = data?.fromCloud === "GMAIL" ? "gmail" : "outlook";
        toCloudImage = data?.toCloud === "GMAIL" ? "gmail" : "outlook";

        $("#appendEmailWorkspaces").append(`
          <div class="emailJobDiv">
              <div class="fromEmailMigrationJob">
                  <div class="imageWrapEmail">
                    <img src="../img/${fromCloudImg}.png" style="width: 20px">
                  </div>
              ...
              <div class="fromEmailMigrationJob">
                    <div class="imageWrapEmail">
                      <img src="../img/${toCloudImage}.png" style="width: 20px">
                    </div>
                    <div class="emailWrapperDiv" title="${
                      data?.toMailId
                    }" id="goToEmailFileFolders" workspaceid="${data?.id}">
...
$(document).on("click", "#backToJobs", function () {
  if ($(this).attr("at") === "FIELFOLDERS") {
    getEmailWorkspaces(
      $("#appendEmailWorkspaces").attr("jobid"),
      $("#appendEmailWorkspaces").attr("jobname")
    );
  } else {
    getEmailReports();
...
$(document).on("click", "#goToEmailFileFolders", function () {
  ...
  $("#CFShowLoading").show();
  $("#emailMigrationMails").addClass("active");
  $("#mailsTableData").css("display", "flex");
  $("#foldersTableData").css("display", "none");
  $("#emailMigrationFolders").removeClass("active");
  getEmailWorkFileFoldersSpaces(wsId, "all", 0, false);
  $(".emailWorkspacesFolders").attr("workspaceid", wsId);
});

function getEmailWorkFileFoldersSpaces(wsId, filter, pageNo, folder) {
  $('[id="refreshEmailWorkspace"]').attr("id", "refreshEmailFileFolders");
  $('[id="refreshEmailJobs"]').attr("id", "refreshEmailFileFolders");

  $('[id="refreshEmailFileFolders"]').attr("wsid", wsId);
  $('[id="refreshEmailFileFolders"]').attr("filter", filter);
  $('[id="refreshEmailFileFolders"]').attr("folder", folder);
  ...
  $("#backToJobs").attr("at", "FIELFOLDERS");
    ...
    success: function (data) {
      ...
      if (pageNo === 0) {
        ...
        <div class="emailMigrationFromMail" style="align-items: center;display: flex;gap: 5px;">Status 
          <span class="dropdown" style="position: absolute;right: 240px;display: none;">
      ...
      <div class="emailShowDiv" style="gap:40px;">
      <div class="folderTheadEmail" style="width:55%">Folder Name</div>
      <div class="folderTheadEmail">Total Count</div>
        <div class="folderTheadEmail" style="display: none;">UnRead mails</div>
        <div class="folderTheadEmail" style="display: none;">Processed Size</div>
        <div class="folderTheadEmail" style="align-items: center;display: flex;gap: 5px;">Status <i class="ms-Icon ms-Icon--Filter" style="cursor: pointer;display:none;"></i></div>
      </div>
        ...
        if (!_data?.folder) {
          ...
          if (_data?.mailFolder) {
            if (folderEmailMapping[_data?.mailFolder.toUpperCase()]) {
              ...
              <div class="folderTheadEmail" style="width:55%">
                  ...
                  <div class="folderTheadEmail" style="display: none;" title="${
                    _data?.unreadCount ? _data?.unreadCount : "-"
                  }">${_data?.unreadCount ? _data?.unreadCount : "-"}</div>
                 <div class="folderTheadEmail" style="display: none;">${CFManageCloudAccountsAjaxCall.getObjectSize(
                   _data?.totalCount
                 )}</div>
...
$(document).on("click", "#refreshEmailJobs", function () {
  getEmailReports(0);
});

$(document).on("click", "#refreshEmailWorkspace", function () {
  getEmailWorkspaces(
    $("#appendEmailWorkspaces").attr("jobid"),
    $("#appendEmailWorkspaces").attr("jobname")
  );
...
$(document).on("click", "#refreshEmailFileFolders", function () {
  getEmailWorkFileFoldersSpaces(
    $('[id="refreshEmailFileFolders"]').attr("wsid"),
    $('[id="refreshEmailFileFolders"]').attr("filter"),
    0,
    $("#emailMigrationFolders").hasClass("active")
  );
});
