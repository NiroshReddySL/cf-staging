      ...
      if (data.length > 0) {
        data?.map((res, index) => {
          ...
          if (res?.processStatus === "IN_QUEUE") {
            checkInp = `<input type="checkbox" style="margin: 0" id="selectPreScanUser" cloudid="${res?.fromCloudId}" id="selectPreScanUser"  scanstatus="${res?.processStatus}" wsid="${res?.id}"/>`;
          } else if (res?.processStatus === "NOT_PROCESSED") {
            checkInp = `<input type="checkbox" id="selectLinkExMappings" userid="${res?.userId}" srcadmincldid="${res?.fromAdminCloudId}" dstnadminid="${res?.toAdminCloudId}" destCloudId="${res?.toCloudId}" srcCloudId="${res?.fromCloudId}" scanstatus="${res?.processStatus}" wsid="${res?.id}"/>`;
          } else {
            checkInp = `<input type="checkbox" style="margin: 0" scanstatus="${res?.processStatus}"disabled/>`;
          }
          $(".linkEx-preScan-usersTable-Body").append(`
        <div class="linkEx-preScan-usersTable-bodyData" workspace="${res?.id}">
        ...
        <div class="placeScanStatus" style="position: relative;">
          ...
          ${
            res?.errorDesc
              ? `<span
                title="${res?.errorDesc}" class="showErrDesc">
                <i
          ...
          <span style="position: absolute;right: -130px;cursor: pointer" class="showCancelWorkSpace"><i style="cursor: pointer" class="lnil lnil-cross-circle" style="font-size: 12px !important;" wsid="${
            res?.id
          }" id="removeSingleWS" title="Remove Workspace"></i></span>
        </div>
      </div>
        `);
        });
